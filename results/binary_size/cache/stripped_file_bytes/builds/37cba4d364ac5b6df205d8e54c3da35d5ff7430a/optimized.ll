...
for.cond10:                                       ; preds = %for.inc35, %if.else8
  ...
  %exitcond28.not = icmp eq i32 %i.0, %smax
  br i1 %exitcond28.not, label %while.cond38, label %while.cond

while.cond:                                       ; preds = %for.cond10, %while.body
  %indvars.iv21 = phi i64 [ %indvars.iv.next22, %while.body ], [ 99, %for.cond10 ]
  %cmp13.not = icmp eq i64 %indvars.iv21, 0
  br i1 %cmp13.not, label %while.cond18, label %while.body

while.body:                                       ; preds = %while.cond
  %arrayidx15 = getelementptr inbounds [100 x i32], ptr %re, i64 0, i64 %indvars.iv21
...
while.cond18:                                     ; preds = %while.cond, %if.end
  %indvars.iv24 = phi i64 [ %indvars.iv.next25.pre-phi, %if.end ], [ 99, %while.cond ]
  %cmp19.not = icmp eq i64 %indvars.iv24, 0
  br i1 %cmp19.not, label %for.inc35, label %while.body20

while.body20:                                     ; preds = %while.cond18
  %arrayidx22 = getelementptr inbounds [100 x i32], ptr %re, i64 0, i64 %indvars.iv24
...
while.body20.if.end_crit_edge:                    ; preds = %while.body20
  %.pre = add nsw i64 %indvars.iv24, -1
  br label %if.end
...
if.then24:                                        ; preds = %while.body20
  ...
  %add = add nsw i32 %4, %div18
  store i32 %add, ptr %arrayidx28, align 4, !tbaa !5
  %rem19 = urem i32 %2, 10
  store i32 %rem19, ptr %arrayidx22, align 4, !tbaa !5
  br label %if.end

if.end:                                           ; preds = %while.body20.if.end_crit_edge, %if.then24
  %indvars.iv.next25.pre-phi = phi i64 [ %.pre, %while.body20.if.end_crit_edge ], [ %3, %if.then24 ]
...
while.cond38:                                     ; preds = %for.cond10, %while.cond38
  %indvars.iv29 = phi i64 [ %indvars.iv.next30, %while.cond38 ], [ 0, %for.cond10 ]
  %arrayidx40 = getelementptr inbounds nuw [100 x i32], ptr %re, i64 0, i64 %indvars.iv29
...
for.cond45:                                       ; preds = %while.cond38, %for.inc51
  %indvars.iv32 = phi i64 [ %indvars.iv.next33, %for.inc51 ], [ %indvars.iv29, %while.cond38 ]
  %cmp46 = icmp samesign ult i64 %indvars.iv32, 100
  br i1 %cmp46, label %for.inc51, label %for.end53

for.inc51:                                        ; preds = %for.cond45
  %arrayidx49 = getelementptr inbounds nuw [100 x i32], ptr %re, i64 0, i64 %indvars.iv32
