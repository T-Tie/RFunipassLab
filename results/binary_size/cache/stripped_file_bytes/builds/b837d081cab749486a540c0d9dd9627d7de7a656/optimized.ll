...
tailrecurse:                                      ; preds = %if.end7, %entry
  %accumulator.tr = phi i32 [ 0, %entry ], [ %add, %if.end7 ]
  %m.tr = phi i32 [ %m, %entry ], [ %sub, %if.end7 ]
...
if.end7:                                          ; preds = %while.body, %while.end
  %m.addr.014 = phi i32 [ %m.tr, %while.end ], [ %m.addr.0, %while.body ]
  %div = sdiv i32 %n, %m.addr.014
  %call = tail call noundef i32 @_Z3getii(i32 noundef %div, i32 noundef %m.addr.014)
  %sub = add nsw i32 %m.addr.014, -1
  %add = add nsw i32 %call, %accumulator.tr
  br label %tailrecurse

return:                                           ; preds = %while.end, %tailrecurse
  %accumulator.ret.tr = phi i32 [ %accumulator.tr, %while.end ], [ 1, %tailrecurse ]
