...
entry:
  ...
  %sum15 = alloca i8, i32 0, align 4
  %sum2616 = alloca i8, i32 0, align 4
...
for.body:                                         ; preds = %for.cond
  ...
  %or.cond6 = or i1 %cmp14, %or.cond
  %5 = call i32 @llvm.smin.i32(i32 %2, i32 %1)
  %smin10 = sext i32 %5 to i64
  %6 = call i32 @llvm.smax.i32(i32 %1, i32 %2)
  br i1 %or.cond6, label %if.then, label %if.else25

if.then:                                          ; preds = %for.body
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef nonnull align 4 dereferenceable(4) %sum15) #8
  br label %for.cond15

for.cond15:                                       ; preds = %for.inc, %if.then
  %indvars.iv11 = phi i64 [ %indvars.iv.next12, %for.inc ], [ %smin10, %if.then ]
  %add9 = phi i32 [ %add, %for.inc ], [ 0, %if.then ]
  %lftr.wideiv13 = trunc i64 %indvars.iv11 to i32
  %exitcond14.not = icmp eq i32 %6, %lftr.wideiv13
  br i1 %exitcond14.not, label %for.inc48, label %for.inc

for.inc:                                          ; preds = %for.cond15
  %arrayidx = getelementptr inbounds [13 x i32], ptr @__const.main.b, i64 0, i64 %indvars.iv11
  %7 = load i32, ptr %arrayidx, align 4, !tbaa !5, !invariant.load !9
  %add = add nsw i32 %7, %add9
  %indvars.iv.next12 = add nsw i64 %indvars.iv11, 1
  br label %for.cond15, !llvm.loop !10

if.else25:                                        ; preds = %for.body
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef nonnull align 4 dereferenceable(4) %sum2616) #8
  br label %for.cond28

for.cond28:                                       ; preds = %for.inc35, %if.else25
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.inc35 ], [ %smin10, %if.else25 ]
  %add348 = phi i32 [ %add34, %for.inc35 ], [ 0, %if.else25 ]
...
for.inc35:                                        ; preds = %for.cond28
  ...
  %add34 = add nsw i32 %8, %add348
  %indvars.iv.next = add nsw i64 %indvars.iv, 1
  br label %for.cond28, !llvm.loop !13

for.inc48:                                        ; preds = %for.cond28, %for.cond15
  %9 = phi i32 [ %add9, %for.cond15 ], [ %add348, %for.cond28 ]
  %sum26.sink1 = phi ptr [ %sum15, %for.cond15 ], [ %sum2616, %for.cond28 ]
