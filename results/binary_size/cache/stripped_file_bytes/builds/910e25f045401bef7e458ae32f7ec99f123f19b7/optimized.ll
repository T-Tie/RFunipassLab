...
while.body:                                       ; preds = %while.cond
  ...
  %call2 = call noundef range(i32 0, 335) i32 @_Z3dayi(i32 noundef %1)
  %2 = load i32, ptr %m2, align 4, !tbaa !5
  %call3 = call noundef range(i32 0, 335) i32 @_Z3dayi(i32 noundef %2)
  ...
  %or.cond2 = or i1 %cmp7, %or.cond
  br i1 %or.cond2, label %if.then, label %if.end14
...
if.then:                                          ; preds = %while.body
  ...
  %spec.select3 = add nuw nsw i32 %call3, %inc12
  br label %if.end14

if.end14:                                         ; preds = %if.then, %while.body
  %d1.0 = phi i32 [ %call2, %while.body ], [ %spec.select, %if.then ]
  %d2.0 = phi i32 [ %call3, %while.body ], [ %spec.select3, %if.then ]
  %sub = sub nsw i32 %d1.0, %d2.0
  %rem15.lhs.trunc = trunc nsw i32 %sub to i16
  %rem151 = srem i16 %rem15.lhs.trunc, 7
  %cmp16 = icmp eq i16 %rem151, 0
