...
define dso_local noundef i32 @_Z6divideii(i32 noundef %n, i32 noundef %a) local_unnamed_addr #0 {
entry:
  %cmp = icmp eq i32 %n, 1
...
for.cond:                                         ; preds = %entry, %for.inc
  ...
  %a.sink = phi i32 [ %dec, %for.inc ], [ %a, %entry ]
  %cmp1 = icmp sgt i32 %a.sink, 1
  br i1 %cmp1, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %rem = srem i32 %n, %a.sink
  %cmp2 = icmp eq i32 %rem, 0
  br i1 %cmp2, label %if.then3, label %for.inc

if.then3:                                         ; preds = %for.body
  %div = sdiv i32 %n, %a.sink
  %call = tail call noundef i32 @_Z6divideii(i32 noundef %div, i32 noundef %a.sink) #6
  %add = add nsw i32 %call, %s.0
  %inc = add nsw i32 %t.0, 1
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then3
  %t.1 = phi i32 [ %inc, %if.then3 ], [ %t.0, %for.body ]
  %s.1 = phi i32 [ %add, %if.then3 ], [ %s.0, %for.body ]
  %dec = add nsw i32 %a.sink, -1
  br label %for.cond, !llvm.loop !5

for.end:                                          ; preds = %for.cond
  %cmp5 = icmp eq i32 %t.0, 0
  %spec.select = select i1 %cmp5, i32 0, i32 %s.0
  br label %cleanup

cleanup:                                          ; preds = %for.end, %entry
  %.sink = phi i32 [ 1, %entry ], [ %spec.select, %for.end ]
  ret i32 %.sink
}
...
entry:
  ...
  %.pr = load i32, ptr %k, align 4, !tbaa !8
  br label %while.cond
...
while.body:                                       ; preds = %while.cond
  ...
  %call2 = call noundef i32 @_Z6divideii(i32 noundef %1, i32 noundef %1) #8
