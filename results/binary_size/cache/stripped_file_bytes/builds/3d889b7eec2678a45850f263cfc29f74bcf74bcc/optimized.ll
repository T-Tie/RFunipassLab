...
if.end:                                           ; preds = %if.then, %if.else
  %call10.sink = phi ptr [ %call10, %if.else ], [ @_ZSt4cout, %if.then ]
  %.sink = phi i32 [ 1, %if.else ], [ %conv, %if.then ]
  %add13.sink = phi i32 [ %add, %if.else ], [ %sum.0, %if.then ]
  %call12 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(8) %call10.sink, ptr noundef nonnull %words)
  %add14 = add nsw i32 %add13.sink, %.sink
...
if.end22:                                         ; preds = %if.else16, %if.end
  %sum.2 = phi i32 [ %add14, %if.end ], [ %add21, %if.else16 ]
  %temp.2 = phi i32 [ %inc15, %if.end ], [ 0, %if.else16 ]
