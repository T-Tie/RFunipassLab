...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 201, ptr noundef nonnull align 16 %a) #6
...
for.cond:                                         ; preds = %for.inc, %entry
  %p.0 = phi ptr [ %a, %entry ], [ %incdec.ptr10, %for.inc ]
  %i.0 = phi i32 [ 0, %entry ], [ %inc, %for.inc ]
  %j.0 = phi i32 [ undef, %entry ], [ %.sink, %for.inc ]
...
for.body:                                         ; preds = %for.cond
  ...
  br i1 %cmp5.not, label %if.else, label %for.inc.sink.split

if.else:                                          ; preds = %for.body
  %cmp7 = icmp eq i32 %j.0, 0
  br i1 %cmp7, label %for.inc.sink.split, label %for.inc

for.inc.sink.split:                               ; preds = %if.else, %for.body
  %.sink3 = phi i8 [ %0, %for.body ], [ 32, %if.else ]
  %.sink.ph = phi i32 [ 0, %for.body ], [ 1, %if.else ]
  %call6 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_c(ptr noundef nonnull align 8 dereferenceable(8) @_ZSt4cout, i8 noundef signext %.sink3)
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %if.else
  %.sink = phi i32 [ 1, %if.else ], [ %.sink.ph, %for.inc.sink.split ]
  %incdec.ptr10 = getelementptr inbounds nuw i8, ptr %p.0, i64 1
...
for.end:                                          ; preds = %for.cond
  ...
  call void @llvm.lifetime.end.p0(i64 noundef 201, ptr noundef nonnull %a) #8
  ret i32 0
}
