...
define dso_local noundef i32 @_Z6factorii(i32 noundef %num, i32 noundef %minA1) local_unnamed_addr #0 {
entry:
  %cmp.not = icmp sgt i32 %num, %minA1
  br i1 %cmp.not, label %if.end, label %cleanup

if.end:                                           ; preds = %entry
  %conv = sitofp i32 %num to double
  %call = tail call double @sqrt(double noundef %conv) #6, !tbaa !5
  %conv1 = fptosi double %call to i32
  %cmp2.not11 = icmp sgt i32 %minA1, %conv1
  br i1 %cmp2.not11, label %cleanup, label %for.body

for.body:                                         ; preds = %if.end, %for.inc
  %i.013 = phi i32 [ %inc7, %for.inc ], [ %minA1, %if.end ]
  %count.012 = phi i32 [ %count.1, %for.inc ], [ 0, %if.end ]
  %rem = srem i32 %num, %i.013
  %div = sdiv i32 %num, %i.013
  %cmp3 = icmp eq i32 %rem, 0
  br i1 %cmp3, label %if.then4, label %for.inc

if.then4:                                         ; preds = %for.body
  %inc = add nsw i32 %count.012, 1
  %call5 = tail call noundef i32 @_Z6factorii(i32 noundef %div, i32 noundef %i.013)
  %add = add nsw i32 %inc, %call5
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then4
  %count.1 = phi i32 [ %add, %if.then4 ], [ %count.012, %for.body ]
  %inc7 = add i32 %i.013, 1
  %exitcond.not = icmp eq i32 %i.013, %conv1
  br i1 %exitcond.not, label %cleanup, label %for.body, !llvm.loop !9

cleanup:                                          ; preds = %for.inc, %if.end, %entry
  %retval.0 = phi i32 [ 0, %entry ], [ 0, %if.end ], [ %count.1, %for.inc ]
  ret i32 %retval.0
}
...
entry:
  ...
  store i32 %dec1, ptr %n, align 4, !tbaa !5
...
attributes #0 = { mustprogress nofree nounwind memory(errnomem: write) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
