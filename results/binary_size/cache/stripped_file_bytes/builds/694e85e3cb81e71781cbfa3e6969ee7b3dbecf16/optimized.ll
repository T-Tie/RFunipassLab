...
while.cond:                                       ; preds = %while.cond.backedge, %entry
  %k.0 = phi i32 [ undef, %entry ], [ %k.0.be, %while.cond.backedge ]
  %tobool.not = phi i1 [ false, %entry ], [ %tobool.not.be, %while.cond.backedge ]
...
for.cond:                                         ; preds = %while.cond, %for.inc
  ...
  %k.1 = phi i32 [ %1, %for.inc ], [ %k.0, %while.cond ]
...
for.end:                                          ; preds = %for.body, %for.cond
  ...
  %smax = call i32 @llvm.smax.i32(i32 %k.2, i32 0)
  %wide.trip.count29 = zext nneg i32 %smax to i64
  %wide.trip.count = zext i32 %k.2 to i64
  br label %for.cond11

for.cond11:                                       ; preds = %for.end, %for.inc28
  %indvars.iv26 = phi i64 [ 0, %for.end ], [ %indvars.iv.next27, %for.inc28 ]
  %s.0 = phi i32 [ 0, %for.end ], [ %s.1, %for.inc28 ]
  %exitcond30.not = icmp eq i64 %indvars.iv26, %wide.trip.count29
  br i1 %exitcond30.not, label %for.end30, label %for.cond14.preheader

for.cond14.preheader:                             ; preds = %for.cond11
  %arrayidx18 = getelementptr inbounds nuw [15 x i32], ptr %a, i64 0, i64 %indvars.iv26
  br label %for.cond14

for.cond14:                                       ; preds = %for.cond14.preheader, %for.body16
  %indvars.iv22 = phi i64 [ 0, %for.cond14.preheader ], [ %indvars.iv.next23, %for.body16 ]
  %s.1 = phi i32 [ %s.0, %for.cond14.preheader ], [ %spec.select, %for.body16 ]
  %exitcond25.not = icmp eq i64 %indvars.iv22, %wide.trip.count
  br i1 %exitcond25.not, label %for.inc28, label %for.body16

for.body16:                                       ; preds = %for.cond14
  %2 = load i32, ptr %arrayidx18, align 4, !tbaa !5
  %arrayidx20 = getelementptr inbounds nuw [15 x i32], ptr %a, i64 0, i64 %indvars.iv22
  %3 = load i32, ptr %arrayidx20, align 4, !tbaa !5
  %mul = shl nsw i32 %3, 1
  %cmp21 = icmp eq i32 %2, %mul
  %inc23 = zext i1 %cmp21 to i32
  %spec.select = add nsw i32 %s.1, %inc23
...
while.cond.backedge:                              ; preds = %for.body, %for.end30
  %k.0.be = phi i32 [ %k.2, %for.end30 ], [ %1, %for.body ]
  %tobool.not.be = phi i1 [ false, %for.end30 ], [ true, %for.body ]
