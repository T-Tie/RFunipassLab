...
for.body:                                         ; preds = %for.cond
  ...
  %call3 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) %p.0) #6
...
for.cond4:                                        ; preds = %for.cond, %for.inc31
  %1 = phi i32 [ %.pre, %for.inc31 ], [ %0, %for.cond ]
  %i.1 = phi i32 [ %inc32, %for.inc31 ], [ 1, %for.cond ]
  %line.0 = phi i32 [ %line.1, %for.inc31 ], [ undef, %for.cond ]
  %p.1 = phi ptr [ %p.2, %for.inc31 ], [ @word, %for.cond ]
...
if.then:                                          ; preds = %for.body6
  ...
  %2 = load i32, ptr %a11, align 4, !tbaa !9
  br label %for.inc31

if.else:                                          ; preds = %for.body6
  %a13 = getelementptr inbounds nuw i8, ptr %p.1, i64 20
  %3 = load i32, ptr %a13, align 4, !tbaa !9
...
if.else22:                                        ; preds = %if.else
  ...
  %add28 = add i32 %add27, %5
  br label %for.inc31

for.inc31:                                        ; preds = %if.then, %if.else22, %if.then16
  %line.1 = phi i32 [ %2, %if.then ], [ %4, %if.then16 ], [ %add28, %if.else22 ]
