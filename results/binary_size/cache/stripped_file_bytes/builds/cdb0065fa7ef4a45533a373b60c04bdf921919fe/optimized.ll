...
while.body:                                       ; preds = %while.cond
  ...
  br i1 %or.cond7, label %if.then, label %if.end14
...
if.then:                                          ; preds = %while.body
  ...
  %spec.select8 = add nuw nsw i32 %call3, %inc12
  br label %if.end14

if.end14:                                         ; preds = %if.then, %while.body
  %d1.0 = phi i32 [ %call2, %while.body ], [ %spec.select, %if.then ]
  %d2.0 = phi i32 [ %call3, %while.body ], [ %spec.select8, %if.then ]
  %sub = sub nsw i32 %d1.0, %d2.0
  %rem15.lhs.trunc = trunc nsw i32 %sub to i16
  %rem156 = srem i16 %rem15.lhs.trunc, 7
  %cmp16 = icmp eq i16 %rem156, 0
