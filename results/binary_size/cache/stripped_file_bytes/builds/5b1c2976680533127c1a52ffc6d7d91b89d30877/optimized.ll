...
tailrecurse.outer:                                ; preds = %if.then2, %entry
  %accumulator.tr.ph = phi i32 [ %a.b.i, %if.then2 ], [ -2147483648, %entry ]
  %s.tr.ph = phi i32 [ %add, %if.then2 ], [ %s, %entry ]
  %1 = sext i32 %s.tr.ph to i64
  br label %tailrecurse

tailrecurse:                                      ; preds = %tailrecurse.outer, %if.else
  %indvars.iv = phi i64 [ %1, %tailrecurse.outer ], [ %indvars.iv.next, %if.else ]
...
if.then2:                                         ; preds = %if.end
  ...
  %add = add nsw i32 %4, 1
  %call = tail call noundef i32 @_Z6daodanii(i32 noundef %add, i32 noundef %3)
  %add5 = add nuw nsw i32 %call, 1
  %a.b.i = tail call noundef i32 @llvm.smax.i32(i32 %add5, i32 %accumulator.tr.ph)
  br label %tailrecurse.outer
...
return:                                           ; preds = %tailrecurse
  %accumulator.ret.tr = tail call noundef i32 @llvm.smax.i32(i32 %accumulator.tr.ph, i32 0)
