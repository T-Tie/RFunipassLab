...
for.cond:                                         ; preds = %if.end31, %entry
  %result.0 = phi i32 [ 0, %entry ], [ %result.1, %if.end31 ]
  %i.0 = phi i32 [ 1, %entry ], [ %inc, %if.end31 ]
  %exitcond.not = icmp eq i32 %i.0, %smax
  br i1 %exitcond.not, label %for.end, label %for.body

for.body:                                         ; preds = %for.cond
  switch i32 %i.0, label %if.end31 [
    i32 1, label %if.then
    i32 3, label %if.then
    i32 5, label %if.then
    i32 7, label %if.then
...
if.end31:                                         ; preds = %for.body, %if.then20, %if.then25, %if.else27, %if.then
