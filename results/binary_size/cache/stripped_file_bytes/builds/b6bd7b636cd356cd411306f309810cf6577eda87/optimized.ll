...
for.cond:                                         ; preds = %for.inc, %entry
  %i.0 = phi i32 [ 0, %entry ], [ %inc13, %for.inc ]
  %flag.0 = phi i32 [ 1, %entry ], [ %flag.110, %for.inc ]
  %a.0 = phi i32 [ 0, %entry ], [ %a.116, %for.inc ]
  %b.0 = phi i32 [ 0, %entry ], [ %b.1, %for.inc ]
  %0 = load i32, ptr %n, align 4, !tbaa !5
  %mul = mul nsw i32 %0, %0
  %cmp = icmp samesign ult i32 %i.0, %mul
...
for.body:                                         ; preds = %for.cond
  ...
  br i1 %cmp2, label %land.lhs.true, label %if.end.thread

land.lhs.true:                                    ; preds = %for.body
  %cmp3 = icmp eq i32 %flag.0, 1
  %inc = zext i1 %cmp3 to i32
  %spec.select = add nsw i32 %a.0, %inc
  %inc11 = add nsw i32 %b.0, 1
  br label %for.inc

if.end.thread:                                    ; preds = %for.body
  %cmp415 = icmp sgt i32 %a.0, 0
  %spec.select26 = select i1 %cmp415, i32 0, i32 %flag.0
  br label %for.inc

for.inc:                                          ; preds = %if.end.thread, %land.lhs.true
  %a.116 = phi i32 [ %spec.select, %land.lhs.true ], [ %a.0, %if.end.thread ]
  %flag.110 = phi i32 [ %flag.0, %land.lhs.true ], [ %spec.select26, %if.end.thread ]
  %b.1 = phi i32 [ %inc11, %land.lhs.true ], [ %b.0, %if.end.thread ]
  %inc13 = add nuw nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !9

for.end:                                          ; preds = %for.cond
  %mul14 = shl nsw i32 %a.0, 1
  %sub = sub nsw i32 %b.0, %mul14
  %div = sdiv i32 %sub, 2
  %add = add nsw i32 %div, 2
  %mul15 = mul nsw i32 %add, %a.0
  %sub16 = sub nsw i32 %mul15, %b.0
