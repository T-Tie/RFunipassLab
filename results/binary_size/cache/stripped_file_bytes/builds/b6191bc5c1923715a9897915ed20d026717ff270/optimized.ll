...
define dso_local noundef i32 @_Z1fii(i32 noundef %a, i32 noundef %j) local_unnamed_addr #3 {
entry:
  %conv.i = sitofp i32 %a to double
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %i.0 = phi i32 [ %j, %entry ], [ %inc, %for.inc ]
...
if.else:                                          ; preds = %if.then
  %call5 = tail call noundef i32 @_Z1fii(i32 noundef %div, i32 noundef %i.0)
  %add6 = add i32 %count.0, 1
  %add7 = add i32 %add6, %call5
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.else, %if.then3
  %count.1 = phi i32 [ %add, %if.then3 ], [ %add7, %if.else ], [ %count.0, %for.body ]
