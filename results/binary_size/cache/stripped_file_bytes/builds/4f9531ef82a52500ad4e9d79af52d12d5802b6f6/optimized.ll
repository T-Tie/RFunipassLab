...
for.cond:                                         ; preds = %for.end, %entry
  ...
  br i1 %cmp.not, label %for.end13, label %for.body
...
for.cond2:                                        ; preds = %for.inc, %for.body
  %storemerge = phi i32 [ 2, %for.body ], [ %inc, %for.inc ]
...
for.inc:                                          ; preds = %for.body6, %if.then
  %4 = phi i32 [ %3, %for.body6 ], [ %.pre, %if.then ]
  %inc = add nsw i32 %4, 1
...
for.end:                                          ; preds = %for.cond2
  ...
  %call1.i = call noundef nonnull align 8 dereferenceable(8) ptr @_ZNSo3putEc(ptr noundef nonnull align 8 dereferenceable(8) %call9, i8 noundef signext %call.i2)
...
for.cond:                                         ; preds = %for.inc, %entry
  %k.0 = phi i32 [ %b, %entry ], [ %inc, %for.inc ]
...
if.end:                                           ; preds = %if.then
  %call = tail call noundef i32 @_Z1tii(i32 noundef %div, i32 noundef %k.0)
  br label %for.inc
...
for.end:                                          ; preds = %for.cond, %if.then3
  ret i32 0
}
