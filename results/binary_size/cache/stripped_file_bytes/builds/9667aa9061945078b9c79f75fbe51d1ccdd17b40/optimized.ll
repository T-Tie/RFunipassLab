...
for.cond1:                                        ; preds = %for.cond, %for.inc
  %1 = phi i32 [ %.pre7, %for.inc ], [ %0, %for.cond ]
...
if.then:                                          ; preds = %for.body3
  ...
  %i.0.leftx.1 = select i1 %cmp6, i32 %i.0, i32 %leftx.1
  %j.0.lefty.1 = select i1 %cmp6, i32 %j.0, i32 %lefty.1
  %rightx.1.i.0 = select i1 %cmp6, i32 %rightx.1, i32 %i.0
  %righty.1.j.0 = select i1 %cmp6, i32 %righty.1, i32 %j.0
  br label %for.inc

for.inc:                                          ; preds = %if.then, %for.body3
  %state.2 = phi i32 [ %state.1, %for.body3 ], [ 1, %if.then ]
  %leftx.2 = phi i32 [ %leftx.1, %for.body3 ], [ %i.0.leftx.1, %if.then ]
  %lefty.2 = phi i32 [ %lefty.1, %for.body3 ], [ %j.0.lefty.1, %if.then ]
  %rightx.2 = phi i32 [ %rightx.1, %for.body3 ], [ %rightx.1.i.0, %if.then ]
  %righty.2 = phi i32 [ %righty.1, %for.body3 ], [ %righty.1.j.0, %if.then ]
