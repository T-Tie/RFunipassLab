...
for.cond:                                         ; preds = %for.end26, %entry
  %indvars.iv48 = phi i32 [ %indvars.iv.next49, %for.end26 ], [ 0, %entry ]
  %indvars.iv35 = phi i64 [ %indvars.iv.next36, %for.end26 ], [ 0, %entry ]
  %max.0 = phi i32 [ %spec.select31, %for.end26 ], [ 0, %entry ]
  %call2 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) @a) #6
  %sub = sub i64 %call2, %conv3
  %cmp.not = icmp ult i64 %sub, %indvars.iv35
  br i1 %cmp.not, label %for.end34, label %for.cond4

for.cond4:                                        ; preds = %for.cond, %for.inc24
  %indvars.iv37 = phi i64 [ %indvars.iv.next38, %for.inc24 ], [ %indvars.iv35, %for.cond ]
  %count.1 = phi i32 [ %count.2, %for.inc24 ], [ 0, %for.cond ]
  %cmp9.not = icmp ult i64 %sub, %indvars.iv37
...
for.inc24:                                        ; preds = %for.body13, %for.end
  %count.2 = phi i32 [ %inc22, %for.end ], [ %count.1, %for.body13 ]
  %indvars.iv.next38 = add nuw nsw i64 %indvars.iv37, 1
  br label %for.cond4, !llvm.loop !14

for.end26:                                        ; preds = %for.cond4
  %arrayidx28 = getelementptr inbounds nuw [500 x i32], ptr @post, i64 0, i64 %indvars.iv35
  store i32 %count.1, ptr %arrayidx28, align 4, !tbaa !5
  %spec.select31 = call i32 @llvm.smax.i32(i32 %count.1, i32 %max.0)
...
for.cond39:                                       ; preds = %for.inc63, %if.else
  %indvars.iv41 = phi i64 [ %indvars.iv.next42, %for.inc63 ], [ 0, %if.else ]
  %exitcond51.not = icmp eq i64 %indvars.iv41, %wide.trip.count50
  br i1 %exitcond51.not, label %if.end66, label %for.body41

for.body41:                                       ; preds = %for.cond39
  %arrayidx43 = getelementptr inbounds nuw [500 x i32], ptr @post, i64 0, i64 %indvars.iv41
