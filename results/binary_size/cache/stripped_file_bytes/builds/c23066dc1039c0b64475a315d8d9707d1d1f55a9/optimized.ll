...
for.body:                                         ; preds = %for.cond
  ...
  %call4 = call noundef i32 @_Z2fjii(i32 noundef %2, i32 noundef 2) #7
...
for.cond:                                         ; preds = %entry, %if.end4
  %counter.0 = phi i32 [ %counter.1, %if.end4 ], [ 1, %entry ]
  %i.0 = phi i32 [ %inc, %if.end4 ], [ %min, %entry ]
  %exitcond.not = icmp eq i32 %i.0, %m
...
if.then3:                                         ; preds = %for.body
  %div = sdiv i32 %m, %i.0
  %call = tail call noundef i32 @_Z2fjii(i32 noundef %div, i32 noundef %i.0) #8
  %add = add nsw i32 %call, %counter.0
  br label %if.end4

if.end4:                                          ; preds = %if.then3, %for.body
  %counter.1 = phi i32 [ %add, %if.then3 ], [ %counter.0, %for.body ]
  %inc = add i32 %i.0, 1
  br label %for.cond, !llvm.loop !13

cleanup:                                          ; preds = %for.cond, %entry
  %retval.0 = phi i32 [ 0, %entry ], [ %counter.0, %for.cond ]
  ret i32 %retval.0
}
