...
for.cond:                                         ; preds = %if.end35, %entry
  %.pr6270 = phi i32 [ %n.promoted, %entry ], [ %.pr6271, %if.end35 ]
  %a5.0 = phi i32 [ 0, %entry ], [ %a5.153, %if.end35 ]
  %a10.0 = phi i32 [ 0, %entry ], [ %a10.13745, %if.end35 ]
  %a20.0 = phi i32 [ 0, %entry ], [ %a20.1203451, %if.end35 ]
  %a50.0 = phi i32 [ 0, %entry ], [ %a50.110163647, %if.end35 ]
  %a100.0 = phi i32 [ 0, %entry ], [ %a100.149183549, %if.end35 ]
  %a1.0 = phi i32 [ 0, %entry ], [ %a1.1, %if.end35 ]
  %cmp.not = icmp eq i32 %.pr6270, 0
  br i1 %cmp.not, label %for.end, label %for.body

for.body:                                         ; preds = %for.cond
  %cmp1 = icmp sgt i32 %.pr6270, 99
  br i1 %cmp1, label %if.end, label %land.lhs.true

if.end:                                           ; preds = %for.body
  %add = add nsw i32 %a100.0, 1
  %sub = add nsw i32 %.pr6270, -100
...
land.lhs.true16:                                  ; preds = %if.end14, %land.lhs.true9
  %.pr6266 = phi i32 [ %.pr6268, %land.lhs.true9 ], [ %sub13, %if.end14 ]
  %a20.121 = phi i32 [ %a20.0, %land.lhs.true9 ], [ %add12, %if.end14 ]
  %cmp17 = icmp sgt i32 %.pr6266, 9
  %sub20 = add nsw i32 %.pr6266, -10
  %.pr6264.ph = select i1 %cmp17, i32 %sub20, i32 %.pr6266
  %add19 = zext i1 %cmp17 to i32
  %a10.1.ph = add nsw i32 %a10.0, %add19
  %cmp24 = icmp sgt i32 %.pr6264.ph, 4
  %sub27 = add nsw i32 %.pr6264.ph, -5
  %.pr6263 = select i1 %cmp24, i32 %sub27, i32 %.pr6264.ph
  %add26 = zext i1 %cmp24 to i32
  %a5.154 = add nsw i32 %a5.0, %add26
  %0 = or i1 %cmp17, %cmp24
  %cmp31 = icmp sgt i32 %.pr6263, 0
  %sub34 = sext i1 %cmp31 to i32
  %.pr6271.ph = add nsw i32 %.pr6263, %sub34
...
2:                                                ; preds = %land.lhs.true16
  store i32 %.pr6271.ph, ptr %n, align 4, !tbaa !5
  br label %if.end35

if.end35:                                         ; preds = %2, %land.lhs.true16, %if.end14, %if.end
  %.pr6271 = phi i32 [ %sub, %if.end ], [ %sub13, %if.end14 ], [ %.pr6271.ph, %land.lhs.true16 ], [ %.pr6271.ph, %2 ]
  %a5.153 = phi i32 [ %a5.0, %if.end ], [ %a5.0, %if.end14 ], [ %a5.154, %land.lhs.true16 ], [ %a5.154, %2 ]
  %a20.1203451 = phi i32 [ %a20.0, %if.end ], [ %add12, %if.end14 ], [ %a20.121, %land.lhs.true16 ], [ %a20.121, %2 ]
  %a100.149183549 = phi i32 [ %add, %if.end ], [ %a100.15, %if.end14 ], [ %a100.15, %land.lhs.true16 ], [ %a100.15, %2 ]
  %a50.110163647 = phi i32 [ %a50.0, %if.end ], [ %a50.1, %if.end14 ], [ %a50.1, %land.lhs.true16 ], [ %a50.1, %2 ]
  %a10.13745 = phi i32 [ %a10.0, %if.end ], [ %a10.0, %if.end14 ], [ %a10.1.ph, %land.lhs.true16 ], [ %a10.1.ph, %2 ]
