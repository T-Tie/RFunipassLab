...
for.body:                                         ; preds = %for.cond
  ...
  %call3 = call noundef i32 @_Z3runi(i32 noundef %3)
  %cmp4 = icmp eq i32 %call3, 0
  br i1 %cmp4, label %for.cond6, label %for.cond28

for.cond6:                                        ; preds = %for.body, %for.inc
  %j.0 = phi i32 [ %inc, %for.inc ], [ %.31, %for.body ]
  %day.2 = phi i32 [ %add, %for.inc ], [ 0, %for.body ]
  %exitcond.not = icmp eq i32 %j.0, %.
  br i1 %exitcond.not, label %if.end51, label %for.body8

for.body8:                                        ; preds = %for.cond6
  %switch.tableidx = add i32 %j.0, -2
  %4 = icmp ult i32 %switch.tableidx, 10
  br i1 %4, label %switch.lookup, label %for.inc

switch.lookup:                                    ; preds = %for.body8
  %5 = zext nneg i32 %switch.tableidx to i64
  %switch.gep = getelementptr inbounds nuw [10 x i32], ptr @switch.table.main, i64 0, i64 %5
  %switch.load = load i32, ptr %switch.gep, align 4
  br label %for.inc

for.inc:                                          ; preds = %for.body8, %switch.lookup
  %.sink = phi i32 [ %switch.load, %switch.lookup ], [ 31, %for.body8 ]
  %add = add nuw nsw i32 %day.2, %.sink
  %inc = add i32 %j.0, 1
  br label %for.cond6, !llvm.loop !9

for.cond28:                                       ; preds = %for.body, %for.inc48
  %j.1 = phi i32 [ %inc49, %for.inc48 ], [ %.31, %for.body ]
  %day.5 = phi i32 [ %add33, %for.inc48 ], [ 0, %for.body ]
  %exitcond32.not = icmp eq i32 %j.1, %.
  br i1 %exitcond32.not, label %if.end51, label %for.body30

for.body30:                                       ; preds = %for.cond28
  %switch.tableidx37 = add i32 %j.1, -2
  %6 = icmp ult i32 %switch.tableidx37, 10
  br i1 %6, label %switch.lookup36, label %for.inc48

switch.lookup36:                                  ; preds = %for.body30
  %7 = zext nneg i32 %switch.tableidx37 to i64
  %switch.gep38 = getelementptr inbounds nuw [10 x i32], ptr @switch.table.main.2, i64 0, i64 %7
  %switch.load39 = load i32, ptr %switch.gep38, align 4
  br label %for.inc48

for.inc48:                                        ; preds = %for.body30, %switch.lookup36
  %.sink35 = phi i32 [ %switch.load39, %switch.lookup36 ], [ 31, %for.body30 ]
  %add33 = add nuw nsw i32 %day.5, %.sink35
  %inc49 = add i32 %j.1, 1
  br label %for.cond28, !llvm.loop !12

if.end51:                                         ; preds = %for.cond28, %for.cond6
  %day.4 = phi i32 [ %day.2, %for.cond6 ], [ %day.5, %for.cond28 ]
