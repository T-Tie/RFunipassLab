...
while.cond:                                       ; preds = %if.end26, %entry
  %f.0 = phi i32 [ 0, %entry ], [ %f.2, %if.end26 ]
...
if.end:                                           ; preds = %if.then8, %if.then
  %f.1 = phi i32 [ %add, %if.then8 ], [ %f.0, %if.then ]
  ...
  %call14 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) %w) #12
  %2 = trunc i64 %call14 to i32
  %conv17 = add i32 %f.1, %2
  br label %if.end26
...
if.else:                                          ; preds = %while.body
  ...
  %3 = trunc i64 %call23 to i32
  %conv25 = add i32 %3, 1
  br label %if.end26

if.end26:                                         ; preds = %if.else, %if.end
  %f.2 = phi i32 [ %conv17, %if.end ], [ %conv25, %if.else ]
  %f1.1 = phi i32 [ %inc12, %if.end ], [ 0, %if.else ]
