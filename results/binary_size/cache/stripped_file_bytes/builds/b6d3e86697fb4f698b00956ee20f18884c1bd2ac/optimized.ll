...
entry:
  ...
  %add = add nsw i32 %n, 1
  %smax = tail call i32 @llvm.smax.i32(i32 noundef %n, i32 noundef 0) #7
  %0 = add nuw i32 %smax, 1
  %wide.trip.count = zext i32 %0 to i64
  br label %for.cond

for.cond:                                         ; preds = %if.end15, %entry
  %y.addr.0 = phi i32 [ %y, %entry ], [ %inc17, %if.end15 ]
  %cmp.not = icmp sgt i32 %y.addr.0, %div
  br i1 %cmp.not, label %for.end18, label %for.body

for.body:                                         ; preds = %for.cond
  %rem = srem i32 %r, %y.addr.0
  %cmp1 = icmp eq i32 %rem, 0
  br i1 %cmp1, label %if.then, label %if.end15

if.then:                                          ; preds = %for.body
  store i32 %y.addr.0, ptr %arrayidx, align 4, !tbaa !5
  br label %for.cond2
...
cleanup.cont:                                     ; preds = %if.end
  %div12 = sdiv i32 %r, %y.addr.0
  %call = tail call noundef i32 @_Z3wrhiiPiii(i32 noundef %add, i32 noundef %div12, ptr nofree noundef nonnull captures(none) %b, i32 noundef %y.addr.0, i32 noundef %a) #8
  br label %if.end15

if.end15:                                         ; preds = %cleanup.cont, %for.body
  %inc17 = add nsw i32 %y.addr.0, 1
