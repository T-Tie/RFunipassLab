...
for.cond:                                         ; preds = %entry, %for.inc
  %j.0 = phi i32 [ %inc, %for.inc ], [ %k, %entry ]
  %m.0 = phi i32 [ %m.1, %for.inc ], [ 0, %entry ]
  %cmp1.not = icmp sgt i32 %j.0, %num
...
if.then3:                                         ; preds = %for.body
  %call = tail call noundef i32 @_Z8functionii(i32 noundef %div, i32 noundef %j.0)
  %add = add nsw i32 %call, %m.0
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then3
  %m.1 = phi i32 [ %add, %if.then3 ], [ %m.0, %for.body ]
  %inc = add nsw i32 %j.0, 1
  br label %for.cond, !llvm.loop !5

cleanup:                                          ; preds = %for.cond, %entry
  %retval.0 = phi i32 [ 1, %entry ], [ %m.0, %for.cond ]
  ret i32 %retval.0
}
...
for.cond2:                                        ; preds = %for.cond, %for.body4
  ...
  %indvars.iv8 = phi i64 [ %indvars.iv.next9, %for.body4 ], [ 0, %for.cond ]
  %3 = sext i32 %2 to i64
  %cmp3 = icmp slt i64 %indvars.iv8, %3
  br i1 %cmp3, label %for.body4, label %for.end12

for.body4:                                        ; preds = %for.cond2
  %arrayidx6 = getelementptr inbounds nuw [100 x i32], ptr %num, i64 0, i64 %indvars.iv8
  ...
  %call1.i = call noundef nonnull align 8 dereferenceable(8) ptr @_ZNSo3putEc(ptr noundef nonnull align 8 dereferenceable(8) %call8, i8 noundef signext %call.i6)
