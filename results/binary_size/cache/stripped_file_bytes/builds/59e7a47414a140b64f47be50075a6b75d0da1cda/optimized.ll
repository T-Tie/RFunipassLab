...
for.cond2.preheader.loopexit:                     ; preds = %for.cond
  %.pre7 = load i32, ptr %str, align 16
  %2 = icmp sgt i32 %.pre7, 0
  %3 = zext i1 %2 to i32
...
if.else:                                          ; preds = %entry
  br i1 %cmp1.not, label %cleanup, label %if.else4
...
cleanup:                                          ; preds = %if.else4, %land.lhs.true, %if.else
  %.sink = phi i32 [ 1, %if.else ], [ 1, %if.else4 ], [ %spec.select, %land.lhs.true ]
  ret i32 %.sink
}
