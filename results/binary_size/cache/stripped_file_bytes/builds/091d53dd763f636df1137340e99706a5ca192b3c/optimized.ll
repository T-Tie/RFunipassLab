...
while.cond.preheader:                             ; preds = %entry, %if.end7
  %m.tr16 = phi i32 [ %sub, %if.end7 ], [ %m, %entry ]
  %accumulator.tr15 = phi i32 [ %add, %if.end7 ], [ 0, %entry ]
  %cmp113 = icmp sgt i32 %m.tr16, 1
  br i1 %cmp113, label %while.body, label %while.end

while.body:                                       ; preds = %while.cond.preheader, %if.end4
  %m.addr.014 = phi i32 [ %dec, %if.end4 ], [ %m.tr16, %while.cond.preheader ]
  %rem = srem i32 %n, %m.addr.014
  %cmp2 = icmp eq i32 %rem, 0
  br i1 %cmp2, label %if.end7, label %if.end4

if.end4:                                          ; preds = %while.body
  %dec = add nsw i32 %m.addr.014, -1
  %cmp1 = icmp sgt i32 %m.addr.014, 2
  br i1 %cmp1, label %while.body, label %return, !llvm.loop !5

while.end:                                        ; preds = %while.cond.preheader
  %cmp5 = icmp eq i32 %m.tr16, 1
  br i1 %cmp5, label %return, label %if.end7

if.end7:                                          ; preds = %while.body, %while.end
  %m.addr.012 = phi i32 [ %m.tr16, %while.end ], [ %m.addr.014, %while.body ]
  %div = sdiv i32 %n, %m.addr.012
  %call = tail call noundef i32 @_Z3getii(i32 noundef %div, i32 noundef %m.addr.012)
  %sub = add nsw i32 %m.addr.012, -1
  %add = add nsw i32 %call, %accumulator.tr15
  br label %while.cond.preheader

return:                                           ; preds = %while.end, %if.end4, %entry
  %accumulator.ret.tr = phi i32 [ 1, %entry ], [ %accumulator.tr15, %if.end4 ], [ %accumulator.tr15, %while.end ]
...
for.body:                                         ; preds = %entry, %_ZSt4endlIcSt11char_traitsIcEERSt13basic_ostreamIT_T0_ES6_.exit
  ...
  %call2 = call noundef i32 @_Z3getii(i32 noundef %1, i32 noundef %1)
...
_ZSt4endlIcSt11char_traitsIcEERSt13basic_ostreamIT_T0_ES6_.exit: ; preds = %if.then.i2.i.i, %if.end.i.i.i
  ...
  %6 = load i32, ptr %t, align 4, !tbaa !8
