...
define dso_local void @_Z6divideii(i32 noundef %n, i32 noundef %n2) local_unnamed_addr #0 {
entry:
  %conv = sitofp i32 %n to double
  %call = tail call double @sqrt(double noundef %conv) #6, !tbaa !5
  %conv1 = fptosi double %call to i32
  %cmp.not7 = icmp sgt i32 %n2, %conv1
  br i1 %cmp.not7, label %for.end, label %for.body

for.body:                                         ; preds = %entry, %for.inc
  %i.08 = phi i32 [ %inc3, %for.inc ], [ %n2, %entry ]
  %rem = srem i32 %n, %i.08
  %div = sdiv i32 %n, %i.08
...
if.then:                                          ; preds = %for.body
  ...
  tail call void @_Z6divideii(i32 noundef %div, i32 noundef %i.08)
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then
  %inc3 = add i32 %i.08, 1
  %exitcond.not = icmp eq i32 %i.08, %conv1
...
for.body:                                         ; preds = %entry, %_ZSt4endlIcSt11char_traitsIcEERSt13basic_ostreamIT_T0_ES6_.exit
  ...
  call void @_Z6divideii(i32 noundef %1, i32 noundef 2)
...
attributes #0 = { mustprogress nofree nounwind memory(readwrite, argmem: none, inaccessiblemem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
