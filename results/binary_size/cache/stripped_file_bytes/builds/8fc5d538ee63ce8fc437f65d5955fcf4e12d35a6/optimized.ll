...
entry:
  ...
  %cmp1417 = icmp eq i32 %pos, %0
  br i1 %cmp1417, label %return, label %if.end.lr.ph.lr.ph

if.end.lr.ph.lr.ph:                               ; preds = %entry
  %add5 = add nsw i32 %count, 1
  br label %if.end.lr.ph

if.end.lr.ph:                                     ; preds = %if.end.lr.ph.lr.ph, %if.then2
  %pos.tr.ph19 = phi i32 [ %pos, %if.end.lr.ph.lr.ph ], [ %add, %if.then2 ]
  %accumulator.tr.ph18 = phi i32 [ -2147483648, %if.end.lr.ph.lr.ph ], [ %call.call7, %if.then2 ]
  %1 = sext i32 %pos.tr.ph19 to i64
  br label %if.end

if.end:                                           ; preds = %if.end.lr.ph, %if.else11
  %indvars.iv = phi i64 [ %1, %if.end.lr.ph ], [ %indvars.iv.next, %if.else11 ]
  %arrayidx = getelementptr inbounds [30 x i32], ptr @high, i64 0, i64 %indvars.iv
  %2 = load i32, ptr %arrayidx, align 4, !tbaa !5
  %cmp1.not = icmp sgt i32 %2, %highflag
  br i1 %cmp1.not, label %if.else11, label %if.then2

if.then2:                                         ; preds = %if.end
  %3 = trunc nsw i64 %indvars.iv to i32
  %add = add nsw i32 %3, 1
  %call = tail call noundef i32 @_Z2ffiii(i32 noundef %add, i32 noundef %2, i32 noundef %add5)
  %call.call7 = tail call i32 @llvm.smax.i32(i32 %call, i32 %accumulator.tr.ph18)
  %cmp14 = icmp eq i32 %add, %0
  br i1 %cmp14, label %return, label %if.end.lr.ph
...
return:                                           ; preds = %if.then2, %if.else11, %entry
  %accumulator.tr.ph.lcssa = phi i32 [ -2147483648, %entry ], [ %accumulator.tr.ph18, %if.else11 ], [ %call.call7, %if.then2 ]
  %accumulator.ret.tr = tail call i32 @llvm.smax.i32(i32 %count, i32 %accumulator.tr.ph.lcssa)
