...
for.cond10:                                       ; preds = %for.inc35, %if.else8
  ...
  %exitcond11.not = icmp eq i32 %i.0, %smax
  br i1 %exitcond11.not, label %while.cond38, label %while.cond

while.cond:                                       ; preds = %for.cond10, %while.body
  %indvars.iv4 = phi i64 [ %indvars.iv.next5, %while.body ], [ 99, %for.cond10 ]
  %cmp13.not = icmp eq i64 %indvars.iv4, 0
  br i1 %cmp13.not, label %while.cond18, label %while.body

while.body:                                       ; preds = %while.cond
  %arrayidx15 = getelementptr inbounds [100 x i32], ptr %re, i64 0, i64 %indvars.iv4
...
while.cond18:                                     ; preds = %while.cond, %if.end
  %indvars.iv7 = phi i64 [ %indvars.iv.next8.pre-phi, %if.end ], [ 99, %while.cond ]
  %cmp19.not = icmp eq i64 %indvars.iv7, 0
  br i1 %cmp19.not, label %for.inc35, label %while.body20

while.body20:                                     ; preds = %while.cond18
  %arrayidx22 = getelementptr inbounds [100 x i32], ptr %re, i64 0, i64 %indvars.iv7
...
if.then24:                                        ; preds = %while.body20
  ...
  %rem2 = urem i32 %2, 10
  store i32 %rem2, ptr %arrayidx22, align 4, !tbaa !5
  br label %if.end

if.end:                                           ; preds = %while.body20.if.end_crit_edge, %if.then24
  %indvars.iv.next8.pre-phi = phi i64 [ %.pre, %while.body20.if.end_crit_edge ], [ %3, %if.then24 ]
...
while.cond38:                                     ; preds = %for.cond10, %while.cond38
  %indvars.iv12 = phi i64 [ %indvars.iv.next13, %while.cond38 ], [ 0, %for.cond10 ]
  %arrayidx40 = getelementptr inbounds nuw [100 x i32], ptr %re, i64 0, i64 %indvars.iv12
...
for.cond45:                                       ; preds = %while.cond38, %for.inc51
  %indvars.iv15 = phi i64 [ %indvars.iv.next16, %for.inc51 ], [ %indvars.iv12, %while.cond38 ]
  %cmp46 = icmp samesign ult i64 %indvars.iv15, 100
  br i1 %cmp46, label %for.inc51, label %for.end53

for.inc51:                                        ; preds = %for.cond45
  %arrayidx49 = getelementptr inbounds nuw [100 x i32], ptr %re, i64 0, i64 %indvars.iv15
