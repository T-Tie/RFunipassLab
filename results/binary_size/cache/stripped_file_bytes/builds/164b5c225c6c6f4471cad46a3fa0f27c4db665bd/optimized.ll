...
for.cond10:                                       ; preds = %if.then25, %for.cond10.preheader
  %indvars.iv26 = phi i64 [ %indvars.iv.next27, %if.then25 ], [ 0, %for.cond10.preheader ]
  %a.0 = phi i32 [ %a.237, %if.then25 ], [ undef, %for.cond10.preheader ]
  %b.0 = phi i32 [ %b.238, %if.then25 ], [ undef, %for.cond10.preheader ]
  %exitcond31.not = icmp eq i64 %indvars.iv26, %wide.trip.count30
  br i1 %exitcond31.not, label %for.end36, label %for.cond13.preheader
...
for.cond13:                                       ; preds = %for.cond13.preheader, %for.inc21
  %indvars.iv22 = phi i64 [ 0, %for.cond13.preheader ], [ %indvars.iv.next23, %for.inc21 ]
...
if.then25:                                        ; preds = %for.cond13, %for.end23
  %b.238 = phi i32 [ %6, %for.end23 ], [ %b.0, %for.cond13 ]
  %a.237 = phi i32 [ %4, %for.end23 ], [ %a.0, %for.cond13 ]
  %arrayidx29 = getelementptr inbounds [1000 x [1000 x i32]], ptr @s, i64 0, i64 %indvars.iv26, i64 %idxprom28
...
for.end36:                                        ; preds = %for.end23, %if.then25, %for.cond10
  %a.1 = phi i32 [ %a.0, %for.cond10 ], [ %a.237, %if.then25 ], [ %4, %for.end23 ]
  %b.1 = phi i32 [ %b.0, %for.cond10 ], [ %b.238, %if.then25 ], [ %6, %for.end23 ]
  %sub37 = add i32 %0, -1
  %smin = call i32 @llvm.smin.i32(i32 %sub37, i32 noundef -1)
  br label %for.cond38

for.cond38:                                       ; preds = %for.inc64, %for.end36
  %i.2 = phi i32 [ %sub37, %for.end36 ], [ %dec65, %for.inc64 ]
...
for.cond42:                                       ; preds = %for.cond42.preheader, %for.inc52
  %k.2 = phi i32 [ %dec, %for.inc52 ], [ %sub37, %for.cond42.preheader ]
...
for.end66:                                        ; preds = %if.then55, %for.cond38, %for.body44
  %i.217 = phi i32 [ %i.2, %for.body44 ], [ %smin, %for.cond38 ], [ %i.2, %if.then55 ]
  %d.1 = phi i32 [ %k.2, %for.body44 ], [ undef, %for.cond38 ], [ undef, %if.then55 ]
  %10 = xor i32 %b.1, -1
  %sub68 = add i32 %d.1, %10
  %11 = xor i32 %a.1, -1
  %sub70 = add i32 %i.217, %11
