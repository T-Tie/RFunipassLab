...
if.end:                                           ; preds = %entry
  ...
  %div89 = lshr i64 %sub, 1
  br label %while.body

while.body:                                       ; preds = %while.body, %if.end
  %0 = phi i64 [ %div89, %if.end ], [ %dec, %while.body ]
  %add.ptr = getelementptr inbounds i32, ptr %__first, i64 %0
  %1 = load i32, ptr %add.ptr, align 4, !tbaa !5, !invariant.load !12
  tail call void @_ZSt13__adjust_heapIPiliN9__gnu_cxx5__ops15_Iter_less_iterEEvT_T0_S5_T1_T2_(ptr noundef %__first, i64 noundef %0, i64 noundef %sub.ptr.div, i32 noundef %1)
...
if.then:                                          ; preds = %entry
  %cmp.i5 = icmp slt i32 %1, %2
  br i1 %cmp.i5, label %if.end16, label %if.else

if.else:                                          ; preds = %if.then
  %cmp.i6 = icmp slt i32 %0, %2
  %3 = call i32 @llvm.smax.i32(i32 %0, i32 %2)
  %spec.select = select i1 %cmp.i6, ptr %__c, ptr %__a
  br label %if.end16

if.else7:                                         ; preds = %entry
  %cmp.i7 = icmp slt i32 %0, %2
  br i1 %cmp.i7, label %if.end16, label %if.else10

if.else10:                                        ; preds = %if.else7
  %cmp.i8 = icmp slt i32 %1, %2
  %4 = call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %spec.select9 = select i1 %cmp.i8, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.then, %if.else10, %if.else, %if.else7
  %5 = phi i32 [ %0, %if.else7 ], [ %3, %if.else ], [ %4, %if.else10 ], [ %1, %if.then ]
  %__b.sink = phi ptr [ %__a, %if.else7 ], [ %spec.select, %if.else ], [ %spec.select9, %if.else10 ], [ %__b, %if.then ]
