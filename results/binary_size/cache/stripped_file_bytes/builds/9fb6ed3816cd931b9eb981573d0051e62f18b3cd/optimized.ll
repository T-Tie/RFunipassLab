; ModuleID = '<stdin>'
source_filename = "/tmp/tmpiqfsmc4d.cpp"
...
for.cond:                                         ; preds = %for.inc28, %entry
  %lenth.0 = phi i64 [ 0, %entry ], [ %lenth.1, %for.inc28 ]
  %i.0 = phi i32 [ 0, %entry ], [ %inc29, %for.inc28 ]
  %flag.0 = phi i32 [ 0, %entry ], [ %flag.111, %for.inc28 ]
  %0 = load i32, ptr %n, align 4, !tbaa !5
  %cmp = icmp slt i32 %i.0, %0
  br i1 %cmp, label %if.then6.i, label %for.end30
...
_ZStrsIcSt11char_traitsIcEERSt13basic_istreamIT_T0_ES6_PS3_.exit: ; preds = %if.then6.i, %land.lhs.true.i, %if.then19.i, %if.then28.i
  %call4 = call i64 @strlen(ptr noundef nonnull readonly captures(none) dereferenceable(1) %a) #10
  %sext = shl i64 %lenth.0, 32
  %conv = ashr exact i64 %sext, 32
  %add = add nsw i64 %conv, 1
  %add5 = add i64 %add, %call4
  %conv6 = trunc i64 %add5 to i32
  %cmp7 = icmp slt i32 %conv6, 82
  br i1 %cmp7, label %if.then, label %if.then16

if.then:                                          ; preds = %_ZStrsIcSt11char_traitsIcEERSt13basic_istreamIT_T0_ES6_PS3_.exit
  %cmp8.not = icmp eq i32 %flag.0, 0
...
if.then16:                                        ; preds = %_ZStrsIcSt11char_traitsIcEERSt13basic_istreamIT_T0_ES6_PS3_.exit
  ...
  %call19 = call i64 @strlen(ptr noundef nonnull readonly captures(none) dereferenceable(1) %a) #10
  %add20 = add i64 %call19, 1
  br label %if.end24

if.end24:                                         ; preds = %if.then9, %if.then, %if.then16
  %flag.111 = phi i32 [ %flag.0, %if.then16 ], [ 1, %if.then ], [ 1, %if.then9 ]
  %lenth.1 = phi i64 [ %add20, %if.then16 ], [ %add5, %if.then ], [ %add5, %if.then9 ]
  %call12 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(8) @_ZSt4cout, ptr noundef nonnull %a)
  br label %for.cond25
...
for.cond25:                                       ; preds = %for.inc, %if.end24
  ...
  br i1 %exitcond.not, label %for.inc28, label %for.inc

for.inc:                                          ; preds = %for.cond25
  %add.ptr = getelementptr inbounds nuw i8, ptr %a, i64 %indvars.iv
  store i8 0, ptr %add.ptr, align 1, !tbaa !21
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1
  br label %for.cond25, !llvm.loop !22

for.inc28:                                        ; preds = %for.cond25
