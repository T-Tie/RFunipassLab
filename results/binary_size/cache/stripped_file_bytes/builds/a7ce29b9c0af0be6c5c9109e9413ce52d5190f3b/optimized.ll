...
for.body:                                         ; preds = %for.cond
  ...
  %or.cond3 = select i1 %or.cond, i1 %cmp4, i1 false
  %3 = load i32, ptr %d, align 4
  %cmp6 = icmp eq i32 %3, 0
  %or.cond4 = select i1 %or.cond3, i1 %cmp6, i1 false
  %4 = load i32, ptr %e, align 4
  %cmp8 = icmp eq i32 %4, 0
  %or.cond5 = select i1 %or.cond4, i1 %cmp8, i1 false
  %5 = load i32, ptr %f, align 4
  %cmp10 = icmp eq i32 %5, 0
  %or.cond6 = select i1 %or.cond5, i1 %cmp10, i1 false
  br i1 %or.cond6, label %cleanup, label %if.else

if.else:                                          ; preds = %for.body
  %cmp11.not = icmp sgt i32 %2, %5
  %cmp13.not = icmp sgt i32 %1, %4
  %or.cond11 = select i1 %cmp11.not, i1 true, i1 %cmp13.not
  br i1 %or.cond11, label %if.else21, label %if.then14
...
if.then14:                                        ; preds = %if.else
  ...
  %reass.sub31 = sub i32 %6, %2
  %sub18 = add i32 %reass.sub31, 43200
  %mul = add i32 %sub18, %5
  %add20 = add i32 %mul, %mul16
  br label %for.inc

if.else21:                                        ; preds = %if.else
  %cmp22.not = icmp slt i32 %2, %5
  %or.cond12 = select i1 %cmp22.not, i1 true, i1 %cmp13.not
  br i1 %or.cond12, label %if.else35, label %if.then25

if.then25:                                        ; preds = %if.else21
  %reass.sub8 = sub i32 %3, %0
  %7 = mul i32 %reass.sub8, 3600
  %sub29 = sub nsw i32 %4, %1
  %mul30 = mul nuw nsw i32 %sub29, 60
  %reass.sub32 = sub i32 %7, %2
  %sub32 = add i32 %reass.sub32, 43200
  %mul28 = add i32 %sub32, %5
  %add34 = add i32 %mul28, %mul30
  br label %for.inc

if.else35:                                        ; preds = %if.else21
  %cmp38.not = icmp slt i32 %1, %4
  %or.cond13 = select i1 %cmp11.not, i1 true, i1 %cmp38.not
  br i1 %or.cond13, label %if.else49, label %if.then39

if.then39:                                        ; preds = %if.else35
  %reass.sub9 = sub i32 %3, %0
  %8 = mul i32 %reass.sub9, 3600
  %sub43 = sub nsw i32 %4, %1
  %mul44 = mul nsw i32 %sub43, 60
  %reass.sub33 = sub i32 %8, %2
  %sub46 = add i32 %reass.sub33, 43200
  %mul42 = add i32 %sub46, %5
  %add48 = add i32 %mul42, %mul44
  br label %for.inc

if.else49:                                        ; preds = %if.else35
  %or.cond14 = select i1 %cmp22.not, i1 true, i1 %cmp38.not
  br i1 %or.cond14, label %for.inc, label %if.then53

if.then53:                                        ; preds = %if.else49
  %reass.sub10 = sub i32 %3, %0
  %9 = mul i32 %reass.sub10, 3600
  %sub57 = sub nsw i32 %4, %1
  %mul58 = mul nsw i32 %sub57, 60
  %reass.sub34 = sub i32 %9, %2
  %sub60 = add i32 %reass.sub34, 43200
  %mul56 = add i32 %sub60, %5
  %add62 = add i32 %mul56, %mul58
  br label %for.inc

for.inc:                                          ; preds = %if.then25, %if.else49, %if.then53, %if.then39, %if.then14
  %ans.1 = phi i32 [ %add20, %if.then14 ], [ %add34, %if.then25 ], [ %add48, %if.then39 ], [ %add62, %if.then53 ], [ %ans.0, %if.else49 ]
  %call67 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.1, i32 noundef %ans.1) #5
