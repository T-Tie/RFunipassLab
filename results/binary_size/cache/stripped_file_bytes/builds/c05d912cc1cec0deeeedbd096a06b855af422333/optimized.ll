; ModuleID = '/tmp/tmpxvngq8in.cpp'
source_filename = "/tmp/tmpxvngq8in.cpp"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

module asm ".globl _ZSt21ios_base_library_initv"

@.str = private unnamed_addr constant [13 x i8] c"%d%d%d%d%d%d\00", align 1
@.str.1 = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1

; Function Attrs: mustprogress norecurse uwtable
define dso_local noundef i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %i = alloca i32, align 4
  %count = alloca i32, align 4
  %sum = alloca i32, align 4
  %a = alloca [1000 x i32], align 16
  %b = alloca [1000 x i32], align 16
  %c = alloca [1000 x i32], align 16
  %d = alloca [1000 x i32], align 16
  %e = alloca [1000 x i32], align 16
  %f = alloca [1000 x i32], align 16
  store i32 0, ptr %retval, align 4
  call void @llvm.lifetime.start.p0(i64 4, ptr %i) #3
  store i32 0, ptr %i, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 4, ptr %count) #3
  store i32 0, ptr %count, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 4, ptr %sum) #3
  store i32 0, ptr %sum, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 4000, ptr %a) #3
  call void @llvm.lifetime.start.p0(i64 4000, ptr %b) #3
  call void @llvm.lifetime.start.p0(i64 4000, ptr %c) #3
  call void @llvm.lifetime.start.p0(i64 4000, ptr %d) #3
  call void @llvm.lifetime.start.p0(i64 4000, ptr %e) #3
  call void @llvm.lifetime.start.p0(i64 4000, ptr %f) #3
  br label %while.cond

while.cond:                                       ; preds = %if.end, %entry
  br label %while.body

while.body:                                       ; preds = %while.cond
  %0 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom = sext i32 %0 to i64
  %arrayidx = getelementptr inbounds [1000 x i32], ptr %a, i64 0, i64 %idxprom
  %1 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom1 = sext i32 %1 to i64
  %arrayidx2 = getelementptr inbounds [1000 x i32], ptr %b, i64 0, i64 %idxprom1
  %2 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom3 = sext i32 %2 to i64
  %arrayidx4 = getelementptr inbounds [1000 x i32], ptr %c, i64 0, i64 %idxprom3
  %3 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom5 = sext i32 %3 to i64
  %arrayidx6 = getelementptr inbounds [1000 x i32], ptr %d, i64 0, i64 %idxprom5
  %4 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom7 = sext i32 %4 to i64
  %arrayidx8 = getelementptr inbounds [1000 x i32], ptr %e, i64 0, i64 %idxprom7
  %5 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom9 = sext i32 %5 to i64
  %arrayidx10 = getelementptr inbounds [1000 x i32], ptr %f, i64 0, i64 %idxprom9
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %arrayidx, ptr noundef %arrayidx2, ptr noundef %arrayidx4, ptr noundef %arrayidx6, ptr noundef %arrayidx8, ptr noundef %arrayidx10)
  %6 = load i32, ptr %count, align 4, !tbaa !5
  %inc = add nsw i32 %6, 1
  store i32 %inc, ptr %count, align 4, !tbaa !5
  %7 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom11 = sext i32 %7 to i64
  %arrayidx12 = getelementptr inbounds [1000 x i32], ptr %a, i64 0, i64 %idxprom11
  %8 = load i32, ptr %arrayidx12, align 4, !tbaa !5
  %cmp = icmp eq i32 %8, 0
  br i1 %cmp, label %land.lhs.true, label %if.end

land.lhs.true:                                    ; preds = %while.body
  %9 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom13 = sext i32 %9 to i64
  %arrayidx14 = getelementptr inbounds [1000 x i32], ptr %b, i64 0, i64 %idxprom13
  %10 = load i32, ptr %arrayidx14, align 4, !tbaa !5
  %cmp15 = icmp eq i32 %10, 0
  br i1 %cmp15, label %land.lhs.true16, label %if.end

land.lhs.true16:                                  ; preds = %land.lhs.true
  %11 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom17 = sext i32 %11 to i64
  %arrayidx18 = getelementptr inbounds [1000 x i32], ptr %c, i64 0, i64 %idxprom17
  %12 = load i32, ptr %arrayidx18, align 4, !tbaa !5
  %cmp19 = icmp eq i32 %12, 0
  br i1 %cmp19, label %land.lhs.true20, label %if.end

land.lhs.true20:                                  ; preds = %land.lhs.true16
  %13 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom21 = sext i32 %13 to i64
  %arrayidx22 = getelementptr inbounds [1000 x i32], ptr %d, i64 0, i64 %idxprom21
  %14 = load i32, ptr %arrayidx22, align 4, !tbaa !5
  %cmp23 = icmp eq i32 %14, 0
  br i1 %cmp23, label %land.lhs.true24, label %if.end

land.lhs.true24:                                  ; preds = %land.lhs.true20
  %15 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom25 = sext i32 %15 to i64
  %arrayidx26 = getelementptr inbounds [1000 x i32], ptr %e, i64 0, i64 %idxprom25
  %16 = load i32, ptr %arrayidx26, align 4, !tbaa !5
  %cmp27 = icmp eq i32 %16, 0
  br i1 %cmp27, label %land.lhs.true28, label %if.end

land.lhs.true28:                                  ; preds = %land.lhs.true24
  %17 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom29 = sext i32 %17 to i64
  %arrayidx30 = getelementptr inbounds [1000 x i32], ptr %f, i64 0, i64 %idxprom29
  %18 = load i32, ptr %arrayidx30, align 4, !tbaa !5
  %cmp31 = icmp eq i32 %18, 0
  br i1 %cmp31, label %if.then, label %if.end

if.then:                                          ; preds = %land.lhs.true28
  br label %while.end

if.end:                                           ; preds = %land.lhs.true28, %land.lhs.true24, %land.lhs.true20, %land.lhs.true16, %land.lhs.true, %while.body
  %19 = load i32, ptr %i, align 4, !tbaa !5
  %inc32 = add nsw i32 %19, 1
  store i32 %inc32, ptr %i, align 4, !tbaa !5
  br label %while.cond, !llvm.loop !9

while.end:                                        ; preds = %if.then
  store i32 0, ptr %i, align 4, !tbaa !5
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %while.end
  %20 = load i32, ptr %i, align 4, !tbaa !5
  %21 = load i32, ptr %count, align 4, !tbaa !5
  %sub = sub nsw i32 %21, 1
  %cmp33 = icmp slt i32 %20, %sub
  br i1 %cmp33, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %22 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom34 = sext i32 %22 to i64
  %arrayidx35 = getelementptr inbounds [1000 x i32], ptr %d, i64 0, i64 %idxprom34
  %23 = load i32, ptr %arrayidx35, align 4, !tbaa !5
  %add = add nsw i32 12, %23
  %24 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom36 = sext i32 %24 to i64
  %arrayidx37 = getelementptr inbounds [1000 x i32], ptr %a, i64 0, i64 %idxprom36
  %25 = load i32, ptr %arrayidx37, align 4, !tbaa !5
  %sub38 = sub nsw i32 %add, %25
  %mul = mul nsw i32 3600, %sub38
  %26 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom39 = sext i32 %26 to i64
  %arrayidx40 = getelementptr inbounds [1000 x i32], ptr %e, i64 0, i64 %idxprom39
  %27 = load i32, ptr %arrayidx40, align 4, !tbaa !5
  %28 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom41 = sext i32 %28 to i64
  %arrayidx42 = getelementptr inbounds [1000 x i32], ptr %b, i64 0, i64 %idxprom41
  %29 = load i32, ptr %arrayidx42, align 4, !tbaa !5
  %sub43 = sub nsw i32 %27, %29
  %mul44 = mul nsw i32 60, %sub43
  %add45 = add nsw i32 %mul, %mul44
  %30 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom46 = sext i32 %30 to i64
  %arrayidx47 = getelementptr inbounds [1000 x i32], ptr %f, i64 0, i64 %idxprom46
  %31 = load i32, ptr %arrayidx47, align 4, !tbaa !5
  %32 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom48 = sext i32 %32 to i64
  %arrayidx49 = getelementptr inbounds [1000 x i32], ptr %c, i64 0, i64 %idxprom48
  %33 = load i32, ptr %arrayidx49, align 4, !tbaa !5
  %sub50 = sub nsw i32 %31, %33
  %add51 = add nsw i32 %add45, %sub50
  store i32 %add51, ptr %sum, align 4, !tbaa !5
  %34 = load i32, ptr %sum, align 4, !tbaa !5
  %call52 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %34)
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %35 = load i32, ptr %i, align 4, !tbaa !5
  %inc53 = add nsw i32 %35, 1
  store i32 %inc53, ptr %i, align 4, !tbaa !5
  br label %for.cond, !llvm.loop !12

for.end:                                          ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 4000, ptr %f) #3
  call void @llvm.lifetime.end.p0(i64 4000, ptr %e) #3
  call void @llvm.lifetime.end.p0(i64 4000, ptr %d) #3
  call void @llvm.lifetime.end.p0(i64 4000, ptr %c) #3
  call void @llvm.lifetime.end.p0(i64 4000, ptr %b) #3
  call void @llvm.lifetime.end.p0(i64 4000, ptr %a) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %sum) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %count) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %i) #3
  ret i32 0
}

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.start.p0(i64 immarg, ptr captures(none)) #1

declare i32 @__isoc23_scanf(ptr noundef, ...) #2

declare i32 @printf(ptr noundef, ...) #2

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.end.p0(i64 immarg, ptr captures(none)) #1

attributes #0 = { mustprogress norecurse uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
attributes #2 = { "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nounwind }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{!"clang version 21.1.8 (https://github.com/llvm/llvm-project.git 2078da43e25a4623cab2d0d60decddf709aaea28)"}
!5 = !{!6, !6, i64 0}
!6 = !{!"int", !7, i64 0}
!7 = !{!"omnipotent char", !8, i64 0}
!8 = !{!"Simple C++ TBAA"}
!9 = distinct !{!9, !10, !11}
!10 = !{!"llvm.loop.mustprogress"}
!11 = !{!"llvm.loop.unroll.disable"}
!12 = distinct !{!12, !10, !11}
