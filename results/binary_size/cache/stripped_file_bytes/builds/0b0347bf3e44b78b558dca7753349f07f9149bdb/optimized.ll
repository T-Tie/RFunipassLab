...
for.cond6:                                        ; preds = %for.body, %if.end23
  ...
  %exitcond38.not = icmp eq i32 %j.0, %spec.select
  br i1 %exitcond38.not, label %if.end51, label %for.body8
...
if.end23:                                         ; preds = %for.body8, %if.end23.fold.split, %if.else20
  %.sink2.sink = phi i32 [ 29, %for.body8 ], [ 31, %if.else20 ], [ 30, %if.end23.fold.split ]
  %add21 = add nuw nsw i32 %.sink2.sink, %day.2
  %inc = add i32 %j.0, 1
...
if.end47:                                         ; preds = %for.body30, %if.end47.fold.split, %if.else44
  %.sink1.sink = phi i32 [ 28, %for.body30 ], [ 31, %if.else44 ], [ 30, %if.end47.fold.split ]
  %add45 = add nuw nsw i32 %.sink1.sink, %day.4
  %inc49 = add i32 %j.1, 1
