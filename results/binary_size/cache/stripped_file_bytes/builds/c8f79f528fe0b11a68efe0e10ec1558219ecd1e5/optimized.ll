...
if.end28:                                         ; preds = %if.then17, %if.then
  %add25.sink = phi i64 [ %add25, %if.then17 ], [ %call8, %if.then ]
...
while.end:                                        ; preds = %if.end28, %while.cond1, %if.else
  %number.2 = phi i32 [ %number.1, %if.else ], [ %number.1, %while.cond1 ], [ %conv26, %if.end28 ]
