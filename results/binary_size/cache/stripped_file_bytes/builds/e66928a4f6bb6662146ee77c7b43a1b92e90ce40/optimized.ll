...
for.cond:                                         ; preds = %for.inc25, %entry
  ...
  %exitcond9.not = icmp eq i64 %indvars.iv, %wide.trip.count8
  br i1 %exitcond9.not, label %cleanup29, label %for.body
...
for.cond4:                                        ; preds = %for.body, %if.end15
  %indvars.iv4 = phi i64 [ %indvars.iv.next5, %if.end15 ], [ %indvars.iv, %for.body ]
  %exitcond.not = icmp eq i64 %indvars.iv4, %wide.trip.count
  br i1 %exitcond.not, label %for.inc25, label %for.body7

for.body7:                                        ; preds = %for.cond4
  %arrayidx9 = getelementptr inbounds nuw i32, ptr %p, i64 %indvars.iv4
