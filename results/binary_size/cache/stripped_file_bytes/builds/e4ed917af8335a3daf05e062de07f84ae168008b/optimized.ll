...
while.cond:                                       ; preds = %if.end33, %entry
  %h.0 = phi i32 [ undef, %entry ], [ %h.17, %if.end33 ]
  %flag.0 = phi i32 [ 1, %entry ], [ %flag.18, %if.end33 ]
  %cmp.not = icmp eq i32 %flag.0, 0
...
for.end:                                          ; preds = %for.cond, %for.end.split.loop.exit
  %h.1 = phi i32 [ %1, %for.end.split.loop.exit ], [ %h.0, %for.cond ]
  %smax = call i32 @llvm.smax.i32(i32 %h.1, i32 noundef 0)
  %wide.trip.count17 = zext nneg i32 %smax to i64
  %wide.trip.count = zext i32 %h.1 to i64
  br label %for.cond12

for.cond12:                                       ; preds = %for.end, %for.inc29
  %indvars.iv14 = phi i64 [ 0, %for.end ], [ %indvars.iv.next15, %for.inc29 ]
  %t.2 = phi i32 [ 0, %for.end ], [ %t.3, %for.inc29 ]
  %exitcond18.not = icmp eq i64 %indvars.iv14, %wide.trip.count17
  br i1 %exitcond18.not, label %for.end31, label %for.cond15.preheader

for.cond15.preheader:                             ; preds = %for.cond12
  %arrayidx19 = getelementptr inbounds nuw [16 x i32], ptr %a, i64 0, i64 %indvars.iv14
  br label %for.cond15

for.cond15:                                       ; preds = %for.cond15.preheader, %for.body17
  %indvars.iv10 = phi i64 [ 0, %for.cond15.preheader ], [ %indvars.iv.next11, %for.body17 ]
  %t.3 = phi i32 [ %t.2, %for.cond15.preheader ], [ %spec.select, %for.body17 ]
  %exitcond13.not = icmp eq i64 %indvars.iv10, %wide.trip.count
  br i1 %exitcond13.not, label %for.inc29, label %for.body17

for.body17:                                       ; preds = %for.cond15
  %2 = load i32, ptr %arrayidx19, align 4, !tbaa !5, !invariant.load !9
  %arrayidx21 = getelementptr inbounds nuw [16 x i32], ptr %a, i64 0, i64 %indvars.iv10
...
if.end33:                                         ; preds = %for.body, %for.end31
  %flag.18 = phi i32 [ %flag.0, %for.end31 ], [ 0, %for.body ]
  %h.17 = phi i32 [ %h.1, %for.end31 ], [ %h.0, %for.body ]
  br label %while.cond, !llvm.loop !15

while.end:                                        ; preds = %while.cond
  call void @llvm.lifetime.end.p0(i64 noundef 64, ptr noundef nonnull align 16 %a) #5
  ret i32 0
}
