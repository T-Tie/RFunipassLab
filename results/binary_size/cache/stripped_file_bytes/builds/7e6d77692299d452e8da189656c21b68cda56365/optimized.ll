...
entry:
  %cmp11 = icmp slt i32 %b, %a
  br i1 %cmp11, label %for.body, label %for.end

for.body:                                         ; preds = %entry, %for.inc
  %t.013 = phi i32 [ %t.1, %for.inc ], [ 0, %entry ]
  %i.012 = phi i32 [ %inc, %for.inc ], [ %b, %entry ]
  %rem = srem i32 %a, %i.012
  %div = sdiv i32 %a, %i.012
  %cmp1 = icmp ne i32 %rem, 0
  %cmp2.not = icmp slt i32 %div, %i.012
  %or.cond = or i1 %cmp1, %cmp2.not
  br i1 %or.cond, label %for.inc, label %if.then

if.then:                                          ; preds = %for.body
  %call = tail call noundef i32 @_Z1sii(i32 noundef %div, i32 noundef %i.012)
  %add = add nsw i32 %call, %t.013
  br label %for.inc
...
for.end:                                          ; preds = %for.end.loopexit, %entry
  %t.0.lcssa = phi i32 [ 1, %entry ], [ %0, %for.end.loopexit ]
  ret i32 %t.0.lcssa
}
