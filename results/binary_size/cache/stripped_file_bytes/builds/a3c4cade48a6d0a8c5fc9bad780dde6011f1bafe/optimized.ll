; ModuleID = '/tmp/tmpnu02slb3.cpp'
source_filename = "/tmp/tmpnu02slb3.cpp"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

module asm ".globl _ZSt21ios_base_library_initv"

@.str = private unnamed_addr constant [13 x i8] c"%d%d%d%d%d%d\00", align 1
@.str.1 = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1

; Function Attrs: mustprogress norecurse uwtable
define dso_local noundef i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %a = alloca i32, align 4
  %b = alloca i32, align 4
  %c = alloca i32, align 4
  %d = alloca i32, align 4
  %e = alloca i32, align 4
  %f = alloca i32, align 4
  %A = alloca i32, align 4
  %B = alloca i32, align 4
  %C = alloca i32, align 4
  %m = alloca i32, align 4
  store i32 0, ptr %retval, align 4
  call void @llvm.lifetime.start.p0(i64 4, ptr %a) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %b) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %c) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %d) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %e) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %f) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %A) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %B) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %C) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %m) #3
  store i32 0, ptr %m, align 4, !tbaa !5
  br label %while.cond

while.cond:                                       ; preds = %if.end46, %entry
  br label %while.body

while.body:                                       ; preds = %while.cond
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %a, ptr noundef %b, ptr noundef %c, ptr noundef %d, ptr noundef %e, ptr noundef %f)
  %0 = load i32, ptr %a, align 4, !tbaa !5
  %1 = load i32, ptr %b, align 4, !tbaa !5
  %cmp = icmp eq i32 %0, %1
  br i1 %cmp, label %land.lhs.true, label %if.else

land.lhs.true:                                    ; preds = %while.body
  %2 = load i32, ptr %b, align 4, !tbaa !5
  %3 = load i32, ptr %c, align 4, !tbaa !5
  %cmp1 = icmp eq i32 %2, %3
  br i1 %cmp1, label %land.lhs.true2, label %if.else

land.lhs.true2:                                   ; preds = %land.lhs.true
  %4 = load i32, ptr %c, align 4, !tbaa !5
  %5 = load i32, ptr %d, align 4, !tbaa !5
  %cmp3 = icmp eq i32 %4, %5
  br i1 %cmp3, label %land.lhs.true4, label %if.else

land.lhs.true4:                                   ; preds = %land.lhs.true2
  %6 = load i32, ptr %d, align 4, !tbaa !5
  %7 = load i32, ptr %e, align 4, !tbaa !5
  %cmp5 = icmp eq i32 %6, %7
  br i1 %cmp5, label %land.lhs.true6, label %if.else

land.lhs.true6:                                   ; preds = %land.lhs.true4
  %8 = load i32, ptr %e, align 4, !tbaa !5
  %9 = load i32, ptr %f, align 4, !tbaa !5
  %cmp7 = icmp eq i32 %8, %9
  br i1 %cmp7, label %land.lhs.true8, label %if.else

land.lhs.true8:                                   ; preds = %land.lhs.true6
  %10 = load i32, ptr %f, align 4, !tbaa !5
  %cmp9 = icmp eq i32 %10, 0
  br i1 %cmp9, label %if.then, label %if.else

if.then:                                          ; preds = %land.lhs.true8
  br label %while.end

if.else:                                          ; preds = %land.lhs.true8, %land.lhs.true6, %land.lhs.true4, %land.lhs.true2, %land.lhs.true, %while.body
  %11 = load i32, ptr %c, align 4, !tbaa !5
  %12 = load i32, ptr %f, align 4, !tbaa !5
  %cmp10 = icmp sgt i32 %11, %12
  br i1 %cmp10, label %if.then11, label %if.else25

if.then11:                                        ; preds = %if.else
  %13 = load i32, ptr %f, align 4, !tbaa !5
  %add = add nsw i32 %13, 60
  %14 = load i32, ptr %c, align 4, !tbaa !5
  %sub = sub nsw i32 %add, %14
  store i32 %sub, ptr %C, align 4, !tbaa !5
  %15 = load i32, ptr %e, align 4, !tbaa !5
  %sub12 = sub nsw i32 %15, 1
  %16 = load i32, ptr %b, align 4, !tbaa !5
  %cmp13 = icmp sle i32 %sub12, %16
  br i1 %cmp13, label %if.then14, label %if.else20

if.then14:                                        ; preds = %if.then11
  %17 = load i32, ptr %e, align 4, !tbaa !5
  %sub15 = sub nsw i32 %17, 1
  %add16 = add nsw i32 %sub15, 60
  %18 = load i32, ptr %b, align 4, !tbaa !5
  %sub17 = sub nsw i32 %add16, %18
  store i32 %sub17, ptr %B, align 4, !tbaa !5
  %19 = load i32, ptr %d, align 4, !tbaa !5
  %add18 = add nsw i32 11, %19
  %20 = load i32, ptr %a, align 4, !tbaa !5
  %sub19 = sub nsw i32 %add18, %20
  store i32 %sub19, ptr %A, align 4, !tbaa !5
  br label %if.end

if.else20:                                        ; preds = %if.then11
  %21 = load i32, ptr %e, align 4, !tbaa !5
  %sub21 = sub nsw i32 %21, 1
  %22 = load i32, ptr %b, align 4, !tbaa !5
  %sub22 = sub nsw i32 %sub21, %22
  store i32 %sub22, ptr %B, align 4, !tbaa !5
  %23 = load i32, ptr %d, align 4, !tbaa !5
  %add23 = add nsw i32 12, %23
  %24 = load i32, ptr %a, align 4, !tbaa !5
  %sub24 = sub nsw i32 %add23, %24
  store i32 %sub24, ptr %A, align 4, !tbaa !5
  br label %if.end

if.end:                                           ; preds = %if.else20, %if.then14
  br label %if.end41

if.else25:                                        ; preds = %if.else
  %25 = load i32, ptr %c, align 4, !tbaa !5
  %26 = load i32, ptr %f, align 4, !tbaa !5
  %cmp26 = icmp sle i32 %25, %26
  br i1 %cmp26, label %if.then27, label %if.end40

if.then27:                                        ; preds = %if.else25
  %27 = load i32, ptr %f, align 4, !tbaa !5
  %28 = load i32, ptr %c, align 4, !tbaa !5
  %sub28 = sub nsw i32 %27, %28
  store i32 %sub28, ptr %C, align 4, !tbaa !5
  %29 = load i32, ptr %e, align 4, !tbaa !5
  %30 = load i32, ptr %b, align 4, !tbaa !5
  %cmp29 = icmp sle i32 %29, %30
  br i1 %cmp29, label %if.then30, label %if.else35

if.then30:                                        ; preds = %if.then27
  %31 = load i32, ptr %e, align 4, !tbaa !5
  %add31 = add nsw i32 %31, 60
  %32 = load i32, ptr %b, align 4, !tbaa !5
  %sub32 = sub nsw i32 %add31, %32
  store i32 %sub32, ptr %B, align 4, !tbaa !5
  %33 = load i32, ptr %d, align 4, !tbaa !5
  %add33 = add nsw i32 11, %33
  %34 = load i32, ptr %a, align 4, !tbaa !5
  %sub34 = sub nsw i32 %add33, %34
  store i32 %sub34, ptr %A, align 4, !tbaa !5
  br label %if.end39

if.else35:                                        ; preds = %if.then27
  %35 = load i32, ptr %e, align 4, !tbaa !5
  %36 = load i32, ptr %b, align 4, !tbaa !5
  %sub36 = sub nsw i32 %35, %36
  store i32 %sub36, ptr %B, align 4, !tbaa !5
  %37 = load i32, ptr %d, align 4, !tbaa !5
  %add37 = add nsw i32 12, %37
  %38 = load i32, ptr %a, align 4, !tbaa !5
  %sub38 = sub nsw i32 %add37, %38
  store i32 %sub38, ptr %A, align 4, !tbaa !5
  br label %if.end39

if.end39:                                         ; preds = %if.else35, %if.then30
  br label %if.end40

if.end40:                                         ; preds = %if.end39, %if.else25
  br label %if.end41

if.end41:                                         ; preds = %if.end40, %if.end
  %39 = load i32, ptr %A, align 4, !tbaa !5
  %mul = mul nsw i32 3600, %39
  %40 = load i32, ptr %B, align 4, !tbaa !5
  %mul42 = mul nsw i32 60, %40
  %add43 = add nsw i32 %mul, %mul42
  %41 = load i32, ptr %C, align 4, !tbaa !5
  %add44 = add nsw i32 %add43, %41
  store i32 %add44, ptr %m, align 4, !tbaa !5
  %42 = load i32, ptr %m, align 4, !tbaa !5
  %call45 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %42)
  br label %if.end46

if.end46:                                         ; preds = %if.end41
  br label %while.cond, !llvm.loop !9

while.end:                                        ; preds = %if.then
  call void @llvm.lifetime.end.p0(i64 4, ptr %m) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %C) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %B) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %A) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %f) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %e) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %d) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %c) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %b) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %a) #3
  ret i32 0
}

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.start.p0(i64 immarg, ptr captures(none)) #1

declare i32 @__isoc23_scanf(ptr noundef, ...) #2

declare i32 @printf(ptr noundef, ...) #2

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.end.p0(i64 immarg, ptr captures(none)) #1

attributes #0 = { mustprogress norecurse uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
attributes #2 = { "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nounwind }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{!"clang version 21.1.8 (https://github.com/llvm/llvm-project.git 2078da43e25a4623cab2d0d60decddf709aaea28)"}
!5 = !{!6, !6, i64 0}
!6 = !{!"int", !7, i64 0}
!7 = !{!"omnipotent char", !8, i64 0}
!8 = !{!"Simple C++ TBAA"}
!9 = distinct !{!9, !10, !11}
!10 = !{!"llvm.loop.mustprogress"}
!11 = !{!"llvm.loop.unroll.disable"}
