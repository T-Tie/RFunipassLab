...
for.inc.sink.split:                               ; preds = %if.else, %for.body
  ...
  %j.1.ph = phi i32 [ 0, %for.body ], [ 1, %if.else ]
  %call9 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_c(ptr noundef nonnull align 8 dereferenceable(8) @_ZSt4cout, i8 noundef signext %.sink)
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %if.else
  %j.1 = phi i32 [ 1, %if.else ], [ %j.1.ph, %for.inc.sink.split ]
