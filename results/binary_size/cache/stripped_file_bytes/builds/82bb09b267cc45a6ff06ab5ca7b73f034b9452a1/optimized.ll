...
if.end:                                           ; preds = %entry
  ...
  %call = tail call double @sqrt(double noundef %conv) #7, !tbaa !5
  %conv1 = fptosi double %call to i32
  br label %for.cond

for.cond:                                         ; preds = %if.end6, %if.end
  %count.0 = phi i32 [ 0, %if.end ], [ %count.1, %if.end6 ]
  %i.0 = phi i32 [ %minA1, %if.end ], [ %inc7, %if.end6 ]
...
if.then4:                                         ; preds = %for.body
  ...
  %call5 = tail call noundef i32 @_Z6factorii(i32 noundef %div, i32 noundef %i.0) #8
  %add = add nsw i32 %inc, %call5
  br label %if.end6

if.end6:                                          ; preds = %if.then4, %for.body
  %count.1 = phi i32 [ %add, %if.then4 ], [ %count.0, %for.body ]
  %inc7 = add nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !9

cleanup:                                          ; preds = %for.cond, %entry
  %retval.0 = phi i32 [ 0, %entry ], [ %count.0, %for.cond ]
  ret i32 %retval.0
}
...
while.body:                                       ; preds = %while.cond
  ...
  %call2 = call noundef i32 @_Z6factorii(i32 noundef %1, i32 noundef 2) #9
...
attributes #7 = { nofree nounwind memory(write) }
