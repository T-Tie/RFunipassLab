...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 500000, ptr noundef nonnull align 16 %a) #6
  call void @llvm.memset.p0.i64(ptr noundef nonnull writeonly align 16 dereferenceable(500000) %a, i8 noundef 0, i64 noundef 500000, i1 noundef false) #7
  br label %while.cond

while.cond:                                       ; preds = %while.cond.backedge, %entry
  %i.0 = phi i32 [ 0, %entry ], [ %i.4, %while.cond.backedge ]
  %j.0 = phi i32 [ 0, %entry ], [ %j.3, %while.cond.backedge ]
  %count.0 = phi i32 [ 0, %entry ], [ %count.2, %while.cond.backedge ]
  %call6 = tail call i32 @getchar() #6
...
for.cond.preheader:                               ; preds = %while.cond
  ...
  br i1 %cmp19, label %for.cond, label %for.cond38.preheader

if.then:                                          ; preds = %while.cond
  %cmp4.not = icmp eq i32 %count.0, 0
  br i1 %cmp4.not, label %if.then9, label %while.cond.backedge

while.cond.backedge:                              ; preds = %if.then, %if.else12, %if.then9
  %i.4 = phi i32 [ %inc, %if.then9 ], [ %i.0, %if.else12 ], [ %i.0, %if.then ]
  %j.3 = phi i32 [ 0, %if.then9 ], [ %inc13, %if.else12 ], [ %j.0, %if.then ]
  %count.2 = phi i32 [ 1, %if.then9 ], [ 0, %if.else12 ], [ %count.0, %if.then ]
...
if.else12:                                        ; preds = %while.cond
  %c.0 = trunc i32 %call6 to i8
  %idxprom = sext i32 %i.0 to i64
  %inc13 = add nsw i32 %j.0, 1
  %idxprom14 = sext i32 %j.0 to i64
  %arrayidx15 = getelementptr inbounds [10000 x [50 x i8]], ptr %a, i64 0, i64 %idxprom, i64 %idxprom14
  store i8 %c.0, ptr %arrayidx15, align 1, !tbaa !8
  br label %while.cond.backedge

for.cond:                                         ; preds = %for.cond.preheader, %for.inc35
  br i1 %cmp26.not, label %for.inc35, label %for.cond20
...
for.cond20:                                       ; preds = %for.cond, %for.cond20
  %2 = phi i8 [ %.pre, %for.cond20 ], [ %0, %for.cond ]
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.cond20 ], [ 0, %for.cond ]
...
for.end53:                                        ; preds = %for.cond38.preheader
  call void @llvm.lifetime.end.p0(i64 noundef 500000, ptr noundef nonnull %a) #6
  ret i32 0
}
