...
entry:
  ...
  %add = add nsw i32 %d, 1
  %smax = tail call i32 @llvm.smax.i32(i32 %d, i32 -1)
  %0 = add i32 %smax, 1
  %smax19 = tail call i32 @llvm.smax.i32(i32 %i, i32 %m)
  %wide.trip.count = zext i32 %0 to i64
  br label %for.cond

for.cond:                                         ; preds = %for.inc12, %entry
  %a.0 = phi i32 [ %i, %entry ], [ %inc13, %for.inc12 ]
  %exitcond20.not = icmp eq i32 %a.0, %smax19
  br i1 %exitcond20.not, label %for.end14.loopexit, label %for.body
...
for.cond2:                                        ; preds = %for.body4, %if.then
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body4 ], [ 0, %if.then ]
  %chenji.0 = phi i32 [ %mul, %for.body4 ], [ 1, %if.then ]
...
cleanup.thread:                                   ; preds = %for.end
  %call = tail call noundef i32 @_Z4fjysiiiiPi(i32 noundef %div, i32 noundef %m, i32 noundef %a.0, i32 noundef %add, ptr noundef nonnull %ji)
  br label %for.inc12
...
for.cond:                                         ; preds = %for.body, %entry
  %j.0 = phi i32 [ 1, %entry ], [ %inc, %for.body ]
...
for.body:                                         ; preds = %for.cond
  ...
  %vtable.i = load ptr, ptr %call3, align 8, !tbaa !13
