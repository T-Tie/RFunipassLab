...
for.inc:                                          ; preds = %for.body3, %if.then, %if.else
  %state.2 = phi i32 [ %state.1, %for.body3 ], [ %state.1, %if.else ], [ 1, %if.then ]
  %leftx.2 = phi i32 [ %leftx.1, %for.body3 ], [ %leftx.1, %if.else ], [ %i.0, %if.then ]
  %lefty.2 = phi i32 [ %lefty.1, %for.body3 ], [ %lefty.1, %if.else ], [ %j.0, %if.then ]
  %rightx.2 = phi i32 [ %rightx.1, %for.body3 ], [ %i.0, %if.else ], [ %rightx.1, %if.then ]
  %righty.2 = phi i32 [ %righty.1, %for.body3 ], [ %j.0, %if.else ], [ %righty.1, %if.then ]
