...
for.cond:                                         ; preds = %for.inc, %entry
  %a.0 = phi i32 [ 0, %entry ], [ %a.111, %for.inc ]
  %flag.0 = phi i32 [ 1, %entry ], [ %flag.14, %for.inc ]
...
if.end.thread:                                    ; preds = %for.body
  %cmp410 = icmp sgt i32 %a.0, 0
  %spec.select18 = select i1 %cmp410, i32 0, i32 %flag.0
  br label %for.inc

for.inc:                                          ; preds = %if.end.thread, %land.lhs.true
  %a.111 = phi i32 [ %spec.select, %land.lhs.true ], [ %a.0, %if.end.thread ]
  %flag.14 = phi i32 [ %flag.0, %land.lhs.true ], [ %spec.select18, %if.end.thread ]
