...
for.cond:                                         ; preds = %if.end, %entry.split
  %i.0 = phi i32 [ %j, %entry.split ], [ %inc6.i, %if.end ]
  %cmp2 = icmp ne i32 %n, %i.0
...
if.then:                                          ; preds = %land.lhs.true3
  ...
  %call = tail call noundef i32 @_Z5yinziii(i32 noundef %i.0, i32 noundef %div) #7
  br label %if.end

if.end:                                           ; preds = %if.then, %land.lhs.true3, %for.cond
  %inc6.i = add nsw i32 %i.0, 1
  br label %for.cond
...
for.cond2:                                        ; preds = %if.end, %for.body
  %i.0 = phi i32 [ 2, %for.body ], [ %inc6.i2, %if.end ]
...
if.then:                                          ; preds = %land.lhs.true
  ...
  %inc = add nsw i32 %3, 1
  store i32 %inc, ptr @a, align 4, !tbaa !5
  %call9 = call noundef i32 @_Z5yinziii(i32 noundef %i.0, i32 noundef %div) #9
  br label %if.end

if.end:                                           ; preds = %if.then, %land.lhs.true, %for.body5
  %inc6.i2 = add nuw nsw i32 %i.0, 1
  br label %for.cond2
