...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 400, ptr noundef nonnull align 16 %b) #6
  call void @llvm.lifetime.start.p0(i64 noundef 400, ptr noundef nonnull align 16 %c) #6
...
if.then:                                          ; preds = %for.body
  %call.i = call noundef float @sqrtf(float noundef nofpclass(nzero) %6) #7, !tbaa !5
...
if.then57:                                        ; preds = %if.then
  %call58 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.2, double noundef %div) #8
  br label %if.end90

if.else:                                          ; preds = %if.then
  %call59 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.3, double noundef %div, double noundef %div55) #8
  br label %if.end90

if.else60:                                        ; preds = %for.body
  %conv64 = fpext float %fneg to double
  %mul67 = fmul float %3, 2.000000e+00
  %conv68 = fpext float %mul67 to double
  %div69 = fdiv double %conv64, %conv68
  %neg81 = fmul float %2, %fneg
  %7 = call float @llvm.fmuladd.f32(float %mul12, float %4, float %neg81)
  %call.i7 = call noundef float @sqrtf(float noundef %7) #7, !tbaa !5
  %conv83 = fpext float %call.i7 to double
  %div88 = fdiv double %conv83, %conv68
  %call89 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.4, double noundef %div69, double noundef %div88, double noundef %div69, double noundef %div88) #8
  br label %if.end90
...
for.end:                                          ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 noundef 400, ptr noundef nonnull %c) #8
