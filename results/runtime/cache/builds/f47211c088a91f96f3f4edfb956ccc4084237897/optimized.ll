...
for.end20:                                        ; preds = %for.body7, %for.cond5
  %sub21.lcssa = phi i32 [ %sub21, %for.body7 ], [ %sub21, %for.cond5 ]
  br label %for.cond22

for.cond22:                                       ; preds = %for.inc57, %for.end20
  %j.0 = phi i32 [ %sub21.lcssa, %for.end20 ], [ %dec58, %for.inc57 ]
  %p.0 = phi i32 [ 0, %for.end20 ], [ %p.1.lcssa, %for.inc57 ]
...
for.end47:                                        ; preds = %for.cond29
  %sum.0.lcssa = phi i32 [ %sum.0, %for.cond29 ]
  %p.1.lcssa = phi i32 [ %p.1, %for.cond29 ]
  %cmp48 = icmp eq i32 %p.1.lcssa, 0
  br i1 %cmp48, label %for.inc57, label %if.else52

if.else52:                                        ; preds = %for.end47
  %add53 = add nsw i32 %sum.0.lcssa, 1
  br label %for.inc57
...
for.end72:                                        ; preds = %for.cond60
  %sum.2.lcssa = phi i32 [ %sum.2, %for.cond60 ]
  %call73 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %sum.2.lcssa) #5
