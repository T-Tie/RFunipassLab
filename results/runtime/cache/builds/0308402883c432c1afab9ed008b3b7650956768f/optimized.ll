...
entry:
  ...
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef nonnull @.str, ptr noundef nonnull align 4 %n, ptr noundef nonnull align 4 %a, ptr noundef nonnull align 4 %b, ptr noundef nonnull align 4 %t)
...
if.end:                                           ; preds = %entry, %if.else
  %2 = phi i32 [ %0, %entry ], [ %1, %if.else ]
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %if.end
  %3 = phi i32 [ %2, %if.end ], [ %8, %for.inc ]
...
if.else6:                                         ; preds = %for.body
  ...
  %or.cond2 = select i1 %cmp7, i1 %cmp4, i1 false
  br i1 %or.cond2, label %for.inc.sink.split, label %for.inc

for.inc.sink.split:                               ; preds = %if.else6, %if.then5
  %.sink = phi i32 [ %6, %if.then5 ], [ %5, %if.else6 ]
  %.ph = phi i32 [ %5, %if.then5 ], [ %6, %if.else6 ]
  store i32 %.sink, ptr %b, align 4, !tbaa !5
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %if.else6
  %8 = phi i32 [ %6, %if.else6 ], [ %.ph, %for.inc.sink.split ]
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef nonnull %c) #4
  %inc = add nuw nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !9

for.end:                                          ; preds = %for.cond
  %call14 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.2, i32 noundef %3)
