...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 4, ptr %year) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %month1) #3
  call void @llvm.lifetime.start.p0(i64 4, ptr %month2) #3
...
if.end26:                                         ; preds = %lor.lhs.false22
  %total1.2 = phi i32 [ %total1.1, %lor.lhs.false22 ]
...
for.inc.gvnsink.split:                            ; preds = %land.lhs.true, %lor.lhs.false32, %if.else
  %.sink5 = phi i32 [ 28, %if.else ], [ 29, %lor.lhs.false32 ], [ 29, %land.lhs.true ]
  %add37 = add nsw i32 %total1.2, %.sink5
  br label %for.inc

for.inc:                                          ; preds = %if.end26.thread, %for.inc.gvnsink.split, %if.end26
  %total1.3 = phi i32 [ %add37, %for.inc.gvnsink.split ], [ %total1.2, %if.end26 ], [ %add25, %if.end26.thread ]
...
if.end68:                                         ; preds = %lor.lhs.false64
  %total2.2 = phi i32 [ %total2.1, %lor.lhs.false64 ]
...
for.inc85.gvnsink.split:                          ; preds = %land.lhs.true73, %lor.lhs.false76, %if.else81
  %.sink4 = phi i32 [ 28, %if.else81 ], [ 29, %lor.lhs.false76 ], [ 29, %land.lhs.true73 ]
  %add82 = add nsw i32 %total2.2, %.sink4
  br label %for.inc85

for.inc85:                                        ; preds = %if.end68.thread, %for.inc85.gvnsink.split, %if.end68
  %total2.3 = phi i32 [ %add82, %for.inc85.gvnsink.split ], [ %total2.2, %if.end68 ], [ %add67, %if.end68.thread ]
...
for.end97:                                        ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 4, ptr %month2) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %month1) #3
  call void @llvm.lifetime.end.p0(i64 4, ptr %year) #3
