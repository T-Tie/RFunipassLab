...
for.cond:                                         ; preds = %for.inc, %if.else
  %0 = phi i32 [ %inc, %for.inc ], [ 0, %if.else ]
  %cmp1 = icmp slt i32 %0, 25
...
for.cond2:                                        ; preds = %for.inc13, %for.end
  %i.0 = phi i32 [ %add, %for.end ], [ %inc14, %for.inc13 ]
  %cmp3 = icmp slt i32 %i.0, %m
...
if.then10:                                        ; preds = %for.body4
  %call = call noundef i32 @_Z2ljPiii(ptr nofree noundef nonnull readonly captures(none) %a, i32 noundef %i.0, i32 noundef %m) #8
...
for.cond20:                                       ; preds = %for.inc30, %for.end15
  %i.1 = phi i32 [ %add, %for.end15 ], [ %inc31, %for.inc30 ]
  %t.0 = phi i32 [ %3, %for.end15 ], [ %t.1, %for.inc30 ]
  %cmp21 = icmp slt i32 %i.1, %m
...
for.inc30:                                        ; preds = %for.body22, %if.then26
  %t.1 = phi i32 [ %4, %if.then26 ], [ %t.0, %for.body22 ]
...
cleanup:                                          ; preds = %entry, %for.end32
  ...
  call void @llvm.lifetime.end.p0(i64 noundef 100, ptr noundef align 16 %b) #6
  ret i32 %retval.0
}
