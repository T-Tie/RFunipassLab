...
for.cond9:                                        ; preds = %for.body12, %_ZSt4sortIPiEvT_S1_.exit
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body12 ], [ 1, %_ZSt4sortIPiEvT_S1_.exit ]
  %exitcond = icmp eq i64 %indvars.iv, %wide.trip.count
  br i1 %exitcond, label %for.end18, label %for.body12
...
while.cond:                                       ; preds = %while.cond, %if.end
  %__parent.0 = phi i64 [ %div1314, %if.end ], [ %dec, %while.cond ]
...
if.end16:                                         ; preds = %if.else10, %if.else7, %if.else, %if.then
  %.sink = phi i32 [ %1, %if.then ], [ %., %if.else ], [ %0, %if.else7 ], [ %.28, %if.else10 ]
