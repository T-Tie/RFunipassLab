...
for.body:                                         ; preds = %for.cond
  ...
  %or.cond.not52 = and i1 %cmp8, %cmp10.not
  %rem11 = srem i32 %3, 400
  %cmp12 = icmp eq i32 %rem11, 0
  %or.cond51 = or i1 %cmp12, %or.cond.not52
  %smax53 = call i32 @llvm.smax.i32(i32 %a.1, i32 %b.1)
...
for.cond14:                                       ; preds = %for.body, %for.inc
  ...
  %exitcond54.not = icmp eq i32 %m.0, %smax53
  br i1 %exitcond54.not, label %for.end, label %for.body16

for.body16:                                       ; preds = %for.cond14
  %switch.tableidx = add i32 %m.0, -1
  %5 = icmp ult i32 %switch.tableidx, 12
  br i1 %5, label %switch.lookup, label %for.inc

switch.lookup:                                    ; preds = %for.body16
  %6 = zext nneg i32 %switch.tableidx to i64
  %switch.gep = getelementptr inbounds nuw [12 x i32], ptr @switch.table.main, i64 0, i64 %6
  %switch.load = load i32, ptr %switch.gep, align 4
  br label %for.inc

for.inc:                                          ; preds = %for.body16, %switch.lookup
  %.sink = phi i32 [ %switch.load, %switch.lookup ], [ 30, %for.body16 ]
  %add = add nuw nsw i32 %x.1, %.sink
  %inc = add i32 %m.0, 1
  br label %for.cond14, !llvm.loop !9

for.end:                                          ; preds = %for.cond14
  %rem20 = urem i32 %x.1, 7
...
for.cond30:                                       ; preds = %for.body, %for.inc40
  ...
  %exitcond.not = icmp eq i32 %m.1, %smax53
  br i1 %exitcond.not, label %for.end42, label %for.body32

for.body32:                                       ; preds = %for.cond30
  %switch.tableidx57 = add i32 %m.1, -1
  %7 = icmp ult i32 %switch.tableidx57, 12
  br i1 %7, label %switch.lookup56, label %for.inc40

switch.lookup56:                                  ; preds = %for.body32
  %8 = zext nneg i32 %switch.tableidx57 to i64
  %switch.gep58 = getelementptr inbounds nuw [12 x i32], ptr @switch.table.main.1, i64 0, i64 %8
  %switch.load59 = load i32, ptr %switch.gep58, align 4
  br label %for.inc40

for.inc40:                                        ; preds = %for.body32, %switch.lookup56
  %.sink55 = phi i32 [ %switch.load59, %switch.lookup56 ], [ 30, %for.body32 ]
  %add34 = add nuw nsw i32 %x.3, %.sink55
  %inc41 = add i32 %m.1, 1
  br label %for.cond30, !llvm.loop !12

for.end42:                                        ; preds = %for.cond30
  %rem43 = urem i32 %x.3, 7
