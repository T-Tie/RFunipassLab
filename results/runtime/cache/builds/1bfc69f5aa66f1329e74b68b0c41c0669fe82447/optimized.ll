...
for.cond.outer:                                   ; preds = %land.lhs.true, %entry
  %indvars.iv.ph = phi i64 [ %indvars.iv.next, %land.lhs.true ], [ %idxprom, %entry ]
  %z.0.ph = phi i32 [ %spec.select, %land.lhs.true ], [ 1, %entry ]
  br label %for.cond
...
for.body:                                         ; preds = %for.cond
  ...
  br i1 %cmp3.not, label %for.cond, label %land.lhs.true, !llvm.loop !13

land.lhs.true:                                    ; preds = %for.body
  %4 = trunc nsw i64 %indvars.iv.next to i32
  %call = tail call noundef i32 @_Z1Fi(i32 noundef %4)
  %add7 = add nsw i32 %call, 1
  %spec.select = tail call i32 @llvm.smax.i32(i32 %z.0.ph, i32 %add7)
