...
if.end17:                                         ; preds = %if.then14, %if.else15, %if.then
  %phiofops = phi i1 [ false, %if.else15 ], [ false, %if.then14 ], [ true, %if.then ]
  %s.0 = phi i32 [ 1, %if.then ], [ 0, %if.then14 ], [ -1, %if.else15 ]
  %r.0 = phi i32 [ %sub, %if.then ], [ 0, %if.then14 ], [ %sub16, %if.else15 ]
  br label %for.cond
