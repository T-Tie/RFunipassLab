...
if.end38:                                         ; preds = %land.lhs.true, %lor.lhs.false32, %if.else
  ...
  %add37 = add nsw i32 %total1.2, %.sink1
  br label %for.inc

for.inc:                                          ; preds = %if.end26, %if.end38, %if.end26.thread
  %total1.3 = phi i32 [ %add37, %if.end38 ], [ %total1.2, %if.end26 ], [ %add25, %if.end26.thread ]
...
if.end83:                                         ; preds = %land.lhs.true73, %lor.lhs.false76, %if.else81
  ...
  %add82 = add nsw i32 %total2.2, %.sink
  br label %for.inc85

for.inc85:                                        ; preds = %if.end68, %if.end83, %if.end68.thread
  %total2.3 = phi i32 [ %add82, %if.end83 ], [ %total2.2, %if.end68 ], [ %add67, %if.end68.thread ]
