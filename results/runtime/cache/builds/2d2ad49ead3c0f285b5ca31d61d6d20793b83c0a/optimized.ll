...
for.end28:                                        ; preds = %if.then23, %for.cond
  %phiofops = phi i1 [ false, %for.cond ], [ true, %if.then23 ]
  %k.1 = phi i32 [ %k.2, %if.then23 ], [ 0, %for.cond ]
  br i1 %phiofops, label %if.then30, label %if.else
...
for.cond44:                                       ; preds = %for.inc51, %for.end40
  %i.2 = phi i32 [ %add43, %for.end40 ], [ %inc52, %for.inc51 ]
...
if.end56:                                         ; preds = %if.else, %for.end53
