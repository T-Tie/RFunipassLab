...
for.end:                                          ; preds = %if.then, %for.cond2
  %phiofops = phi i1 [ false, %for.cond2 ], [ true, %if.then ]
  %flag.1 = phi i32 [ 1, %if.then ], [ 0, %for.cond2 ]
  %cmp6 = icmp eq i32 %j.0, %index.0
  br i1 %cmp6, label %land.lhs.true, label %if.end11

land.lhs.true:                                    ; preds = %for.end
  br i1 %phiofops, label %if.end11, label %if.then7
...
if.end11:                                         ; preds = %if.then7, %land.lhs.true, %for.end
  %index.1 = phi i32 [ %index.0, %land.lhs.true ], [ %inc10, %if.then7 ], [ %index.0, %for.end ]
  br label %for.inc12
