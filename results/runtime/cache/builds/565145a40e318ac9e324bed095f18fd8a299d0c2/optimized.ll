...
for.cond3:                                        ; preds = %for.cond, %for.inc
  %4 = phi i32 [ %.pre40, %for.inc ], [ %2, %for.cond ]
...
for.inc:                                          ; preds = %for.cond3
  ...
  %.pre40 = load i32, ptr %y1, align 4, !tbaa !5
...
for.cond16:                                       ; preds = %for.inc30, %for.end10
  ...
  %8 = phi i32 [ %5, %for.end10 ], [ %.pre34, %for.inc30 ]
...
for.cond19:                                       ; preds = %for.cond16, %for.inc27
  %9 = phi i32 [ %.pre39, %for.inc27 ], [ %7, %for.cond16 ]
...
for.end32:                                        ; preds = %for.cond16
  ...
  %smax31 = call i32 @llvm.smax.i32(i32 %10, i32 noundef 0)
...
for.cond37:                                       ; preds = %for.inc74, %for.end32
  %indvars.iv26 = phi i64 [ %indvars.iv.next27, %for.inc74 ], [ 0, %for.end32 ]
  %exitcond33.not = icmp eq i64 %indvars.iv26, %wide.trip.count32
  br i1 %exitcond33.not, label %for.cond77, label %for.cond40.preheader

for.cond40.preheader:                             ; preds = %for.cond37
  %14 = mul nsw i64 %indvars.iv26, %12
  %15 = mul nsw i64 %indvars.iv26, %13
  %invariant.gep46 = getelementptr i32, ptr %call36, i64 %14
  %16 = getelementptr i32, ptr %call2, i64 %15
  br label %for.cond40

for.cond40:                                       ; preds = %for.cond40.preheader, %for.inc71
  %indvars.iv19 = phi i64 [ 0, %for.cond40.preheader ], [ %indvars.iv.next20, %for.inc71 ]
  %exitcond25.not = icmp eq i64 %indvars.iv19, %wide.trip.count24
  br i1 %exitcond25.not, label %for.inc74, label %for.body42

for.body42:                                       ; preds = %for.cond40
  %gep47 = getelementptr i32, ptr %invariant.gep46, i64 %indvars.iv19
  store i32 0, ptr %gep47, align 4, !tbaa !5
  %invariant.gep = getelementptr i32, ptr %call15, i64 %indvars.iv19
  br label %for.cond47
...
for.inc68:                                        ; preds = %for.cond47
  ...
  store i32 %add63, ptr %gep47, align 4, !tbaa !5
...
for.cond77:                                       ; preds = %for.cond37, %for.inc98
  ...
  %22 = phi i32 [ %.pre35, %for.inc98 ], [ %10, %for.cond37 ]
...
for.cond80:                                       ; preds = %for.cond77, %for.inc95
  %23 = phi i32 [ %.pre36, %for.inc95 ], [ %21, %for.cond77 ]
...
if.then:                                          ; preds = %for.body82
  ...
  %.pre41 = add nsw i32 %.pre37, -1
  br label %if.end

if.end:                                           ; preds = %if.then, %for.body82
  %sub90.pre-phi = phi i32 [ %.pre41, %if.then ], [ %sub, %for.body82 ]
  %26 = phi i32 [ %.pre37, %if.then ], [ %25, %for.body82 ]
  %cmp91 = icmp eq i32 %j.3, %sub90.pre-phi
  br i1 %cmp91, label %if.then92, label %for.inc95

if.then92:                                        ; preds = %if.end
  %putchar13 = call i32 @putchar(i32 noundef 10) #9
  %.pre36.pre = load i32, ptr %y2, align 4, !tbaa !5
  br label %for.inc95

for.inc95:                                        ; preds = %if.end, %if.then92
  %.pre36 = phi i32 [ %26, %if.end ], [ %.pre36.pre, %if.then92 ]
  %inc96 = add nuw nsw i32 %j.3, 1
  br label %for.cond80, !llvm.loop !19

for.inc98:                                        ; preds = %for.cond80
  %inc99 = add nuw nsw i32 %i.3, 1
  %.pre35 = load i32, ptr %x1, align 4, !tbaa !5
