...
for.inc.gvnsink.split:                            ; preds = %land.lhs.true, %lor.lhs.false32, %if.else
  %.sink13 = phi i32 [ 28, %if.else ], [ 29, %lor.lhs.false32 ], [ 29, %land.lhs.true ]
  %add37 = add nsw i32 %total1.1, %.sink13
  br label %for.inc

for.inc:                                          ; preds = %if.end26.thread, %for.inc.gvnsink.split, %if.end26
  %total1.3 = phi i32 [ %add37, %for.inc.gvnsink.split ], [ %total1.1, %if.end26 ], [ %add25, %if.end26.thread ]
...
for.inc85.gvnsink.split:                          ; preds = %land.lhs.true73, %lor.lhs.false76, %if.else81
  %.sink12 = phi i32 [ 28, %if.else81 ], [ 29, %lor.lhs.false76 ], [ 29, %land.lhs.true73 ]
  %add82 = add nsw i32 %total2.1, %.sink12
  br label %for.inc85

for.inc85:                                        ; preds = %if.end68.thread, %for.inc85.gvnsink.split, %if.end68
  %total2.3 = phi i32 [ %add82, %for.inc85.gvnsink.split ], [ %total2.1, %if.end68 ], [ %add67, %if.end68.thread ]
