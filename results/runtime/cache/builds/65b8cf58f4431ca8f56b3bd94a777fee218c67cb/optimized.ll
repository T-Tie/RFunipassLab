...
if.else:                                          ; preds = %land.lhs.true
  ...
  %spec.select5 = add nsw i32 %a.0, %inc19
  br label %for.inc

for.inc:                                          ; preds = %if.else, %for.body, %if.then
  %a.1 = phi i32 [ %inc, %if.then ], [ %a.0, %for.body ], [ %spec.select5, %if.else ]
...
for.cond29:                                       ; preds = %for.cond26, %for.body34
  %indvars.iv7 = phi i64 [ %indvars.iv.next8, %for.body34 ], [ 0, %for.cond26 ]
  %t.1 = phi i32 [ %spec.select, %for.body34 ], [ 0, %for.cond26 ]
  %arrayidx31 = getelementptr inbounds nuw [301 x i8], ptr %str, i64 0, i64 %indvars.iv7
...
for.cond54:                                       ; preds = %for.cond26, %for.inc80
  ...
  %exitcond13.not = icmp eq i32 %4, 123
  br i1 %exitcond13.not, label %if.end83, label %for.cond57

for.cond57:                                       ; preds = %for.cond54, %for.body62
  %indvars.iv10 = phi i64 [ %indvars.iv.next11, %for.body62 ], [ 0, %for.cond54 ]
  %l.1 = phi i32 [ %spec.select4, %for.body62 ], [ 0, %for.cond54 ]
  %arrayidx59 = getelementptr inbounds nuw [301 x i8], ptr %str, i64 0, i64 %indvars.iv10
...
for.body62:                                       ; preds = %for.cond57
  ...
  %spec.select4 = add nuw nsw i32 %l.1, %inc68
