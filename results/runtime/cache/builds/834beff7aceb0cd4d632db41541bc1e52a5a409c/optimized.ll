...
for.inc41:                                        ; preds = %if.else37, %if.then34, %if.then
  %__c.addr.i21.sink56 = phi ptr [ %__c.addr.i21, %if.else37 ], [ %__c.addr.i5, %if.then34 ], [ %__c.addr.i, %if.then ]
  %__c.addr.i21.sink55 = phi ptr [ %__c.addr.i21, %if.else37 ], [ %__c.addr.i5, %if.then34 ], [ %__c.addr.i, %if.then ]
  %__c.addr.i21.sink = phi ptr [ %__c.addr.i21, %if.else37 ], [ %__c.addr.i5, %if.then34 ], [ %__c.addr.i, %if.then ]
  call void @llvm.lifetime.start.p0(i64 noundef 1, ptr noundef nonnull captures(none) dereferenceable(1) %__c.addr.i21.sink56)
  call void @outlined_ir_func_0(ptr noundef nonnull dereferenceable(1) %__c.addr.i21.sink55)
...
if.then_to_outline:
  ...
  %1 = load i64, ptr %gep47, align 8, !tbaa !31, !invariant.load !30
...
for.inc41.exitStub:                               ; preds = %if.end.i, %if.then.i
  ret void
}
