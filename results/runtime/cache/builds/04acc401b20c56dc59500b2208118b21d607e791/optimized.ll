...
for.cond:                                         ; preds = %if.end29, %entry
  %m.0 = phi i32 [ undef, %entry ], [ %m.2, %if.end29 ]
  %i.0 = phi i32 [ 0, %entry ], [ %inc31, %if.end29 ]
...
for.cond14:                                       ; preds = %if.end, %if.then
  %m.3 = phi i32 [ %i.0, %if.then ], [ %inc, %if.end ]
  %j.0 = phi i32 [ 1, %if.then ], [ %inc25, %if.end ]
...
for.end:                                          ; preds = %for.cond14, %for.body16
  %m.4 = phi i32 [ %inc, %for.body16 ], [ %m.3, %for.cond14 ]
  %cmp26 = icmp eq i32 %j.0, %conv9
  br i1 %cmp26, label %if.else, label %if.end29
...
for.cond37:                                       ; preds = %for.body39, %if.else
  %i.1 = phi i32 [ 0, %if.else ], [ %inc44, %for.body39 ]
...
for.cond48:                                       ; preds = %for.body50, %for.end45
  %i.2 = phi i32 [ %add, %for.end45 ], [ %inc55, %for.body50 ]
...
entry:
  %0 = load i32, ptr %__c1, align 4, !tbaa !17
  %1 = load i32, ptr %__c2, align 4, !tbaa !17
  %cmp = icmp eq i32 %0, %1
  ret i1 %cmp
}
