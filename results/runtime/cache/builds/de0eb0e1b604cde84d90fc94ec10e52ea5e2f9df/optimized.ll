...
for.body:                                         ; preds = %for.cond
  ...
  %or.cond18 = or i1 %cmp34, %or.cond
  %.sink2 = select i1 %or.cond18, i32 29, i32 28
...
for.cond40.preheader:                             ; preds = %for.cond2
  ...
  %smax22 = call i32 @llvm.smax.i32(i32 %5, i32 1)
  br label %for.cond40
...
if.then28:                                        ; preds = %if.end
  %add37 = add nsw i32 %.sink2, %total1.1
  br label %for.inc

for.inc:                                          ; preds = %if.end, %if.then28, %if.end26.thread
  %total1.2 = phi i32 [ %add25, %if.end26.thread ], [ %add37, %if.then28 ], [ %total1.1, %if.end ]
  %inc = add nuw i32 %4, 1
...
for.cond40:                                       ; preds = %for.cond40.preheader, %for.inc85
  ...
  %exitcond23.not = icmp eq i32 %6, %smax22
  br i1 %exitcond23.not, label %for.end87, label %for.body42
...
if.then70:                                        ; preds = %if.end58
  %add82 = add nsw i32 %.sink2, %total2.1
  br label %for.inc85

for.inc85:                                        ; preds = %if.end58, %if.then70, %if.end68.thread
  %total2.2 = phi i32 [ %add67, %if.end68.thread ], [ %add82, %if.then70 ], [ %total2.1, %if.end58 ]
