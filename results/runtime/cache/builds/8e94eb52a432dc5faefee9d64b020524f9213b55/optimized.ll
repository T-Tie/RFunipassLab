...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 240, ptr noundef %p) #8
  br label %for.cond
...
for.body:                                         ; preds = %for.cond
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 8, ptr noundef nonnull align 8 captures(none) dereferenceable(8) %x1) #8
  call void @llvm.lifetime.start.p0(i64 noundef 8, ptr noundef nonnull align 8 captures(none) dereferenceable(8) %x2) #8
...
if.then:                                          ; preds = %for.body
  ...
  %call24 = call i32 (ptr, ...) @printf(ptr noundef @.str.2, double noundef %.reload, double noundef %5)
  br label %for.inc
...
if.then30:                                        ; preds = %if.else
  call void @llvm.lifetime.start.p0(i64 noundef -1, ptr noundef nonnull align 8 captures(none) dereferenceable(8) %.loc1)
  call fastcc void @outlined_ir_func_0(ptr nofree noundef nonnull readonly align 8 captures(none) dereferenceable(8) %b, ptr nofree noundef nonnull readonly align 8 captures(none) dereferenceable(8) %a, ptr nofree noundef nonnull readonly align 8 captures(none) dereferenceable(8) %c, ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) %x1, ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) %x2, ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) %.loc1) #8
  %.reload2 = load double, ptr %.loc1, align 8
  call void @llvm.lifetime.end.p0(i64 noundef -1, ptr noundef captures(none) %.loc1)
  %call49 = call i32 (ptr, ...) @printf(ptr noundef @.str.3, double noundef %.reload2)
  br label %for.inc
...
if.end67:                                         ; preds = %if.then56, %if.then58
  ...
  %call72 = call double @sqrt(double noundef %6) #8, !tbaa !5
  %div74 = fdiv double %call72, %mul65
  %call75 = call i32 (ptr, ...) @printf(ptr noundef @.str.4, double noundef %div66, double noundef %div74, double noundef %div66, double noundef %div74)
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.end67, %if.else50, %if.then30
  %d.1 = phi double [ %d.0, %if.then ], [ %d.0, %if.then30 ], [ %div66, %if.end67 ], [ %d.0, %if.else50 ]
  call void @llvm.lifetime.end.p0(i64 noundef 8, ptr noundef captures(none) %x2) #8
  call void @llvm.lifetime.end.p0(i64 noundef 8, ptr noundef captures(none) %x1) #8
...
for.end:                                          ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 noundef 240, ptr noundef %p) #8
...
attributes #5 = { minsize mustprogress norecurse nounwind optsize uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
