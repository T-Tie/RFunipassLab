...
if.else:                                          ; preds = %while.body
  ...
  %conv8 = zext i32 %count.0 to i64
  %add = add nuw nsw i64 %conv8, 1
...
if.end31.gvnsink.split:                           ; preds = %while.body, %if.then12
  %_ZSt4cout.sink = phi ptr [ %call13, %if.then12 ], [ @_ZSt4cout, %while.body ]
  %call3 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(8) %_ZSt4cout.sink, ptr noundef nonnull %word)
  %call17 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) %word) #7
  %conv18 = trunc i64 %call17 to i32
  br label %if.end31

if.end31:                                         ; preds = %if.else19, %if.end31.gvnsink.split, %if.then21
  %count.1 = phi i32 [ %conv18, %if.end31.gvnsink.split ], [ 0, %if.then21 ], [ %conv10, %if.else19 ]
