...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 100, ptr noundef align 16 %b) #6
...
for.cond2:                                        ; preds = %for.body4, %for.end
  %i.1 = phi i32 [ 0, %for.end ], [ %inc9, %for.body4 ]
  %cmp3 = icmp slt i32 %i.1, %0
  br i1 %cmp3, label %for.body4, label %for.end10

for.body4:                                        ; preds = %for.cond2
  %call5 = call noundef i32 @_Z2ljPiii(ptr nofree noundef nonnull readonly align 16 captures(none) dereferenceable(100) %a, i32 noundef %i.1, i32 noundef %0) #7
...
for.cond11:                                       ; preds = %if.end, %for.end10
  %i.2 = phi i32 [ 0, %for.end10 ], [ %inc20, %if.end ]
  %t.0 = phi i32 [ 0, %for.end10 ], [ %t.1, %if.end ]
  %cmp12 = icmp slt i32 %i.2, %0
...
if.end:                                           ; preds = %if.then, %for.body13
  %t.1 = phi i32 [ %1, %if.then ], [ %t.0, %for.body13 ]
  %inc20 = add nsw i32 %i.2, 1
  br label %for.cond11, !llvm.loop !13

for.end21:                                        ; preds = %for.cond11
  %call22 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %t.0) #7
  %call23 = call i32 @getchar() #7
  %call24 = call i32 @getchar() #7
  %call25 = call i32 @getchar() #7
  call void @llvm.lifetime.end.p0(i64 noundef 100, ptr noundef %b) #7
...
entry:
  %b = alloca [25 x i32], align 16
  call void @llvm.lifetime.start.p0(i64 noundef 100, ptr noundef align 16 %b) #8
  %sub = sub nsw i32 %m, 1
...
for.cond2:                                        ; preds = %if.end, %for.end
  %i.1 = phi i32 [ %add, %for.end ], [ %inc14, %if.end ]
  %cmp3 = icmp slt i32 %i.1, %m
...
if.then10:                                        ; preds = %for.body4
  %call = call noundef i32 @_Z2ljPiii(ptr nofree noundef nonnull readonly captures(none) %a, i32 noundef %i.1, i32 noundef %m) #9
...
for.cond20:                                       ; preds = %if.end29, %for.end15
  %i.2 = phi i32 [ %add, %for.end15 ], [ %inc31, %if.end29 ]
  %t.0 = phi i32 [ %2, %for.end15 ], [ %t.1, %if.end29 ]
  %cmp21 = icmp slt i32 %i.2, %m
  br i1 %cmp21, label %for.body22, label %for.end32

for.body22:                                       ; preds = %for.cond20
  %idxprom23 = sext i32 %i.2 to i64
...
cleanup:                                          ; preds = %entry, %for.end32
  ...
  call void @llvm.lifetime.end.p0(i64 noundef 100, ptr noundef align 16 %b) #8
  ret i32 %retval.0
}
