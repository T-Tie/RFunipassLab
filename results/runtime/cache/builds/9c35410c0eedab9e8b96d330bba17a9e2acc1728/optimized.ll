...
for.cond9:                                        ; preds = %for.inc24, %if.end
  %i.sink = phi ptr [ %i, %if.end ], [ %j, %for.inc24 ]
...
land.lhs.true:                                    ; preds = %for.body33
  ...
  br i1 %cmp37, label %for.inc53.gvnsink.split, label %if.then48

if.then48:                                        ; preds = %land.lhs.true
  br label %for.inc53.gvnsink.split

for.inc53.gvnsink.split:                          ; preds = %land.lhs.true, %if.then48
  %.str.1.sink = phi ptr [ @.str.1, %if.then48 ], [ @.str, %land.lhs.true ]
  %b.1.ph = phi i32 [ %b.0, %if.then48 ], [ 1, %land.lhs.true ]
  %call51 = call i32 (ptr, ...) @printf(ptr noundef %.str.1.sink, i32 noundef %8)
  br label %for.inc53
