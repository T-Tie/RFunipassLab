...
entry:
  %cmp = icmp eq i32 %M, 1
  br i1 %cmp, label %cleanup, label %if.end.peel

if.end.peel:                                      ; preds = %entry
  %sub = add nsw i32 %M, -1
  %cmp1.not.peel = icmp sge i32 %N, %M
  %spec.select = zext i1 %cmp1.not.peel to i32
  %spec.select23 = select i1 %cmp1.not.peel, i32 %sub, i32 %N
  %smax = tail call i32 @llvm.smax.i32(i32 %M, i32 2)
  br label %for.cond

for.cond:                                         ; preds = %if.end.peel, %for.inc
  %i.0 = phi i32 [ %inc, %for.inc ], [ 2, %if.end.peel ]
  %num.0 = phi i32 [ %num.1, %for.inc ], [ 0, %if.end.peel ]
...
for.body:                                         ; preds = %for.cond
  ...
  %cmp6.not = icmp sgt i32 %i.0, %spec.select23
  %or.cond = or i1 %cmp6.not, %cmp5
  br i1 %or.cond, label %for.inc, label %if.then7

if.then7:                                         ; preds = %for.body
  %call8 = tail call noundef i32 @_Z3calii(i32 noundef %div, i32 noundef %i.0)
  %add9 = add nsw i32 %call8, %num.0
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then7
  %num.1 = phi i32 [ %add9, %if.then7 ], [ %num.0, %for.body ]
  %inc = add nuw i32 %i.0, 1
  br label %for.cond, !llvm.loop !5

cleanup.loopexit:                                 ; preds = %for.cond
  %0 = add nsw i32 %num.0, %spec.select
  br label %cleanup

cleanup:                                          ; preds = %entry, %cleanup.loopexit
  %accumulator.ret.tr = phi i32 [ %0, %cleanup.loopexit ], [ 0, %entry ]
