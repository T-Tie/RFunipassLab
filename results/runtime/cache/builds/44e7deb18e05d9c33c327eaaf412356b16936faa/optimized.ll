...
if.else:                                          ; preds = %if.then
  ...
  %spec.select18 = select i1 %cmp.i2, ptr %__c, ptr %__a
  br label %if.end16
...
if.else10:                                        ; preds = %if.else7
  ...
  %spec.select19 = tail call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %spec.select20 = select i1 %cmp.i4, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.else7, %if.then, %if.else10, %if.else
  %.sink13 = phi i32 [ %spec.select, %if.else ], [ %spec.select19, %if.else10 ], [ %1, %if.then ], [ %0, %if.else7 ]
  %__b.sink = phi ptr [ %spec.select18, %if.else ], [ %spec.select20, %if.else10 ], [ %__b, %if.then ], [ %__a, %if.else7 ]
  %.sink = load i32, ptr %__result, align 4, !tbaa !5
