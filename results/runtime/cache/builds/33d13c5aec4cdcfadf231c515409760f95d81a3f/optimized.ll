...
land.lhs.true:                                    ; preds = %entry
  %cmp1 = icmp sge i32 %m, %i
  br i1 %cmp1, label %return, label %if.else.thread
...
while.cond:                                       ; preds = %if.end, %if.then3
  %j.0 = phi i32 [ %i, %if.then3 ], [ %inc, %if.end ]
  %sum.0 = phi i32 [ 0, %if.then3 ], [ %sum.1, %if.end ]
  %cmp4 = icmp sle i32 %j.0, %m
...
if.then8:                                         ; preds = %land.lhs.true6
  %div = sdiv i32 %m, %j.0
  %sub = sub nsw i32 %x, 1
  %call = call noundef i32 @_Z6devideiii(i32 noundef %div, i32 noundef %sub, i32 noundef %j.0) #6
...
return:                                           ; preds = %if.else.thread, %if.else, %while.cond, %land.lhs.true
  %retval.0 = phi i32 [ 1, %land.lhs.true ], [ %sum.0, %while.cond ], [ 0, %if.else ], [ 0, %if.else.thread ]
  ret i32 %retval.0
}
...
for.cond2:                                        ; preds = %for.inc, %for.body
  %sum.0 = phi i32 [ 0, %for.body ], [ %add, %for.inc ]
  %X.0 = phi i32 [ 1, %for.body ], [ %inc, %for.inc ]
  %cmp3 = icmp sle i32 %X.0, 16
  br i1 %cmp3, label %for.inc, label %for.inc8

for.inc:                                          ; preds = %for.cond2
  %1 = load i32, ptr %a, align 4, !tbaa !8
  %call5 = call noundef i32 @_Z6devideiii(i32 noundef %1, i32 noundef %X.0, i32 noundef 1) #8
  %add = add nsw i32 %sum.0, %call5
