...
for.inc74.gvnsink.split:                          ; preds = %if.then, %if.then44, %if.else58
  %k.2 = phi i32 [ %inc18, %if.then ], [ %k.0, %if.then44 ], [ %k.0, %if.else58 ]
  %add66.sink = phi i32 [ %add, %if.else58 ], [ %add, %if.then44 ], [ %conv, %if.then ]
  %idxprom67 = sext i32 %k.2 to i64
  %arrayidx68 = getelementptr inbounds [1000 x [81 x i8]], ptr %newword, i64 0, i64 %idxprom67
  %idxprom69 = sext i32 %add66.sink to i64
...
for.inc74:                                        ; preds = %for.inc74.gvnsink.split, %if.then33
  %k.1 = phi i32 [ %k.2, %for.inc74.gvnsink.split ], [ %inc41, %if.then33 ]
  %j.1 = phi i32 [ %inc71, %for.inc74.gvnsink.split ], [ 0, %if.then33 ]
