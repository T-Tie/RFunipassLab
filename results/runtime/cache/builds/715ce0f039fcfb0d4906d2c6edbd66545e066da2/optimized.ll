...
for.cond3:                                        ; preds = %for.cond.cleanup8, %for.end
  %i.0 = phi i32 [ 1, %for.end ], [ %inc31, %for.cond.cleanup8 ]
...
for.cond6:                                        ; preds = %if.end, %for.body5
  %r.0 = phi i32 [ 0, %for.body5 ], [ %inc28, %if.end ]
  %i.1 = phi i32 [ %i.0, %for.body5 ], [ %i.2, %if.end ]
...
for.cond15:                                       ; preds = %for.body18, %if.then
  %j.0 = phi i32 [ %i.1, %if.then ], [ %add, %for.body18 ]
...
for.cond33:                                       ; preds = %if.end44, %for.end32
  %i.3 = phi i32 [ 0, %for.end32 ], [ %inc46, %if.end44 ]
...
if.end44:                                         ; preds = %for.body35, %if.else
  ...
  %call43 = call i32 (ptr, ...) @printf(ptr noundef %.str.1.sink, i32 noundef %8) #7
