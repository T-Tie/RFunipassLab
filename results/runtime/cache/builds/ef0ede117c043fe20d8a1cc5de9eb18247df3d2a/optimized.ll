...
for.cond2:                                        ; preds = %for.inc, %for.body
  ...
  %1 = phi i32 [ %inc, %for.inc ], [ 1, %for.body ]
...
for.inc:                                          ; preds = %if.end26, %if.end38, %if.end26.thread
  %total1.2 = phi i32 [ %add25, %if.end26.thread ], [ %add37, %if.end38 ], [ %total1.1, %if.end26 ]
  %inc = add nsw i32 %1, 1
...
if.end83:                                         ; preds = %land.lhs.true73, %lor.lhs.false76, %if.else81
  %.sink1 = phi i32 [ 28, %if.else81 ], [ 29, %lor.lhs.false76 ], [ 29, %land.lhs.true73 ]
  %add82 = add nsw i32 %total2.1, %.sink1
  br label %for.inc85

for.inc85:                                        ; preds = %if.end68, %if.end83, %if.end68.thread
  %total2.2 = phi i32 [ %add67, %if.end68.thread ], [ %add82, %if.end83 ], [ %total2.1, %if.end68 ]
  %inc86 = add nsw i32 %4, 1
