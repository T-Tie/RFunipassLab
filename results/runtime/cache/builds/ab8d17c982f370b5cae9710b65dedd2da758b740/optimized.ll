...
for.end23:                                        ; preds = %for.end23thread-pre-split, %if.then
  %x1.2 = phi i32 [ %j.1, %if.then ], [ %x1.0, %for.end23thread-pre-split ]
  %y1.2 = phi i32 [ %i.1, %if.then ], [ %y1.0, %for.end23thread-pre-split ]
  %cmp24 = icmp ne i32 %x1.2, 0
...
for.body35:                                       ; preds = %for.cond33
  ...
  %3 = load i32, ptr %arrayidx39, align 4, !tbaa !5, !invariant.load !13
...
for.end44:                                        ; preds = %for.end44thread-pre-split, %if.then41
  %x2.2 = phi i32 [ %j.2, %if.then41 ], [ %x2.0, %for.end44thread-pre-split ]
  %y2.2 = phi i32 [ %i.2, %if.then41 ], [ %y2.0, %for.end44thread-pre-split ]
