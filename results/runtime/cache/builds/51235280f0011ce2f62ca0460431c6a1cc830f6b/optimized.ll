...
entry:
  ...
  %smax21 = call range(i32 0, -2147483648) i32 @llvm.smax.i32(i32 %conv7, i32 noundef 0)
  %wide.trip.count22 = zext nneg i32 %smax21 to i64
  %wide.trip.count = zext nneg i32 %smax to i64
  br label %for.cond

for.cond:                                         ; preds = %for.inc20, %entry
  %indvars.iv14 = phi i64 [ %indvars.iv.next15, %for.inc20 ], [ 0, %entry ]
  %cmp23 = phi i1 [ false, %for.inc20 ], [ true, %entry ]
  %i.0 = phi i32 [ %inc21, %for.inc20 ], [ 0, %entry ]
  %exitcond23.not = icmp eq i64 %indvars.iv14, %wide.trip.count22
  br i1 %exitcond23.not, label %for.end22, label %for.cond8
...
if.then24:                                        ; preds = %if.then24.loopexit, %for.end22
  %.pre-phi = phi i64 [ %.pre, %if.then24.loopexit ], [ %wide.trip.count22, %for.end22 ]
  %i.012 = phi i32 [ %i.0, %if.then24.loopexit ], [ %smax21, %for.end22 ]
  %add = add nsw i32 %i.012, %conv
  %smax31 = call i32 @llvm.smax.i32(i32 %i.012, i32 %add)
  %2 = sub nsw i32 %smax31, %i.012
  %wide.trip.count32 = zext i32 %2 to i64
  br label %for.cond25

for.cond25:                                       ; preds = %for.inc32, %if.then24
  %indvars.iv26 = phi i64 [ %indvars.iv.next27, %for.inc32 ], [ 0, %if.then24 ]
  %indvars.iv24 = phi i64 [ %indvars.iv.next25, %for.inc32 ], [ %.pre-phi, %if.then24 ]
  %exitcond33.not = icmp eq i64 %indvars.iv26, %wide.trip.count32
  br i1 %exitcond33.not, label %if.end36, label %for.inc32

for.inc32:                                        ; preds = %for.cond25
  %arrayidx29 = getelementptr inbounds nuw [50 x i8], ptr %c, i64 0, i64 %indvars.iv26
  %3 = load i8, ptr %arrayidx29, align 1, !tbaa !5, !invariant.load !8
  %arrayidx31 = getelementptr inbounds nuw [300 x i8], ptr %a, i64 0, i64 %indvars.iv24
