...
if.end:                                           ; preds = %if.then, %for.body
  ...
  %cmp33.not = icmp ne i32 %rem32, 0
  %or.cond.not30 = and i1 %cmp31, %cmp33.not
  %rem35 = srem i32 %4, 400
  %cmp36 = icmp eq i32 %rem35, 0
  %or.cond29 = or i1 %cmp36, %or.cond.not30
...
if.then30:                                        ; preds = %for.body5
  br i1 %or.cond29, label %if.then37, label %if.else39
...
for.inc:                                          ; preds = %for.body5, %if.then18, %if.else39, %if.then37, %if.then26
  %total.1 = phi i32 [ %add, %if.then18 ], [ %add27, %if.then26 ], [ %add38, %if.then37 ], [ %add40, %if.else39 ], [ %total.0, %for.body5 ]
  %inc = add i32 %j.0, 1
...
for.end:                                          ; preds = %for.cond3
  ...
  %puts28 = call i32 @puts(ptr nonnull dereferenceable(1) %str.1.str)
