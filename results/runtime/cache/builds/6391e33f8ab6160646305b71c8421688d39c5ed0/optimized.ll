...
for.body:                                         ; preds = %for.cond
  ...
  %spec.select50 = select i1 %cmp5, i32 %1, i32 %b.0
  %a.1 = select i1 %cmp4, i32 %1, i32 %spec.select
  %b.1 = select i1 %cmp4, i32 %2, i32 %spec.select50
  ...
  %or.cond51 = or i1 %cmp12, %or.cond
  %smax60 = call i32 @llvm.smax.i32(i32 %a.1, i32 %b.1)
  br i1 %or.cond51, label %for.cond14, label %for.cond30

for.cond14:                                       ; preds = %for.body, %for.inc
  %m.0 = phi i32 [ %inc, %for.inc ], [ %a.1, %for.body ]
  %x.1 = phi i32 [ %add18, %for.inc ], [ 0, %for.body ]
  %exitcond61.not = icmp eq i32 %m.0, %smax60
  br i1 %exitcond61.not, label %for.inc53, label %NodeBlock19
...
NodeBlock:                                        ; preds = %NodeBlock3
  ...
  %switch.selectcmp54 = icmp eq i32 %m.0, 2
  %switch.select55 = select i1 %switch.selectcmp54, i32 29, i32 %switch.select
  br label %for.inc
...
for.inc:                                          ; preds = %NodeBlock, %LeafBlock, %LeafBlock5, %LeafBlock7, %LeafBlock11, %LeafBlock13, %sw.default
  %.sink45 = phi i32 [ 30, %sw.default ], [ 31, %LeafBlock13 ], [ 31, %LeafBlock11 ], [ 31, %LeafBlock7 ], [ 31, %LeafBlock5 ], [ 31, %LeafBlock ], [ %switch.select55, %NodeBlock ]
  %add18 = add nuw nsw i32 %.sink45, %x.1
  %inc = add i32 %m.0, 1
...
for.cond30:                                       ; preds = %for.body, %for.inc40
  ...
  %exitcond.not = icmp eq i32 %m.1, %smax60
...
NodeBlock25:                                      ; preds = %NodeBlock27
  %switch.selectcmp56 = icmp eq i32 %m.1, 3
  %switch.select57 = select i1 %switch.selectcmp56, i32 31, i32 30
  %switch.selectcmp58 = icmp eq i32 %m.1, 2
  %switch.select59 = select i1 %switch.selectcmp58, i32 28, i32 %switch.select57
  br label %for.inc40
...
for.inc40:                                        ; preds = %NodeBlock25, %LeafBlock21, %LeafBlock29, %LeafBlock31, %LeafBlock35, %LeafBlock37, %sw.default37
  %.sink = phi i32 [ 30, %sw.default37 ], [ 31, %LeafBlock37 ], [ 31, %LeafBlock35 ], [ 31, %LeafBlock31 ], [ 31, %LeafBlock29 ], [ 31, %LeafBlock21 ], [ %switch.select59, %NodeBlock25 ]
...
for.inc53:                                        ; preds = %for.cond30, %for.cond14
  ...
  %spec.select53 = select i1 %cmp44, ptr @.str, ptr @.str.1
  %call49 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(8) @_ZSt4cout, ptr noundef nonnull %spec.select53)
