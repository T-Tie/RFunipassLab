...
for.cond:                                         ; preds = %if.end, %entry
  %i.0 = phi i32 [ %sub, %entry ], [ %dec, %if.end ]
  %pre.0 = phi i32 [ %conv, %entry ], [ %pre.1, %if.end ]
...
for.cond3:                                        ; preds = %for.body6, %if.then
  %j.0 = phi i32 [ %add, %if.then ], [ %inc, %for.body6 ]
  %cmp4 = icmp slt i32 %j.0, %pre.0
...
if.end:                                           ; preds = %for.cond.cleanup5, %for.body
  %pre.1 = phi i32 [ %i.0, %for.cond.cleanup5 ], [ %pre.0, %for.body ]
...
for.cond14:                                       ; preds = %for.body17, %for.end12
  %j13.0 = phi i32 [ 0, %for.end12 ], [ %inc22, %for.body17 ]
