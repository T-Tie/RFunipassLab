...
entry:
  %smax = tail call i32 @llvm.smax.i32(i32 %m1, i32 1)
  br label %for.cond
...
for.cond13.preheader:                             ; preds = %for.cond
  %smax29 = tail call i32 @llvm.smax.i32(i32 %m2, i32 1)
  br label %for.cond13

for.body:                                         ; preds = %for.cond
  %0 = and i32 %i.0, 2147483645
  %or.cond = icmp eq i32 %0, 4
  %1 = icmp eq i32 %0, 9
  %or.cond2 = or i1 %or.cond, %1
  %spec.select.v = select i1 %or.cond2, i32 30, i32 31
  %spec.select = add nsw i32 %spec.select.v, %x1.0
...
if.then9:                                         ; preds = %if.then8
  %sub = add nsw i32 %spec.select, -2
  br label %for.inc

if.else:                                          ; preds = %if.then8
  %sub10 = add nsw i32 %spec.select, -3
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.else, %if.then9
  %x1.2 = phi i32 [ %sub, %if.then9 ], [ %sub10, %if.else ], [ %spec.select, %for.body ]
...
for.cond13:                                       ; preds = %for.cond13.preheader, %for.inc37
  ...
  %exitcond30.not = icmp eq i32 %j.0, %smax29
  br i1 %exitcond30.not, label %for.end39, label %for.body15

for.body15:                                       ; preds = %for.cond13
  %2 = and i32 %j.0, 2147483645
  %or.cond3 = icmp eq i32 %2, 4
  %3 = icmp eq i32 %2, 9
  %or.cond5 = or i1 %or.cond3, %3
  %spec.select28.v = select i1 %or.cond5, i32 30, i32 31
  %spec.select28 = add nsw i32 %spec.select28.v, %x2.0
...
if.then31:                                        ; preds = %if.then28
  %sub32 = add nsw i32 %spec.select28, -2
  br label %for.inc37

if.else33:                                        ; preds = %if.then28
  %sub34 = add nsw i32 %spec.select28, -3
  br label %for.inc37

for.inc37:                                        ; preds = %for.body15, %if.else33, %if.then31
  %x2.2 = phi i32 [ %sub32, %if.then31 ], [ %sub34, %if.else33 ], [ %spec.select28, %for.body15 ]
