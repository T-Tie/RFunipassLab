...
for.inc21:                                        ; preds = %for.body8, %if.then17, %if.then
  %tIndex.1 = phi i32 [ %j.0, %if.then17 ], [ %tIndex.0, %if.then ], [ %tIndex.0, %for.body8 ]
  %temp.1 = phi i32 [ %3, %if.then17 ], [ %temp.0, %if.then ], [ %temp.0, %for.body8 ]
...
if.end32:                                         ; preds = %for.end23, %if.else
  ...
  %cmp35 = icmp sgt i32 %add.sink, %ans.0
