...
lor.lhs.false.thread:                             ; preds = %if.else79
  %cmp814 = icmp eq i32 %j.4, 1
  br i1 %cmp814, label %for.inc122, label %if.else95
...
for.inc122.gvnsink.split:                         ; preds = %if.then85, %if.else107
  %sub112.sink = phi i32 [ %sub112, %if.else107 ], [ %i.2, %if.then85 ]
  %.sink = phi i32 [ %7, %if.else107 ], [ %5, %if.then85 ]
  %idxprom113 = sext i32 %sub112.sink to i64
  ...
  store i32 %.sink, ptr %arrayidx117, align 4, !tbaa !5
  br label %for.inc122

for.inc122:                                       ; preds = %lor.lhs.false.thread, %for.inc122.gvnsink.split, %if.then97, %if.else79, %lor.lhs.false, %land.lhs.true
...
for.end127:                                       ; preds = %for.cond69
  %call = call noundef i32 @_Z3cuti(i32 noundef %dec)
  %add128 = add nsw i32 %4, %call
  store i32 %add128, ptr %sum, align 4, !tbaa !5
  br label %cleanup

cleanup:                                          ; preds = %for.end65, %for.end127
  %sum.sink = phi ptr [ %sum, %for.end127 ], [ getelementptr inbounds ([100 x i32], ptr getelementptr inbounds ([100 x [100 x i32]], ptr @a, i64 0, i64 1), i64 0, i64 1), %for.end65 ]
  %8 = load i32, ptr %sum.sink, align 4, !tbaa !5
