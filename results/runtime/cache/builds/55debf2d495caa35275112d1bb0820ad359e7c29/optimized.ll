...
for.cond34:                                       ; preds = %for.inc59, %for.end33
  ...
  br i1 %cmp35, label %for.body36, label %for.end61.loopexit3
...
for.end50:                                        ; preds = %for.cond38
  %cmp55.lcssa = phi i1 [ %cmp55, %for.cond38 ]
  %x2.2 = phi i32 [ %x2.0, %for.cond38 ]
  %y2.2 = phi i32 [ %y2.0, %for.cond38 ]
  br i1 %cmp55.lcssa, label %for.end61.loopexit3, label %for.inc59
...
for.end61.loopexit:                               ; preds = %for.body40
  %i.2.lcssa6 = phi i32 [ %i.2, %for.body40 ]
  %j.2.lcssa5 = phi i32 [ %j.2, %for.body40 ]
  br label %for.end61

for.end61.loopexit3:                              ; preds = %for.cond34, %for.end50
  %x2.1.ph = phi i32 [ %x2.2, %for.end50 ], [ %x2.0, %for.cond34 ]
  %y2.1.ph = phi i32 [ %y2.2, %for.end50 ], [ %y2.0, %for.cond34 ]
  br label %for.end61

for.end61:                                        ; preds = %for.end61.loopexit3, %for.end61.loopexit
  %x2.1 = phi i32 [ %i.2.lcssa6, %for.end61.loopexit ], [ %x2.1.ph, %for.end61.loopexit3 ]
  %y2.1 = phi i32 [ %j.2.lcssa5, %for.end61.loopexit ], [ %y2.1.ph, %for.end61.loopexit3 ]
  %y1.1.neg = sub i32 0, %y1.1
  %add = add i32 %y1.1.neg, -1
  %sub63 = add i32 %add, %y2.1
  %x1.1.neg = sub i32 0, %x1.1
  %add65 = add i32 %x1.1.neg, -1
  %sub66 = add i32 %add65, %x2.1
  %mul = mul nsw i32 %sub63, %sub66
  %call67 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %mul) #5
