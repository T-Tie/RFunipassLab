...
for.cond:                                         ; preds = %if.end19, %entry
  ...
  %i.0 = phi i32 [ 0, %entry ], [ %i.2, %if.end19 ]
...
while.body:                                       ; preds = %while.cond
  br i1 %phiofops, label %if.then, label %if.else
...
if.end:                                           ; preds = %if.else, %if.then
  %count.2 = phi i32 [ %inc, %if.then ], [ %count.1, %if.else ]
  %first.2 = phi i32 [ %first.1, %if.then ], [ 1, %if.else ]
...
while.end:                                        ; preds = %if.end, %while.cond
  %i.2 = phi i32 [ %inc7, %if.end ], [ %i.1, %while.cond ]
...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %number) #12
