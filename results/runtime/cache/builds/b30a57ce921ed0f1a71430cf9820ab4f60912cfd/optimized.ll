...
entry:
  ...
  %smax29 = call range(i32 0, -2147483648) i32 @llvm.smax.i32(i32 %conv7, i32 noundef 0)
...
for.cond8:                                        ; preds = %for.cond, %for.inc
  %indvars.iv24 = phi i64 [ %indvars.iv.next25, %for.inc ], [ 0, %for.cond ]
  %indvars.iv22 = phi i64 [ %indvars.iv.next23, %for.inc ], [ %indvars.iv, %for.cond ]
  %exitcond.not = icmp eq i64 %indvars.iv24, %wide.trip.count
  br i1 %exitcond.not, label %if.then24.loopexit, label %for.body10

for.body10:                                       ; preds = %for.cond8
  %arrayidx = getelementptr inbounds nuw [300 x i8], ptr %a, i64 0, i64 %indvars.iv22
  %0 = load i8, ptr %arrayidx, align 1, !tbaa !5
  %arrayidx13 = getelementptr inbounds nuw [50 x i8], ptr %b, i64 0, i64 %indvars.iv24
...
if.then24:                                        ; preds = %if.then24.loopexit, %for.end22
  %.pre-phi = phi i64 [ %.pre, %if.then24.loopexit ], [ %wide.trip.count30, %for.end22 ]
  %i.021 = phi i32 [ %i.0, %if.then24.loopexit ], [ %smax29, %for.end22 ]
  %add = add nsw i32 %i.021, %conv
  %smax39 = call i32 @llvm.smax.i32(i32 %i.021, i32 %add)
  %2 = sub nsw i32 %smax39, %i.021
  %wide.trip.count40 = zext i32 %2 to i64
  br label %for.cond25

for.cond25:                                       ; preds = %for.inc32, %if.then24
  %indvars.iv34 = phi i64 [ %indvars.iv.next35, %for.inc32 ], [ 0, %if.then24 ]
  %indvars.iv32 = phi i64 [ %indvars.iv.next33, %for.inc32 ], [ %.pre-phi, %if.then24 ]
  %exitcond41.not = icmp eq i64 %indvars.iv34, %wide.trip.count40
  br i1 %exitcond41.not, label %if.end36, label %for.inc32
