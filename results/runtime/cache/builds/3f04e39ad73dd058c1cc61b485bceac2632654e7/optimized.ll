...
for.cond:                                         ; preds = %for.inc, %entry
  ...
  %d.0 = phi double [ undef, %entry ], [ %d.2, %for.inc ]
...
if.then:                                          ; preds = %for.body
  %fneg = fneg double %1
  %call15 = call double @sqrt(double noundef %4) #9, !tbaa !5
  %add = fadd double %fneg, %call15
  %mul = fmul double 2.000000e+00, %2
  %div = fdiv double %add, %mul
  %call21 = call double @sqrt(double noundef %4) #9, !tbaa !5
  %sub = fsub double %fneg, %call21
  %div23 = fdiv double %sub, %mul
  %call24 = call i32 (ptr, ...) @printf(ptr noundef @.str.2, double noundef %div, double noundef %div23) #8
  br label %for.inc
...
if.then30:                                        ; preds = %if.else
  ...
  %call36 = call double @sqrt(double noundef %4) #9, !tbaa !5
  %add37 = fadd double %fneg31, %call36
  %mul38 = fmul double 2.000000e+00, %2
  %div39 = fdiv double %add37, %mul38
  %call45 = call double @sqrt(double noundef %4) #9, !tbaa !5
  %call49 = call i32 (ptr, ...) @printf(ptr noundef @.str.3, double noundef %div39) #8
  br label %for.inc
...
if.end67:                                         ; preds = %if.else62, %if.then64, %if.then58
  %d.1 = phi double [ %div61, %if.then58 ], [ %div66, %if.then64 ], [ poison, %if.else62 ]
  %mul70 = fmul double %1, %1
  %neg71 = fneg double %mul70
  %5 = call double @llvm.fmuladd.f64(double %mul9, double %3, double %neg71)
  %call72 = call double @sqrt(double noundef %5) #9, !tbaa !5
  %mul73 = fmul double 2.000000e+00, %2
  %div74 = fdiv double %call72, %mul73
  %call75 = call i32 (ptr, ...) @printf(ptr noundef @.str.4, double noundef %d.1, double noundef %div74, double noundef %d.1, double noundef %div74) #8
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.end67, %if.else50, %if.then30
  %d.2 = phi double [ %d.0, %if.then ], [ %d.0, %if.then30 ], [ %d.1, %if.end67 ], [ %d.0, %if.else50 ]
