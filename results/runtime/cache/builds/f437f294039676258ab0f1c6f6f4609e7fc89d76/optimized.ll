...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 8, ptr noundef %sample) #10
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %.sink = phi i32 [ 1, %entry ], [ %inc, %for.inc ]
  %0 = load i32, ptr %n, align 4, !tbaa !5
  %cmp = icmp sle i32 %.sink, %0
...
if.then:                                          ; preds = %for.body
  store double %div, ptr %sample, align 8, !tbaa !9
  br label %for.inc

if.else:                                          ; preds = %for.body
  %3 = load double, ptr %sample, align 8, !tbaa !9
  %sub = fsub double %div, %3
  %cmp5 = fcmp ogt double %sub, 5.000000e-02
  br i1 %cmp5, label %if.end19, label %if.else9

if.else9:                                         ; preds = %if.else
  %sub11 = fsub double %3, %div
  %cmp12 = fcmp ogt double %sub11, 5.000000e-02
  br i1 %cmp12, label %if.end19, label %if.else16

if.else16:                                        ; preds = %if.else9
  br label %if.end19

if.end19:                                         ; preds = %if.else16, %if.else9, %if.else
  %.str.2.sink.sink = phi ptr [ @.str, %if.else ], [ @.str.2, %if.else16 ], [ @.str.1, %if.else9 ]
  %call17 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(272) @_ZSt4cout, ptr noundef nonnull dereferenceable(5) %.str.2.sink.sink)
...
for.inc:                                          ; preds = %if.then, %if.end19
  %inc = add nsw i32 %.sink, 1
  br label %for.cond, !llvm.loop !11

for.end:                                          ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 noundef 8, ptr noundef %sample) #10
