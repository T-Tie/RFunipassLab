...
for.cond:                                         ; preds = %for.inc, %if.end
  %4 = phi i32 [ %10, %for.inc ], [ %2, %if.end ]
  %5 = phi i32 [ %11, %for.inc ], [ %3, %if.end ]
  %6 = phi i32 [ %inc, %for.inc ], [ 3, %if.end ]
...
for.inc.sink.split:                               ; preds = %for.body, %if.then7
  %.sink = phi i32 [ %9, %if.then7 ], [ %7, %for.body ]
  %.ph11 = phi i32 [ %7, %if.then7 ], [ %9, %for.body ]
  store i32 %.sink, ptr %b, align 4, !tbaa !5
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %if.else
  %10 = phi i32 [ %8, %if.else ], [ %.sink, %for.inc.sink.split ]
  %11 = phi i32 [ %9, %if.else ], [ %.ph11, %for.inc.sink.split ]
