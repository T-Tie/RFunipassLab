...
for.cond10:                                       ; preds = %for.inc28, %for.end9
  %n3.0 = phi i32 [ undef, %for.end9 ], [ %n3.1, %for.inc28 ]
  %n2.0 = phi i32 [ -1, %for.end9 ], [ %n2.1, %for.inc28 ]
  %n1.0 = phi i32 [ -1, %for.end9 ], [ %n1.1, %for.inc28 ]
  %i.1 = phi i32 [ 0, %for.end9 ], [ %inc29, %for.inc28 ]
  %n4.0 = phi i32 [ undef, %for.end9 ], [ %n4.1, %for.inc28 ]
...
for.cond13:                                       ; preds = %for.inc25, %for.body12
  %n3.1 = phi i32 [ %n3.0, %for.body12 ], [ %n3.3, %for.inc25 ]
  %n2.1 = phi i32 [ %n2.0, %for.body12 ], [ %n2.3, %for.inc25 ]
  %n1.1 = phi i32 [ %n1.0, %for.body12 ], [ %n1.3, %for.inc25 ]
  %j.1 = phi i32 [ 0, %for.body12 ], [ %inc26, %for.inc25 ]
  %n4.1 = phi i32 [ %n4.0, %for.body12 ], [ %n4.3, %for.inc25 ]
  %cmp14 = icmp slt i32 %j.1, %0
  br i1 %cmp14, label %for.body15, label %for.inc28
...
if.then:                                          ; preds = %for.body15
  %cmp21 = icmp sge i32 %n1.1, 0
  br i1 %cmp21, label %for.inc25, label %lor.lhs.false

lor.lhs.false:                                    ; preds = %if.then
  %cmp22 = icmp sge i32 %n2.1, 0
  br i1 %cmp22, label %for.inc25, label %if.end24

if.end24:                                         ; preds = %lor.lhs.false
  br label %for.inc25

for.inc25:                                        ; preds = %for.body15, %if.then, %lor.lhs.false, %if.end24
  %n3.3 = phi i32 [ %i.1, %lor.lhs.false ], [ %i.1, %if.then ], [ %n3.1, %for.body15 ], [ %i.1, %if.end24 ]
  %n2.3 = phi i32 [ %n2.1, %lor.lhs.false ], [ %n2.1, %if.then ], [ %n2.1, %for.body15 ], [ %j.1, %if.end24 ]
  %n1.3 = phi i32 [ %n1.1, %lor.lhs.false ], [ %n1.1, %if.then ], [ %n1.1, %for.body15 ], [ %i.1, %if.end24 ]
  %n4.3 = phi i32 [ %j.1, %lor.lhs.false ], [ %j.1, %if.then ], [ %n4.1, %for.body15 ], [ %j.1, %if.end24 ]
...
for.end30:                                        ; preds = %for.cond10
  %sub = sub nsw i32 %n3.0, %n1.0
  %sub31 = sub nsw i32 %sub, 1
  %sub32 = sub nsw i32 %n4.0, %n2.0
  %sub33 = sub nsw i32 %sub32, 1
  %mul = mul nsw i32 %sub31, %sub33
  %call34 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %mul) #7
