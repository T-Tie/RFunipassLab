...
; Function Attrs: mustprogress uwtable
define linkonce_odr dso_local void @_ZSt22__move_median_to_firstIPiN9__gnu_cxx5__ops15_Iter_less_iterEEvT_S4_S4_S4_T0_(ptr noundef %__result, ptr noundef align 4 %__a, ptr noundef align 4 %__b, ptr noundef align 4 %__c) #4 comdat {
entry:
...
if.else:                                          ; preds = %if.then
  ...
  br i1 %cmp.i2, label %if.end16, label %if.else5
...
if.else10:                                        ; preds = %if.else7
  ...
  br i1 %cmp.i4, label %if.end16, label %if.else13

if.else13:                                        ; preds = %if.else10
  br label %if.end16

if.end16:                                         ; preds = %if.else10, %if.else, %if.then9, %if.else13, %if.then2, %if.else5
  %.sink27 = phi i32 [ %1, %if.else13 ], [ %0, %if.then9 ], [ %0, %if.else5 ], [ %1, %if.then2 ], [ %2, %if.else ], [ %2, %if.else10 ]
  %__b.sink = phi ptr [ %__b, %if.else13 ], [ %__a, %if.then9 ], [ %__a, %if.else5 ], [ %__b, %if.then2 ], [ %__c, %if.else ], [ %__c, %if.else10 ]
  %.sink = phi i32 [ %6, %if.else13 ], [ %5, %if.then9 ], [ %4, %if.else5 ], [ %3, %if.then2 ], [ %4, %if.else ], [ %6, %if.else10 ]
  store i32 %.sink27, ptr %__result, align 4, !tbaa !5
