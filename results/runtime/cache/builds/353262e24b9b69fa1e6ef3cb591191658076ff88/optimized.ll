...
if.end:                                           ; preds = %if.else37, %if.then34
  %.str.2.sink = phi ptr [ @.str.2, %if.else37 ], [ @.str.1, %if.then34 ]
  br label %if.end40

if.end40:                                         ; preds = %if.end, %if.then
  %.str.2.sink.sink = phi ptr [ %.str.2.sink, %if.end ], [ @.str, %if.then ]
  %call38 = call noundef nonnull align 8 dereferenceable(8) ptr @_ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc(ptr noundef nonnull align 8 dereferenceable(8) @_ZSt4cout, ptr noundef %.str.2.sink.sink)
