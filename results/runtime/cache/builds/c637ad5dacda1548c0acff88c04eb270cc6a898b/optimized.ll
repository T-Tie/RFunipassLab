...
if.end.thread:                                    ; preds = %while.body
  ...
  %cmp83 = icmp sle i32 %add, 80
  br i1 %cmp83, label %if.end19, label %if.then25
...
if.end19:                                         ; preds = %if.end.thread, %if.else14
  %add7.sink58 = phi i32 [ %add7, %if.else14 ], [ %add, %if.end.thread ]
  %call15.sink = phi ptr [ %call15, %if.else14 ], [ @_ZSt4cout, %if.end.thread ]
  ...
  %cmp20 = icmp eq i32 %add7.sink58, 80
...
if.end29:                                         ; preds = %if.end19, %if.end23.thread, %if.then25
  %sum.1 = phi i32 [ 0, %if.end23.thread ], [ %conv, %if.then25 ], [ %add7.sink58, %if.end19 ]
