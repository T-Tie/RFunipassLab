...
if.else:                                          ; preds = %if.then
  ...
  %spec.select31 = select i1 %cmp.i2, ptr %__c, ptr %__a
  br label %if.end16
...
if.else10:                                        ; preds = %if.else7
  ...
  %spec.select32 = tail call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %spec.select33 = select i1 %cmp.i4, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.else10, %if.else, %if.else7, %if.then
  %.sink = phi i32 [ %1, %if.then ], [ %0, %if.else7 ], [ %spec.select, %if.else ], [ %spec.select32, %if.else10 ]
  %__b.sink = phi ptr [ %__b, %if.then ], [ %__a, %if.else7 ], [ %spec.select31, %if.else ], [ %spec.select33, %if.else10 ]
