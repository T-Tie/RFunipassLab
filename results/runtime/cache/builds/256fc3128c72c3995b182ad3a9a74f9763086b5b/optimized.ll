; ModuleID = '<stdin>'
source_filename = "/tmp/tmp4b16r1n6.cpp"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

module asm ".globl _ZSt21ios_base_library_initv"

@.str = private unnamed_addr constant [3 x i8] c"%d\00", align 1
@.str.1 = private unnamed_addr constant [4 x i8] c"%d,\00", align 1

; Function Attrs: mustprogress norecurse uwtable
define dso_local noundef i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %result = alloca [1000 x i32], align 16
  %n = alloca i32, align 4
  %flag = alloca i32, align 4
  %k = alloca i32, align 4
  %i = alloca i32, align 4
  %index = alloca i32, align 4
  %j = alloca i32, align 4
  call void @llvm.lifetime.start.p0(i64 noundef 4000, ptr noundef align 16 %result) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %n) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %flag) #3
  store i32 0, ptr %flag, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %k) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %i) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %index) #3
  store i32 0, ptr %index, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %j) #3
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef align 4 %k)
  store i32 0, ptr %i, align 4, !tbaa !5
  br label %for.cond

for.cond:                                         ; preds = %for.inc12, %entry
  %0 = load i32, ptr %i, align 4, !tbaa !5
  %1 = load i32, ptr %k, align 4, !tbaa !5
  %cmp = icmp slt i32 %0, %1
  br i1 %cmp, label %for.body, label %for.end14

for.body:                                         ; preds = %for.cond
  %call1 = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %n)
  store i32 0, ptr %j, align 4, !tbaa !5
  br label %for.cond2

for.cond2:                                        ; preds = %for.inc, %for.body
  %2 = load i32, ptr %j, align 4, !tbaa !5
  %3 = load i32, ptr %index, align 4, !tbaa !5
  %cmp3 = icmp slt i32 %2, %3
  br i1 %cmp3, label %for.body4, label %for.end

for.body4:                                        ; preds = %for.cond2
  %4 = load i32, ptr %j, align 4, !tbaa !5
  %idxprom = sext i32 %4 to i64
  %arrayidx = getelementptr inbounds [1000 x i32], ptr %result, i64 0, i64 %idxprom
  %5 = load i32, ptr %arrayidx, align 4, !tbaa !5, !invariant.load !9
  %6 = load i32, ptr %n, align 4, !tbaa !5
  %cmp5 = icmp eq i32 %5, %6
  br i1 %cmp5, label %if.then, label %for.inc

if.then:                                          ; preds = %for.body4
  store i32 1, ptr %flag, align 4, !tbaa !5
  br label %for.end

for.inc:                                          ; preds = %for.body4
  %7 = load i32, ptr %j, align 4, !tbaa !5
  %inc = add nsw i32 %7, 1
  store i32 %inc, ptr %j, align 4, !tbaa !5
  br label %for.cond2, !llvm.loop !10

for.end:                                          ; preds = %if.then, %for.cond2
  %8 = load i32, ptr %j, align 4, !tbaa !5
  %9 = load i32, ptr %index, align 4, !tbaa !5
  %cmp6 = icmp eq i32 %8, %9
  br i1 %cmp6, label %land.lhs.true, label %for.inc12

land.lhs.true:                                    ; preds = %for.end
  %10 = load i32, ptr %flag, align 4, !tbaa !5
  %tobool = icmp ne i32 %10, 0
  br i1 %tobool, label %for.inc12, label %if.then7

if.then7:                                         ; preds = %land.lhs.true
  %11 = load i32, ptr %n, align 4, !tbaa !5
  %12 = load i32, ptr %index, align 4, !tbaa !5
  %idxprom8 = sext i32 %12 to i64
  %arrayidx9 = getelementptr inbounds [1000 x i32], ptr %result, i64 0, i64 %idxprom8
  store i32 %11, ptr %arrayidx9, align 4, !tbaa !5
  %13 = load i32, ptr %index, align 4, !tbaa !5
  %inc10 = add nsw i32 %13, 1
  store i32 %inc10, ptr %index, align 4, !tbaa !5
  br label %for.inc12

for.inc12:                                        ; preds = %for.end, %land.lhs.true, %if.then7
  store i32 0, ptr %flag, align 4, !tbaa !5
  %14 = load i32, ptr %i, align 4, !tbaa !5
  %inc13 = add nsw i32 %14, 1
  store i32 %inc13, ptr %i, align 4, !tbaa !5
  br label %for.cond, !llvm.loop !13

for.end14:                                        ; preds = %for.cond
  store i32 0, ptr %i, align 4, !tbaa !5
  br label %for.cond15

for.cond15:                                       ; preds = %for.inc21, %for.end14
  %15 = load i32, ptr %i, align 4, !tbaa !5
  %16 = load i32, ptr %index, align 4, !tbaa !5
  %sub = sub nsw i32 %16, 1
  %cmp16 = icmp slt i32 %15, %sub
  br i1 %cmp16, label %for.inc21, label %for.end23

for.inc21:                                        ; preds = %for.cond15
  %17 = load i32, ptr %i, align 4, !tbaa !5
  %idxprom18 = sext i32 %17 to i64
  %arrayidx19 = getelementptr inbounds [1000 x i32], ptr %result, i64 0, i64 %idxprom18
  %18 = load i32, ptr %arrayidx19, align 4, !tbaa !5, !invariant.load !9
  %call20 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %18)
  %19 = load i32, ptr %i, align 4, !tbaa !5
  %inc22 = add nsw i32 %19, 1
  store i32 %inc22, ptr %i, align 4, !tbaa !5
  br label %for.cond15, !llvm.loop !14

for.end23:                                        ; preds = %for.cond15
  %20 = load i32, ptr %index, align 4, !tbaa !5
  %sub24 = sub nsw i32 %20, 1
  %idxprom25 = sext i32 %sub24 to i64
  %arrayidx26 = getelementptr inbounds [1000 x i32], ptr %result, i64 0, i64 %idxprom25
  %21 = load i32, ptr %arrayidx26, align 4, !tbaa !5, !invariant.load !9
  %call27 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %21)
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %j) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %index) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %i) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %k) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %flag) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %n) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4000, ptr noundef %result) #4
  ret i32 0
}

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.start.p0(i64 immarg, ptr captures(none)) #1

declare i32 @__isoc23_scanf(ptr noundef, ...) #2

declare i32 @printf(ptr noundef, ...) #2

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.end.p0(i64 immarg, ptr captures(none)) #1

attributes #0 = { mustprogress norecurse uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
attributes #2 = { "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nofree nounwind }
attributes #4 = { nounwind }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{!"clang version 21.1.8 (https://github.com/llvm/llvm-project.git 2078da43e25a4623cab2d0d60decddf709aaea28)"}
!5 = !{!6, !6, i64 0}
!6 = !{!"int", !7, i64 0}
!7 = !{!"omnipotent char", !8, i64 0}
!8 = !{!"Simple C++ TBAA"}
!9 = !{}
!10 = distinct !{!10, !11, !12}
!11 = !{!"llvm.loop.mustprogress"}
!12 = !{!"llvm.loop.unroll.disable"}
!13 = distinct !{!13, !11, !12}
!14 = distinct !{!14, !11, !12}
