...
for.cond:                                         ; preds = %for.cond.cleanup3, %entry
  %row.0 = phi i32 [ 0, %entry ], [ %inc9, %for.cond.cleanup3 ]
...
for.cond1:                                        ; preds = %for.body4, %for.body
  %col.0 = phi i32 [ 0, %for.body ], [ %inc, %for.body4 ]
...
for.cond11:                                       ; preds = %cleanup, %for.end10
  %a.0 = phi i32 [ 0, %for.end10 ], [ %inc28, %cleanup ]
  %s.0 = phi i32 [ undef, %for.end10 ], [ %s.1, %cleanup ]
  %r.0 = phi i32 [ undef, %for.end10 ], [ %r.1, %cleanup ]
...
for.cond15:                                       ; preds = %if.else, %for.body14
  %b.0 = phi i32 [ 0, %for.body14 ], [ %inc25, %if.else ]
  %cmp16 = icmp slt i32 %b.0, %0
  br i1 %cmp16, label %for.body18, label %cleanup
...
for.cond31:                                       ; preds = %cleanup48, %for.end30
  ...
  %cmp32 = icmp sge i32 %c.0, 0
  br i1 %cmp32, label %for.body34, label %for.end53

for.body34:                                       ; preds = %for.cond31
  br label %for.cond36

for.cond36:                                       ; preds = %if.else46, %for.body34
  %d.0 = phi i32 [ %sub, %for.body34 ], [ %dec, %if.else46 ]
  %cmp37 = icmp sge i32 %d.0, 0
...
for.end53:                                        ; preds = %for.cond31
  %sub54 = sub nsw i32 %r.0, %p.0
  %sub55 = sub nsw i32 %sub54, 1
  %sub56 = sub nsw i32 %q.0, %s.0
  %sub57 = sub nsw i32 %sub56, 1
  %mul = mul nsw i32 %sub55, %sub57
  %call58 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %mul)
