...
entry:
  ...
  %smax34 = call range(i32 0, -2147483648) i32 @llvm.smax.i32(i32 %conv7, i32 noundef 0)
...
for.cond:                                         ; preds = %if.end19, %entry
  ...
  %i.0 = phi i32 [ %inc21, %if.end19 ], [ 0, %entry ]
  %cmp23 = phi i1 [ false, %if.end19 ], [ true, %entry ]
  %exitcond36.not = icmp eq i64 %indvars.iv, %wide.trip.count35
  br i1 %exitcond36.not, label %for.end22, label %for.cond8

for.cond8:                                        ; preds = %for.cond, %if.end
  %indvars.iv29 = phi i64 [ %indvars.iv.next30, %if.end ], [ 0, %for.cond ]
  %indvars.iv27 = phi i64 [ %indvars.iv.next28, %if.end ], [ %indvars.iv, %for.cond ]
  %exitcond.not = icmp eq i64 %indvars.iv29, %wide.trip.count
  br i1 %exitcond.not, label %if.then24.loopexit, label %for.body10

for.body10:                                       ; preds = %for.cond8
  %arrayidx = getelementptr inbounds nuw [300 x i8], ptr %a, i64 0, i64 %indvars.iv27
  %0 = load i8, ptr %arrayidx, align 1, !tbaa !5, !invariant.load !8
  %arrayidx13 = getelementptr inbounds nuw [50 x i8], ptr %b, i64 0, i64 %indvars.iv29
  %1 = load i8, ptr %arrayidx13, align 1, !tbaa !5, !invariant.load !8
  %cmp15.not = icmp eq i8 %0, %1
  br i1 %cmp15.not, label %if.end, label %if.end19
...
if.then24:                                        ; preds = %if.then24.loopexit, %for.end22
  %.pre-phi = phi i64 [ %.pre, %if.then24.loopexit ], [ %wide.trip.count35, %for.end22 ]
  %i.026 = phi i32 [ %i.0, %if.then24.loopexit ], [ %smax34, %for.end22 ]
  %add = add nsw i32 %i.026, %conv
  %smax44 = call i32 @llvm.smax.i32(i32 %i.026, i32 %add)
  %2 = sub nsw i32 %smax44, %i.026
  %wide.trip.count45 = zext i32 %2 to i64
  br label %for.cond25

for.cond25:                                       ; preds = %for.body27, %if.then24
  %indvars.iv39 = phi i64 [ %indvars.iv.next40, %for.body27 ], [ 0, %if.then24 ]
  %indvars.iv37 = phi i64 [ %indvars.iv.next38, %for.body27 ], [ %.pre-phi, %if.then24 ]
  %exitcond46.not = icmp eq i64 %indvars.iv39, %wide.trip.count45
  br i1 %exitcond46.not, label %if.end36, label %for.body27

for.body27:                                       ; preds = %for.cond25
  %arrayidx29 = getelementptr inbounds nuw [50 x i8], ptr %c, i64 0, i64 %indvars.iv39
  %3 = load i8, ptr %arrayidx29, align 1, !tbaa !5, !invariant.load !8
  %arrayidx31 = getelementptr inbounds nuw [300 x i8], ptr %a, i64 0, i64 %indvars.iv37
