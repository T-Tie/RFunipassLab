...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 500, ptr %ch) #5
  br label %while.cond
...
if.end:                                           ; preds = %if.then14, %if.else9
  %i.2 = phi i32 [ %inc15, %if.then14 ], [ %i.0, %if.else9 ]
  br label %if.end16

if.end16:                                         ; preds = %if.end, %if.then8
  %i.1 = phi i32 [ %inc, %if.then8 ], [ %i.2, %if.end ]
  br label %if.end17
...
for.end:                                          ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 500, ptr %ch) #5
  ret i32 0
}
