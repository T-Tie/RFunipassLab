...
while.cond:                                       ; preds = %if.end29, %entry
  ...
  %or.cond16 = select i1 %call16.i, i1 %or.cond.i, i1 false
  br i1 %or.cond16, label %if.then19.i, label %_ZStrsIcSt11char_traitsIcEERSt13basic_istreamIT_T0_ES6_PS3_.exit
...
if.end.thread:                                    ; preds = %while.body
  %cmp85 = icmp slt i32 %add, 80
  br i1 %cmp85, label %if.else14, label %if.then25
...
if.end19:                                         ; preds = %if.end, %if.else14
  ...
  %sum.16 = phi i32 [ %add7, %if.else14 ], [ %add, %if.end ]
...
if.end29:                                         ; preds = %if.end19, %if.then21, %if.then25
  ...
  %sum.3 = phi i32 [ %conv, %if.then25 ], [ %sum.16, %if.end19 ], [ 0, %if.then21 ]
