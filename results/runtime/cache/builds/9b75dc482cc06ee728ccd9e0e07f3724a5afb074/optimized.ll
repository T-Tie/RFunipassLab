...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 10000, ptr noundef nonnull %c) #5
...
LeafBlock3:                                       ; preds = %NodeBlock
  ...
  br i1 %SwitchLeaf4, label %if.then, label %for.inc

LeafBlock:                                        ; preds = %NodeBlock
  %SwitchLeaf = icmp eq i32 %conv3, 10
  br i1 %SwitchLeaf, label %for.end, label %for.inc

if.then:                                          ; preds = %LeafBlock3
  store i8 0, ptr %arrayidx2, align 1, !tbaa !5
  %inc = add nsw i32 %j.0, 1
  br label %for.inc

for.inc:                                          ; preds = %LeafBlock3, %LeafBlock, %if.then
  %i.1 = phi i32 [ -1, %if.then ], [ %i.0, %LeafBlock ], [ %i.0, %LeafBlock3 ]
  %j.1 = phi i32 [ %inc, %if.then ], [ %j.0, %LeafBlock ], [ %j.0, %LeafBlock3 ]
...
for.cond19:                                       ; preds = %for.inc26, %for.end
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.inc26 ], [ %0, %for.end ]
...
for.inc26:                                        ; preds = %for.cond19
  ...
  %call25 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, ptr noundef nonnull %arrayidx23) #5
  %indvars.iv.next = add nsw i64 %indvars.iv, -1
  br label %for.cond19, !llvm.loop !11

for.end27:                                        ; preds = %for.cond19
  %call30 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.1, ptr noundef nonnull %c) #5
  call void @llvm.lifetime.end.p0(i64 noundef 10000, ptr noundef nonnull %c) #5
  ret i32 0
}
