; ModuleID = '<stdin>'
source_filename = "/tmp/tmp5rwmbybt.cpp"
...
@dp = dso_local local_unnamed_addr global [40000 x [200 x i32]] zeroinitializer, align 16
...
for.body:                                         ; preds = %for.cond
  ...
  %call2 = call noundef i32 @_Z2mmii(i32 noundef %1, i32 noundef 2)
...
define dso_local noundef range(i32 1, -2147483648) i32 @_Z2mmii(i32 noundef %n, i32 noundef %i) local_unnamed_addr #3 {
entry:
  %idxprom = sext i32 %n to i64
  %idxprom1 = sext i32 %i to i64
  %arrayidx2 = getelementptr inbounds [40000 x [200 x i32]], ptr @dp, i64 0, i64 %idxprom, i64 %idxprom1
...
for.cond:                                         ; preds = %for.cond.preheader, %for.inc
  %sum.0 = phi i32 [ %sum.1, %for.inc ], [ 1, %for.cond.preheader ]
  %j.0 = phi i32 [ %inc, %for.inc ], [ %i, %for.cond.preheader ]
  %conv = sitofp i32 %j.0 to double
  %call.i = tail call noundef double @sqrt(double noundef %conv.i) #6, !tbaa !5
  %cmp7 = fcmp ult double %call.i, %conv
  br i1 %cmp7, label %for.cond.cleanup, label %for.body

for.cond.cleanup:                                 ; preds = %for.cond
  store i32 %sum.0, ptr %arrayidx2, align 4, !tbaa !5
  br label %cleanup
...
if.then9:                                         ; preds = %for.body
  %call10 = tail call noundef i32 @_Z2mmii(i32 noundef %div, i32 noundef %j.0)
  %add = add nuw nsw i32 %call10, %sum.0
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then9
  %sum.1 = phi i32 [ %add, %if.then9 ], [ %sum.0, %for.body ]
  %inc = add nsw i32 %j.0, 1
  br label %for.cond, !llvm.loop !14

cleanup:                                          ; preds = %entry, %for.cond.cleanup
  %retval.0 = phi i32 [ %sum.0, %for.cond.cleanup ], [ %0, %entry ]
  ret i32 %retval.0
}
...
attributes #3 = { mustprogress nofree nounwind memory(readwrite, argmem: none, inaccessiblemem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
