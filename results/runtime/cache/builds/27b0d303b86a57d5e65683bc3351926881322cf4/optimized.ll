...
tailrecurse.outer:                                ; preds = %if.then11, %entry
  ...
  %i.tr.ph = phi i32 [ %7, %if.then11 ], [ %i, %entry ]
  %0 = sext i32 %i.tr.ph to i64
  br label %tailrecurse

tailrecurse:                                      ; preds = %tailrecurse.outer, %if.else5
  %indvars.iv = phi i64 [ %0, %tailrecurse.outer ], [ %indvars.iv.next, %if.else5 ]
...
if.else5:                                         ; preds = %tailrecurse
  ...
  br i1 %cmp10.not, label %tailrecurse, label %if.then11

if.then11:                                        ; preds = %if.else5
  %6 = trunc nsw i64 %indvars.iv to i32
  %7 = trunc nsw i64 %indvars.iv.next to i32
  %call = tail call noundef i32 @_Z5afteriii(i32 noundef %7, i32 noundef %6, i32 noundef %n) #7
