...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 302, ptr nonnull %ss) #7
...
for.cond:                                         ; preds = %for.inc23, %entry
  %indvars.iv11 = phi i64 [ %indvars.iv.next12, %for.inc23 ], [ 0, %entry ]
  %s.0 = phi i32 [ %s.1, %for.inc23 ], [ 0, %entry ]
  %exitcond14.not = icmp eq i64 %indvars.iv11, 28
  br i1 %exitcond14.not, label %for.cond.cleanup, label %for.cond3.preheader

for.cond3.preheader:                              ; preds = %for.cond
  %arrayidx9 = getelementptr inbounds nuw [27 x i8], ptr @__const.main.zimu, i64 0, i64 %indvars.iv11
  br label %for.cond3
...
for.cond.cleanup5:                                ; preds = %for.cond3
  %cond = icmp eq i32 %count.1, 0
  br i1 %cond, label %if.then20, label %if.then13
...
if.then13:                                        ; preds = %for.cond.cleanup5
...
for.inc23:                                        ; preds = %if.then13, %if.then20
  %s.1 = phi i32 [ %add21, %if.then20 ], [ %s.0, %if.then13 ]
...
if.end29:                                         ; preds = %if.then27, %for.cond.cleanup
  call void @llvm.lifetime.end.p0(i64 302, ptr nonnull %ss) #7
  ret i32 0
}
