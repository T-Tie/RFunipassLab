...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 50, ptr %c) #4
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %a, ptr noundef %b, ptr noundef %c)
...
for.cond:                                         ; preds = %for.inc20, %entry
  %s.0 = phi i32 [ 0, %entry ], [ %s.2, %for.inc20 ]
...
for.end:                                          ; preds = %if.then, %for.cond8
  %phiofops = phi i1 [ true, %for.cond8 ], [ false, %if.then ]
  %s.2 = phi i32 [ 1, %if.then ], [ 0, %for.cond8 ]
  br i1 %phiofops, label %if.then18, label %if.end19
...
for.end22:                                        ; preds = %if.then18, %for.cond
  %s.1 = phi i32 [ 0, %if.then18 ], [ %s.0, %for.cond ]
...
for.cond25:                                       ; preds = %for.inc32, %if.then24
  %d.0 = phi i32 [ %i.0, %if.then24 ], [ %inc33, %for.inc32 ]
...
if.end36:                                         ; preds = %for.end35, %for.end22
  ...
  call void @llvm.lifetime.end.p0(i64 50, ptr %c) #4
