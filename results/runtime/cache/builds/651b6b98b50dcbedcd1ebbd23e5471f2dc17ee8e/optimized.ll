...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 10000, ptr noundef nonnull align 16 %c) #5
  call void @llvm.memset.p0.i64(ptr noundef nonnull writeonly align 16 dereferenceable(10000) %c, i8 noundef 0, i64 noundef 10000, i1 noundef false) #6
  br label %for.cond

for.cond:                                         ; preds = %if.end, %entry
  %i.0 = phi i32 [ 0, %entry ], [ %inc14, %if.end ]
  %j.0 = phi i32 [ 0, %entry ], [ %j.1, %if.end ]
  %call = tail call i32 @getchar() #7
  ...
  %Pivot = icmp slt i32 %sext, 536870912
  br i1 %Pivot, label %LeafBlock, label %LeafBlock14

LeafBlock14:                                      ; preds = %for.cond
  %SwitchLeaf15 = icmp eq i32 %sext, 536870912
  br i1 %SwitchLeaf15, label %if.then, label %if.end

LeafBlock:                                        ; preds = %for.cond
  %SwitchLeaf = icmp eq i32 %sext, 167772160
  br i1 %SwitchLeaf, label %for.end, label %if.end

if.then:                                          ; preds = %LeafBlock14
  store i8 0, ptr %arrayidx2, align 1, !tbaa !5
  %inc = add nsw i32 %j.0, 1
  br label %if.end

if.end:                                           ; preds = %LeafBlock14, %LeafBlock, %if.then
  %i.1 = phi i32 [ -1, %if.then ], [ %i.0, %LeafBlock ], [ %i.0, %LeafBlock14 ]
  %j.1 = phi i32 [ %inc, %if.then ], [ %j.0, %LeafBlock ], [ %j.0, %LeafBlock14 ]
...
for.cond19:                                       ; preds = %for.body21, %for.end
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body21 ], [ %0, %for.end ]
...
for.body21:                                       ; preds = %for.cond19
  ...
  %call25 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, ptr noundef nonnull %arrayidx23) #7
  %indvars.iv.next = add nsw i64 %indvars.iv, -1
  br label %for.cond19, !llvm.loop !11

for.end27:                                        ; preds = %for.cond19
  %call30 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.1, ptr noundef nonnull %c) #7
  call void @llvm.lifetime.end.p0(i64 noundef 10000, ptr noundef nonnull %c) #7
  ret i32 0
}
