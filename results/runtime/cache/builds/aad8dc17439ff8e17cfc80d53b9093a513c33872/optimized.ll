...
for.cond3:                                        ; preds = %if.end32, %for.end
  %indvars.iv24 = phi i64 [ %indvars.iv.next25, %if.end32 ], [ 1, %for.end ]
  %ans.0 = phi i32 [ %spec.select19, %if.end32 ], [ 1, %for.end ]
  %exitcond29.not = icmp eq i64 %indvars.iv24, %wide.trip.count28
  br i1 %exitcond29.not, label %for.end42, label %for.cond6.preheader

for.cond6.preheader:                              ; preds = %for.cond3
  %arrayidx12 = getelementptr inbounds nuw [25 x i32], ptr %h, i64 0, i64 %indvars.iv24
  %2 = load i32, ptr %arrayidx12, align 4, !tbaa !5, !invariant.load !12
  br label %for.cond6

for.cond6:                                        ; preds = %for.cond6.preheader, %if.end20
  %indvars.iv21 = phi i64 [ 0, %for.cond6.preheader ], [ %indvars.iv.next22, %if.end20 ]
  %temp.0 = phi i32 [ -1, %for.cond6.preheader ], [ %temp.1, %if.end20 ]
  %tIndex.0 = phi i32 [ -1, %for.cond6.preheader ], [ %tIndex.1, %if.end20 ]
  %exitcond.not = icmp eq i64 %indvars.iv21, %indvars.iv24
  br i1 %exitcond.not, label %for.end23, label %for.body8

for.body8:                                        ; preds = %for.cond6
  %arrayidx10 = getelementptr inbounds nuw [25 x i32], ptr %h, i64 0, i64 %indvars.iv21
  %3 = load i32, ptr %arrayidx10, align 4, !tbaa !5, !invariant.load !12
  %cmp13.not = icmp slt i32 %3, %2
  br i1 %cmp13.not, label %if.end20, label %if.then

if.then:                                          ; preds = %for.body8
  %arrayidx15 = getelementptr inbounds nuw [25 x i32], ptr %len, i64 0, i64 %indvars.iv21
  ...
  %5 = trunc nuw nsw i64 %indvars.iv21 to i32
  %spec.select18 = select i1 %cmp16, i32 %5, i32 %tIndex.0
  br label %if.end20

if.end20:                                         ; preds = %if.then, %for.body8
  %temp.1 = phi i32 [ %temp.0, %for.body8 ], [ %spec.select, %if.then ]
  %tIndex.1 = phi i32 [ %tIndex.0, %for.body8 ], [ %spec.select18, %if.then ]
...
if.end32:                                         ; preds = %for.end23, %if.else
  ...
  %7 = getelementptr inbounds nuw [25 x i32], ptr %len, i64 0, i64 %indvars.iv24
  store i32 %add.sink, ptr %7, align 4, !tbaa !5
  %spec.select19 = call i32 @llvm.smax.i32(i32 %add.sink, i32 %ans.0)
