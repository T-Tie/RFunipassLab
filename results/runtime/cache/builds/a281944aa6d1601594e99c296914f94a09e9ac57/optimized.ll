; ModuleID = '<stdin>'
source_filename = "/tmp/tmp25uouv25.cpp"
...
for.body:                                         ; preds = %for.cond
  %arrayidx = getelementptr inbounds nuw [50 x i32], ptr %a, i64 0, i64 %indvars.iv
  %call1 = call noundef nonnull align 8 dereferenceable(16) ptr @_ZNSirsERi(ptr noundef nonnull align 8 dereferenceable(16) @_ZSt3cin, ptr noundef nonnull align 4 dereferenceable(4) %arrayidx)
  %2 = load i32, ptr %arrayidx, align 4, !tbaa !5
  %call4 = call noundef i32 @_Z1fii(i32 noundef %2, i32 noundef 2)
...
define dso_local noundef i32 @_Z1fii(i32 noundef %n, i32 noundef %p) local_unnamed_addr #3 {
entry:
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %sum.0 = phi i32 [ 0, %entry ], [ %sum.2, %for.inc ]
  %m.0 = phi i32 [ %p, %entry ], [ %inc, %for.inc ]
  %cmp.not = icmp sgt i32 %m.0, %n
...
if.then:                                          ; preds = %for.body
  %cmp2 = icmp slt i32 %m.0, %n
  br i1 %cmp2, label %if.then3, label %if.end

if.then3:                                         ; preds = %if.then
  %call = tail call noundef i32 @_Z1fii(i32 noundef %div, i32 noundef %m.0)
  %add = add nsw i32 %call, %sum.0
  br label %if.end

if.end:                                           ; preds = %if.then3, %if.then
  %sum.1 = phi i32 [ %add, %if.then3 ], [ %sum.0, %if.then ]
  %cmp4 = icmp eq i32 %m.0, %n
  %add6 = zext i1 %cmp4 to i32
  %spec.select = add nsw i32 %sum.1, %add6
  br label %for.inc

for.inc:                                          ; preds = %if.end, %for.body
  %sum.2 = phi i32 [ %sum.0, %for.body ], [ %spec.select, %if.end ]
