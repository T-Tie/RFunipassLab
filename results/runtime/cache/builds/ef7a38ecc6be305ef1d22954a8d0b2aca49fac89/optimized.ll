...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 500, ptr %dna1) #4
  call void @llvm.lifetime.start.p0(i64 500, ptr %dna2) #4
...
for.cond:                                         ; preds = %for.inc, %if.then
  ...
  %t.0 = phi i32 [ 0, %if.then ], [ %t.4, %for.inc ]
...
if.end:                                           ; preds = %if.then53, %if.then45
  %t.3 = phi i32 [ %inc, %if.then53 ], [ %t.0, %if.then45 ]
...
if.end63:                                         ; preds = %if.end62, %lor.lhs.false40, %lor.lhs.false21
  %t.2 = phi i32 [ %t.3, %if.end62 ], [ %t.0, %lor.lhs.false40 ], [ %t.0, %lor.lhs.false21 ]
...
land.lhs.true78:                                  ; preds = %land.lhs.true73
  ...
  br i1 %cmp82, label %if.then103, label %lor.lhs.false83
...
lor.lhs.false83:                                  ; preds = %land.lhs.true78, %land.lhs.true73, %land.lhs.true68, %if.end63
  ...
  %conv86 = sext i8 %2 to i32
  %cmp87 = icmp ne i32 %conv86, 65
  br i1 %cmp87, label %land.lhs.true88, label %if.end104
...
if.then103:                                       ; preds = %land.lhs.true98, %land.lhs.true78
  br label %for.end

if.end104:                                        ; preds = %land.lhs.true98, %land.lhs.true93, %land.lhs.true88, %lor.lhs.false83
  br label %for.inc

for.inc:                                          ; preds = %if.end104, %if.then61
  %t.4 = phi i32 [ %t.3, %if.then61 ], [ %t.2, %if.end104 ]
  %inc105 = add nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !8

for.end:                                          ; preds = %if.then103, %for.cond
  %phiofops = phi i1 [ true, %for.cond ], [ false, %if.then103 ]
  %t.1 = phi i32 [ %t.2, %if.then103 ], [ %t.0, %for.cond ]
  %flag.1 = phi i32 [ 1, %if.then103 ], [ 0, %for.cond ]
  br i1 %phiofops, label %if.then107, label %if.end118

if.then107:                                       ; preds = %for.end
  %conv108 = sitofp i32 %t.1 to double
...
if.end113:                                        ; preds = %if.then111, %if.then107
  %4 = load double, ptr %a, align 8, !tbaa !11
  %cmp114 = fcmp ole double %div, %4
...
if.end119:                                        ; preds = %if.end118, %entry
  %flag.0 = phi i32 [ %flag.1, %if.end118 ], [ 0, %entry ]
...
if.end127:                                        ; preds = %if.then125, %if.end123
  ...
  call void @llvm.lifetime.end.p0(i64 500, ptr %dna2) #4
  call void @llvm.lifetime.end.p0(i64 500, ptr %dna1) #4
  ret i32 0
}
