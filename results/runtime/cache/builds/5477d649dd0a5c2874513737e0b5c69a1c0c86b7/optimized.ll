...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 800, ptr nonnull %sz) #6
...
for.cond1:                                        ; preds = %for.cond, %for.inc14
  ...
  %indvars.iv13 = phi i64 [ %indvars.iv.next14, %for.inc14 ], [ 0, %for.cond ]
  %2 = sext i32 %1 to i64
...
for.body3:                                        ; preds = %for.cond1
  ...
  %rem = urem i32 %spec.select, 7
  %cmp9 = icmp eq i32 %rem, 0
  br i1 %cmp9, label %if.then10, label %for.inc14

if.then10:                                        ; preds = %for.body3
  %arrayidx12 = getelementptr inbounds nuw [200 x i32], ptr %sz, i64 0, i64 %indvars.iv13
...
for.cond17:                                       ; preds = %for.cond1, %for.body19
  %6 = phi i32 [ %.pre19, %for.body19 ], [ %1, %for.cond1 ]
  %indvars.iv16 = phi i64 [ %indvars.iv.next17, %for.body19 ], [ 0, %for.cond1 ]
  %7 = sext i32 %6 to i64
  %cmp18 = icmp slt i64 %indvars.iv16, %7
  br i1 %cmp18, label %for.body19, label %for.end29

for.body19:                                       ; preds = %for.cond17
  %arrayidx21 = getelementptr inbounds nuw [200 x i32], ptr %sz, i64 0, i64 %indvars.iv16
...
for.end29:                                        ; preds = %for.cond17
  call void @llvm.lifetime.end.p0(i64 800, ptr nonnull %sz) #6
...
entry:
  ...
  %cmp30.not = icmp ne i32 %rem29, 0
  %or.cond29.not30 = and i1 %cmp28, %cmp30.not
  %smax = tail call i32 @llvm.smax.i32(i32 %mon, i32 1)
  %brmerge = or i1 %cmp25, %or.cond29.not30
  br label %for.cond
...
switch.early.test:                                ; preds = %for.body
  %1 = and i32 %i.0, 2147483645
  switch i32 %1, label %if.else22 [
    i32 8, label %if.then
    i32 5, label %if.then
    i32 1, label %if.then
...
for.inc:                                          ; preds = %if.then, %if.else22, %if.else33, %if.then31, %if.then20
  %result.1 = phi i32 [ %add, %if.then ], [ %add21, %if.then20 ], [ %add32, %if.then31 ], [ %add34, %if.else33 ], [ %result.0, %if.else22 ]
