; ModuleID = '<stdin>'
source_filename = "/tmp/tmpqor4pcq3.cpp"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

module asm ".globl _ZSt21ios_base_library_initv"

@.str = private unnamed_addr constant [3 x i8] c"%d\00", align 1
@.str.1 = private unnamed_addr constant [4 x i8] c"%d,\00", align 1

; Function Attrs: mustprogress norecurse uwtable
define dso_local noundef i32 @main() #0 {
entry:
  %a = alloca [10000 x i32], align 16
  %n = alloca i32, align 4
  %w = alloca i32, align 4
  %i = alloca i32, align 4
  %j = alloca i32, align 4
  %k = alloca i32, align 4
  %t = alloca i32, align 4
  %e = alloca i32, align 4
  %x = alloca i32, align 4
  %x4 = alloca i8, i32 0, align 4
  %x2 = alloca i8, i32 0, align 4
  %y = alloca i32, align 4
  %y5 = alloca i8, i32 0, align 4
  %y3 = alloca i8, i32 0, align 4
  call void @llvm.lifetime.start.p0(i64 noundef 40000, ptr noundef align 16 %a) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %n) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %w) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %i) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %j) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %k) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %t) #3
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef align 4 %e) #3
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef align 4 %w)
  store i32 0, ptr %n, align 4, !tbaa !5
  store i32 0, ptr %i, align 4, !tbaa !5
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %0 = load i32, ptr %i, align 4, !tbaa !5
  %1 = load i32, ptr %w, align 4, !tbaa !5
  %cmp = icmp slt i32 %0, %1
  br i1 %cmp, label %for.inc, label %for.end

for.inc:                                          ; preds = %for.cond
  %idxprom = sext i32 %0 to i64
  %arrayidx = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom
  %call1 = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %arrayidx)
  %inc = add nsw i32 %0, 1
  store i32 %inc, ptr %i, align 4, !tbaa !5
  br label %for.cond, !llvm.loop !9

for.end:                                          ; preds = %for.cond
  store i32 0, ptr %k, align 4, !tbaa !5
  br label %for.cond2

for.cond2:                                        ; preds = %for.inc11, %for.end
  %2 = load i32, ptr %k, align 4, !tbaa !5
  %cmp3 = icmp slt i32 %2, %1
  br i1 %cmp3, label %for.body4, label %for.end13

for.body4:                                        ; preds = %for.cond2
  %idxprom5 = sext i32 %2 to i64
  %arrayidx6 = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom5
  %3 = load i32, ptr %arrayidx6, align 4, !tbaa !5
  %rem = srem i32 %3, 2
  %cmp7 = icmp ne i32 %rem, 0
  br i1 %cmp7, label %if.then, label %if.else

if.then:                                          ; preds = %for.body4
  %4 = load i32, ptr %n, align 4, !tbaa !5
  %inc8 = add nsw i32 %4, 1
  store i32 %inc8, ptr %n, align 4, !tbaa !5
  br label %for.inc11

if.else:                                          ; preds = %for.body4
  store i32 0, ptr %arrayidx6, align 4, !tbaa !5
  br label %for.inc11

for.inc11:                                        ; preds = %if.then, %if.else
  %inc12 = add nsw i32 %2, 1
  store i32 %inc12, ptr %k, align 4, !tbaa !5
  br label %for.cond2, !llvm.loop !12

for.end13:                                        ; preds = %for.cond2
  %sub = sub nsw i32 %1, 1
  br label %for.cond14

for.cond14:                                       ; preds = %for.inc40, %for.end13
  %sub.sink = phi i32 [ %sub, %for.end13 ], [ %dec, %for.inc40 ]
  %5 = phi i32 [ %dec, %for.inc40 ], [ %sub, %for.end13 ]
  store i32 %sub.sink, ptr %e, align 4, !tbaa !5
  %cmp15 = icmp sge i32 %5, 0
  br i1 %cmp15, label %for.body16, label %for.end41

for.body16:                                       ; preds = %for.cond14
  store i32 0, ptr %j, align 4, !tbaa !5
  br label %for.cond17

for.cond17:                                       ; preds = %for.inc37, %for.body16
  %6 = load i32, ptr %j, align 4, !tbaa !5
  %7 = load i32, ptr %e, align 4, !tbaa !5
  %cmp18 = icmp slt i32 %6, %7
  br i1 %cmp18, label %for.body19, label %for.inc40

for.body19:                                       ; preds = %for.cond17
  %idxprom20 = sext i32 %6 to i64
  %arrayidx21 = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom20
  %8 = load i32, ptr %arrayidx21, align 4, !tbaa !5
  %add = add nsw i32 %6, 1
  %idxprom22 = sext i32 %add to i64
  %arrayidx23 = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom22
  %9 = load i32, ptr %arrayidx23, align 4, !tbaa !5
  %cmp24 = icmp sgt i32 %8, %9
  br i1 %cmp24, label %if.then25, label %for.inc37

if.then25:                                        ; preds = %for.body19
  store i32 %8, ptr %arrayidx23, align 4, !tbaa !5
  store i32 %9, ptr %arrayidx21, align 4, !tbaa !5
  br label %for.inc37

for.inc37:                                        ; preds = %for.body19, %if.then25
  store i32 %add, ptr %j, align 4, !tbaa !5
  br label %for.cond17, !llvm.loop !13

for.inc40:                                        ; preds = %for.cond17
  %dec = add nsw i32 %7, -1
  br label %for.cond14, !llvm.loop !14

for.end41:                                        ; preds = %for.cond14
  %10 = load i32, ptr %n, align 4, !tbaa !5
  %sub42 = sub nsw i32 %1, %10
  store i32 %sub42, ptr %t, align 4, !tbaa !5
  br label %for.cond43

for.cond43:                                       ; preds = %for.inc50, %for.end41
  %11 = load i32, ptr %t, align 4, !tbaa !5
  %12 = load i32, ptr %w, align 4, !tbaa !5
  %sub44 = sub nsw i32 %12, 1
  %cmp45 = icmp slt i32 %11, %sub44
  br i1 %cmp45, label %for.inc50, label %for.end52

for.inc50:                                        ; preds = %for.cond43
  %idxprom47 = sext i32 %11 to i64
  %arrayidx48 = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom47
  %13 = load i32, ptr %arrayidx48, align 4, !tbaa !5
  %call49 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %13)
  %inc51 = add nsw i32 %11, 1
  store i32 %inc51, ptr %t, align 4, !tbaa !5
  br label %for.cond43, !llvm.loop !15

for.end52:                                        ; preds = %for.cond43
  %idxprom54 = sext i32 %sub44 to i64
  %arrayidx55 = getelementptr inbounds [10000 x i32], ptr %a, i64 0, i64 %idxprom54
  %14 = load i32, ptr %arrayidx55, align 4, !tbaa !5
  %call56 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %14)
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %e) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %t) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %k) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %j) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %i) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %w) #4
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef %n) #4
  call void @llvm.lifetime.end.p0(i64 noundef 40000, ptr noundef %a) #4
  ret i32 0
}

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.start.p0(i64 immarg, ptr captures(none)) #1

declare i32 @__isoc23_scanf(ptr noundef, ...) #2

declare i32 @printf(ptr noundef, ...) #2

; Function Attrs: nocallback nofree nosync nounwind willreturn memory(argmem: readwrite)
declare void @llvm.lifetime.end.p0(i64 immarg, ptr captures(none)) #1

attributes #0 = { mustprogress norecurse uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nocallback nofree nosync nounwind willreturn memory(argmem: readwrite) }
attributes #2 = { "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nofree nounwind }
attributes #4 = { nounwind }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{!"clang version 21.1.8 (https://github.com/llvm/llvm-project.git 2078da43e25a4623cab2d0d60decddf709aaea28)"}
!5 = !{!6, !6, i64 0}
!6 = !{!"int", !7, i64 0}
!7 = !{!"omnipotent char", !8, i64 0}
!8 = !{!"Simple C++ TBAA"}
!9 = distinct !{!9, !10, !11}
!10 = !{!"llvm.loop.mustprogress"}
!11 = !{!"llvm.loop.unroll.disable"}
!12 = distinct !{!12, !10, !11}
!13 = distinct !{!13, !10, !11}
!14 = distinct !{!14, !10, !11}
!15 = distinct !{!15, !10, !11}
