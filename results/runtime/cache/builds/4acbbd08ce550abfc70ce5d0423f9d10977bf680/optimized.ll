...
for.cond40.preheader:                             ; preds = %for.cond2
  ...
  %smax47 = call i32 @llvm.smax.i32(i32 %4, i32 noundef 1)
  br label %for.cond40
...
if.then28:                                        ; preds = %if.end
  br i1 %or.cond43, label %if.then35, label %if.else
...
for.inc:                                          ; preds = %if.end, %if.end26.thread, %if.else, %if.then35
  %total1.3 = phi i32 [ %add36, %if.then35 ], [ %add37, %if.else ], [ %add25, %if.end26.thread ], [ %total1.1, %if.end ]
...
for.cond40:                                       ; preds = %for.cond40.preheader, %for.inc85
  ...
  %exitcond48.not = icmp eq i32 %m.0, %smax47
  br i1 %exitcond48.not, label %for.end87, label %for.body42
...
if.then70:                                        ; preds = %if.end58
  br i1 %or.cond43, label %if.then79, label %if.else81
...
for.inc85:                                        ; preds = %if.end58, %if.end68.thread, %if.else81, %if.then79
  %total2.3 = phi i32 [ %add80, %if.then79 ], [ %add82, %if.else81 ], [ %add67, %if.end68.thread ], [ %total2.1, %if.end58 ]
...
for.end87:                                        ; preds = %for.cond40
  ...
  %puts46 = call i32 @puts(ptr noundef nonnull dereferenceable(1) %str.1.str) #6
