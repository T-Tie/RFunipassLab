...
for.inc:                                          ; preds = %if.else, %if.end, %for.body
  %.str.3.sink.sink = phi ptr [ @.str.1, %for.body ], [ @.str.3, %if.else ], [ @.str.2, %if.end ]
  %s.1 = phi i32 [ %conv, %for.body ], [ %conv, %if.else ], [ %add7, %if.end ]
  %call13 = call i32 (ptr, ...) @printf(ptr noundef %.str.3.sink.sink, ptr noundef %word)
