...
for.cond:                                         ; preds = %for.inc, %entry
  ...
  %flag.0 = phi i32 [ 1, %entry ], [ %flag.19, %for.inc ]
  %a.0 = phi i32 [ 0, %entry ], [ %a.115, %for.inc ]
...
if.end.thread:                                    ; preds = %for.body
  %cmp414 = icmp sgt i32 %a.0, 0
  %spec.select25 = select i1 %cmp414, i32 0, i32 %flag.0
  br label %for.inc

for.inc:                                          ; preds = %if.end.thread, %land.lhs.true
  %a.115 = phi i32 [ %spec.select, %land.lhs.true ], [ %a.0, %if.end.thread ]
  %flag.19 = phi i32 [ %flag.0, %land.lhs.true ], [ %spec.select25, %if.end.thread ]
