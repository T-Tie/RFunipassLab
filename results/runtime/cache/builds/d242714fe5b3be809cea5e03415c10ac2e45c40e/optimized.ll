...
while.cond.i.i:                                   ; preds = %while.body.i.i, %for.inc.i
  %__last.addr.0.i.i = phi ptr [ %__i.0.i, %for.inc.i ], [ %__next.0.i.i, %while.body.i.i ]
...
if.else:                                          ; preds = %if.then
  ...
  %spec.select14 = select i1 %cmp.i2, ptr %__c, ptr %__a
  br label %if.end16
...
if.else10:                                        ; preds = %if.else7
  ...
  %spec.select15 = tail call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %spec.select16 = select i1 %cmp.i4, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.else10, %if.else, %if.else7, %if.then
  %.sink9 = phi i32 [ %1, %if.then ], [ %0, %if.else7 ], [ %spec.select, %if.else ], [ %spec.select15, %if.else10 ]
  %__b.sink = phi ptr [ %__b, %if.then ], [ %__a, %if.else7 ], [ %spec.select14, %if.else ], [ %spec.select16, %if.else10 ]
  store i32 %.sink9, ptr %__result, align 4, !tbaa !5
