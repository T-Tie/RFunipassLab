...
for.inc:                                          ; preds = %for.cond
  ...
  call void @_Z6yinshuii(i32 noundef %1, i32 noundef 2) #6
...
entry:
  ...
  br i1 %cmp.not, label %if.then6, label %for.cond.preheader

for.cond.preheader:                               ; preds = %entry
  br label %for.cond

for.cond:                                         ; preds = %for.cond.preheader, %for.inc
  %i.0 = phi i32 [ %inc, %for.inc ], [ %x, %for.cond.preheader ]
  %cmp1.not = icmp sgt i32 %i.0, %a
  br i1 %cmp1.not, label %if.end8.loopexit, label %for.body
...
if.then3:                                         ; preds = %for.body
  %div = sdiv i32 %a, %i.0
  tail call void @_Z6yinshuii(i32 noundef %div, i32 noundef %i.0) #6
  br label %for.inc
...
if.end8:                                          ; preds = %if.end8.loopexit, %if.then6
  ret void
}
