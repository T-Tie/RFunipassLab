...
for.cond3:                                        ; preds = %if.end, %while.body
  %j.0 = phi i32 [ 0, %while.body ], [ %inc23, %if.end ]
  %cmp4 = icmp slt i32 %j.0, %i.1
  br i1 %cmp4, label %for.body5, label %cleanup
...
for.cond11:                                       ; preds = %for.body14, %if.then
  %k.0 = phi i32 [ %i.1, %if.then ], [ %add, %for.body14 ]
  %sub = sub nsw i32 %4, 1
...
if.end28:                                         ; preds = %if.then26, %cleanup
  %i.2 = phi i32 [ %inc27, %if.then26 ], [ %i.1, %cleanup ]
  br label %while.cond, !llvm.loop !14

while.end:                                        ; preds = %while.cond
  br label %for.cond29

for.cond29:                                       ; preds = %for.body32, %while.end
  %i.3 = phi i32 [ 0, %while.end ], [ %inc38, %for.body32 ]
