...
for.cond13.preheader:                             ; preds = %for.cond
  %smax28 = call i32 @llvm.smax.i32(i32 noundef %m2, i32 noundef 1) #9
  br label %for.cond13
...
for.inc:                                          ; preds = %for.body, %if.end.thread, %if.else, %if.then9
  %x1.2 = phi i32 [ %sub, %if.then9 ], [ %sub10, %if.else ], [ %dec, %if.end.thread ], [ %add, %for.body ]
...
for.cond13:                                       ; preds = %for.cond13.preheader, %for.inc37
  ...
  %exitcond29.not = icmp eq i32 %j.0, %smax28
  br i1 %exitcond29.not, label %for.end39, label %for.body15
...
for.inc37:                                        ; preds = %for.body15, %if.end26.thread, %if.else33, %if.then31
  %x2.2 = phi i32 [ %sub32, %if.then31 ], [ %sub34, %if.else33 ], [ %dec25, %if.end26.thread ], [ %add16, %for.body15 ]
