...
for.cond10:                                       ; preds = %for.inc27, %for.end9
  %y1.0 = phi i32 [ undef, %for.end9 ], [ %y1.2, %for.inc27 ]
  %x1.0 = phi i32 [ 0, %for.end9 ], [ %x1.2, %for.inc27 ]
...
for.end23:                                        ; preds = %for.cond13
  %y1.2 = phi i32 [ %y1.0, %for.cond13 ]
  %x1.2 = phi i32 [ %x1.0, %for.cond13 ]
  %3 = phi i32 [ %x1.0, %for.cond13 ]
...
for.end29:                                        ; preds = %for.body15, %for.end23, %for.cond10
  %y1.1 = phi i32 [ %y1.2, %for.end23 ], [ %y1.0, %for.cond10 ], [ %i.1, %for.body15 ]
  %x1.1 = phi i32 [ %x1.2, %for.end23 ], [ %x1.0, %for.cond10 ], [ %j.1, %for.body15 ]
  br label %for.cond30

for.cond30:                                       ; preds = %for.inc48, %for.end29
  %y2.0 = phi i32 [ undef, %for.end29 ], [ %y2.2, %for.inc48 ]
  %x2.0 = phi i32 [ 0, %for.end29 ], [ %x2.2, %for.inc48 ]
  %.sink = phi i32 [ %0, %for.end29 ], [ %dec49, %for.inc48 ]
  %cmp31 = icmp sge i32 %.sink, 1
...
for.cond33:                                       ; preds = %for.inc43, %for.body32
  %.sink2 = phi i32 [ %0, %for.body32 ], [ %dec, %for.inc43 ]
  %cmp34 = icmp sge i32 %.sink2, 1
...
for.body35:                                       ; preds = %for.cond33
  ...
  %idxprom38 = sext i32 %.sink2 to i64
...
for.end44:                                        ; preds = %for.cond33
  %y2.2 = phi i32 [ %y2.0, %for.cond33 ]
  %x2.2 = phi i32 [ %x2.0, %for.cond33 ]
  %5 = phi i32 [ %x2.0, %for.cond33 ]
  %cmp45 = icmp ne i32 %5, 0
  br i1 %cmp45, label %for.end50, label %for.inc48

for.inc48:                                        ; preds = %for.end44
  %dec49 = add nsw i32 %.sink, -1
  br label %for.cond30, !llvm.loop !16

for.end50:                                        ; preds = %for.body35, %for.end44, %for.cond30
  %y2.1 = phi i32 [ %y2.2, %for.end44 ], [ %y2.0, %for.cond30 ], [ %.sink, %for.body35 ]
  %x2.1 = phi i32 [ %x2.2, %for.end44 ], [ %x2.0, %for.cond30 ], [ %.sink2, %for.body35 ]
  %.neg = sub i32 0, %x1.1
  %sub = add i32 %x2.1, -1
  %sub51 = add i32 %sub, %.neg
  %.neg3 = sub i32 0, %y1.1
  %sub52 = add i32 %y2.1, -1
  %sub53 = add i32 %sub52, %.neg3
