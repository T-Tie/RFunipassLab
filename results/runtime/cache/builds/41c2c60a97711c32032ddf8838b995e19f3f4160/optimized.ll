...
for.inc:                                          ; preds = %if.else, %if.then9, %if.end.thread, %if.end
  %x1.1 = phi i32 [ %add, %if.end ], [ %sub10, %if.else ], [ %sub, %if.then9 ], [ %dec, %if.end.thread ]
  %inc = add nsw i32 %0, 1
...
for.inc37:                                        ; preds = %if.else33, %if.then31, %if.end26.thread, %if.end26
  %x2.1 = phi i32 [ %add16, %if.end26 ], [ %sub34, %if.else33 ], [ %sub32, %if.then31 ], [ %dec25, %if.end26.thread ]
