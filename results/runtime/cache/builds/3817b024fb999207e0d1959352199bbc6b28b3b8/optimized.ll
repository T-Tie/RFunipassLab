...
for.inc:                                          ; preds = %if.end, %for.body, %if.else
  %.str.3.sink = phi ptr [ @.str.3, %if.else ], [ @.str.1, %for.body ], [ @.str.2, %if.end ]
  %s.1 = phi i32 [ %conv, %if.else ], [ %conv, %for.body ], [ %add7, %if.end ]
  %call13 = call i32 (ptr, ...) @printf(ptr noundef %.str.3.sink, ptr noundef %word) #7
  %inc = add nsw i32 %0, 1
