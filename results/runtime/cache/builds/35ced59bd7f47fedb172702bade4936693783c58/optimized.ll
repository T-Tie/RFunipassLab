...
for.body:                                         ; preds = %for.cond
  ...
  %or.cond23 = or i1 %cmp34, %or.cond
  %spec.select = select i1 %or.cond23, i32 29, i32 28
...
for.cond40.preheader:                             ; preds = %for.cond2
  ...
  %smax28 = call i32 @llvm.smax.i32(i32 %5, i32 noundef 1)
  br label %for.cond40
...
if.end:                                           ; preds = %for.body4, %if.then
  ...
  switch i32 %4, label %for.inc [
    i32 4, label %for.inc.gvnsink.split
    i32 6, label %for.inc.gvnsink.split
    i32 9, label %for.inc.gvnsink.split
    i32 11, label %for.inc.gvnsink.split
...
for.inc.gvnsink.split:                            ; preds = %if.then28, %if.end, %if.end, %if.end, %if.end
  %.sink4.sink = phi i32 [ 30, %if.end ], [ 30, %if.end ], [ 30, %if.end ], [ 30, %if.end ], [ %spec.select, %if.then28 ]
  %add37 = add nsw i32 %.sink4.sink, %total1.1
  br label %for.inc

for.inc:                                          ; preds = %if.end, %for.inc.gvnsink.split
  %total1.2 = phi i32 [ %add37, %for.inc.gvnsink.split ], [ %total1.1, %if.end ]
  %inc = add nuw i32 %4, 1
...
for.cond40:                                       ; preds = %for.cond40.preheader, %for.inc85
  ...
  %exitcond29.not = icmp eq i32 %6, %smax28
  br i1 %exitcond29.not, label %for.end87, label %for.body42
...
if.end58:                                         ; preds = %for.body42, %if.then56
  ...
  switch i32 %6, label %for.inc85 [
    i32 4, label %for.inc85.gvnsink.split
    i32 6, label %for.inc85.gvnsink.split
    i32 9, label %for.inc85.gvnsink.split
    i32 11, label %for.inc85.gvnsink.split
...
for.inc85.gvnsink.split:                          ; preds = %if.then70, %if.end58, %if.end58, %if.end58, %if.end58
  %.sink3.sink = phi i32 [ 30, %if.end58 ], [ 30, %if.end58 ], [ 30, %if.end58 ], [ 30, %if.end58 ], [ %spec.select, %if.then70 ]
  %add82 = add nsw i32 %.sink3.sink, %total2.1
  br label %for.inc85

for.inc85:                                        ; preds = %if.end58, %for.inc85.gvnsink.split
  %total2.2 = phi i32 [ %add82, %for.inc85.gvnsink.split ], [ %total2.1, %if.end58 ]
...
for.end87:                                        ; preds = %for.cond40
  ...
  %puts27 = call i32 @puts(ptr noundef nonnull dereferenceable(1) %str.1.str) #6
