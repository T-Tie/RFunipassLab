...
for.end:                                          ; preds = %for.cond1
  %idxprom6.lcssa = phi i64 [ %idxprom6, %for.cond1 ]
  %arrayidx7 = getelementptr inbounds [1000 x [7 x i32]], ptr %sz, i64 0, i64 %idxprom6.lcssa
...
for.end37:                                        ; preds = %land.lhs.true29, %for.cond
  %s.0.lcssa = phi i32 [ %s.0, %land.lhs.true29 ], [ %s.0, %for.cond ]
  br label %for.cond38

for.cond38:                                       ; preds = %for.inc73, %for.end37
  %i.1 = phi i32 [ 0, %for.end37 ], [ %inc74, %for.inc73 ]
  %cmp39 = icmp slt i32 %i.1, %s.0.lcssa
...
for.cond41:                                       ; preds = %for.inc69, %for.body40
  %j.1 = phi i32 [ 0, %for.body40 ], [ %inc70, %for.inc69 ]
  %q.1 = phi i32 [ 0, %for.body40 ], [ %sub, %for.inc69 ]
  %cmp42 = icmp slt i32 %j.1, 6
...
for.inc69:                                        ; preds = %for.cond41
  ...
  %add58 = add i32 %neg1, 12
  %reass.add2 = add i32 %add58, %9
  %reass.mul3 = mul i32 %reass.add2, 3600
  %add64 = add i32 %11, %.neg
  %add68 = add i32 %add64, %reass.mul
  %sub = add i32 %add68, %reass.mul3
  %inc70 = add nsw i32 %j.1, 1
  br label %for.cond41, !llvm.loop !13

for.inc73:                                        ; preds = %for.cond41
  %q.1.lcssa = phi i32 [ %q.1, %for.cond41 ]
  %call72 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %q.1.lcssa) #5
