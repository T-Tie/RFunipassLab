...
for.inc27:                                        ; preds = %for.cond15, %if.then
  %s.1 = phi i32 [ %b.0, %if.then ], [ %s.0, %for.cond15 ]
  %r.1 = phi i32 [ %a.0, %if.then ], [ %r.0, %for.cond15 ]
...
for.cond36:                                       ; preds = %for.inc47, %for.body34
  %d.0 = phi i32 [ %sub, %for.body34 ], [ %dec, %for.inc47 ]
...
for.inc50:                                        ; preds = %for.cond36, %if.then45
  %q.1 = phi i32 [ %d.0, %if.then45 ], [ %q.0, %for.cond36 ]
  %p.1 = phi i32 [ %c.0, %if.then45 ], [ %p.0, %for.cond36 ]
