...
for.inc:                                          ; preds = %if.else, %for.body, %if.then
  %a.1 = phi i32 [ %inc, %if.then ], [ %a.0, %for.body ], [ %spec.select10, %if.else ]
...
for.cond54:                                       ; preds = %for.cond26, %for.inc80
  ...
  %exitcond18.not = icmp eq i32 %i.2, 123
...
for.cond57:                                       ; preds = %for.cond54, %for.body62
  ...
  %l.1 = phi i32 [ %spec.select9, %for.body62 ], [ 0, %for.cond54 ]
...
for.body62:                                       ; preds = %for.cond57
  ...
  %spec.select9 = add nuw nsw i32 %l.1, %inc68
