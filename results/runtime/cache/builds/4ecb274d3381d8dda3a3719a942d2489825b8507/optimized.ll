...
for.inc8:                                         ; preds = %for.body4, %if.then
  %z.1 = phi i32 [ %call5, %if.then ], [ %z.0, %for.body4 ]
  %inc9 = add nsw i32 %i.1, 1
  br label %for.cond2, !llvm.loop !12

for.end10:                                        ; preds = %for.cond2
  %z.0.lcssa = phi i32 [ %z.0, %for.cond2 ]
  %call11 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %z.0.lcssa) #5
  ret i32 0
}

declare i32 @__isoc23_scanf(ptr noundef, ...) local_unnamed_addr #1

; Function Attrs: mustprogress nofree nosync nounwind willreturn memory(read) uwtable
...
for.inc:                                          ; preds = %for.body, %if.then
  %z.1 = phi i32 [ %add7, %if.then ], [ %z.0, %for.body ]
  br label %for.cond, !llvm.loop !13

for.end:                                          ; preds = %for.cond
  %z.0.lcssa = phi i32 [ %z.0, %for.cond ]
  ret i32 %z.0.lcssa
}
...
attributes #2 = { mustprogress nofree nosync nounwind willreturn memory(read) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
