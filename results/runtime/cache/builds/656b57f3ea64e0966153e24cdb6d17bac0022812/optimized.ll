; ModuleID = '<stdin>'
source_filename = "/tmp/tmpni97vw_o.cpp"
...
define dso_local noundef i32 @_Z6factorii(i32 noundef %a, i32 noundef %min) local_unnamed_addr #0 {
entry:
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %count.0 = phi i32 [ 0, %entry ], [ %count.2, %for.inc ]
  %i.0 = phi i32 [ %min, %entry ], [ %inc, %for.inc ]
...
for.body:                                         ; preds = %for.cond
  ...
  %spec.select = add nsw i32 %count.0, %add
  %rem = srem i32 %a, %i.0
  %div = sdiv i32 %a, %i.0
...
if.then3:                                         ; preds = %for.body
  %call = tail call noundef i32 @_Z6factorii(i32 noundef %div, i32 noundef %i.0)
  %add4 = add nsw i32 %call, %spec.select
  br label %for.inc

for.inc:                                          ; preds = %for.body, %if.then3
  %count.2 = phi i32 [ %add4, %if.then3 ], [ %spec.select, %for.body ]
