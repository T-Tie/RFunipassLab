...
entry:
  %smax = call i32 @llvm.smax.i32(i32 noundef %m1, i32 noundef 1) #9
  br label %for.cond
...
for.cond13.preheader:                             ; preds = %for.cond
  %smax27 = call i32 @llvm.smax.i32(i32 noundef %m2, i32 noundef 1) #9
  br label %for.cond13
...
for.inc:                                          ; preds = %for.body, %if.then8, %if.end.thread
  %x1.2 = phi i32 [ %sub10, %if.then8 ], [ %dec, %if.end.thread ], [ %add, %for.body ]
...
if.then28:                                        ; preds = %for.body15
  ...
  %spec.select25.neg = select i1 %tobool30.not, i32 -3, i32 -2
  %sub34 = add nsw i32 %spec.select25.neg, %add16
  br label %for.inc37

for.inc37:                                        ; preds = %for.body15, %if.then28, %if.end26.thread
  %x2.2 = phi i32 [ %sub34, %if.then28 ], [ %dec25, %if.end26.thread ], [ %add16, %for.body15 ]
