...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 20000, ptr %words) #7
  br label %for.cond
...
for.cond17:                                       ; preds = %for.body19, %if.then
  %.sink2 = phi i32 [ %j.0, %if.then ], [ %add16, %for.body19 ]
...
for.cond36:                                       ; preds = %for.body38, %if.then30
  %.sink1 = phi i32 [ %j.0, %if.then30 ], [ %add35, %for.body38 ]
...
for.inc55:                                        ; preds = %if.end54, %for.end46, %for.end27
  %j.1 = phi i32 [ %add16, %for.end27 ], [ %add35, %for.end46 ], [ %j.0, %if.end54 ]
...
for.cond63:                                       ; preds = %for.body65, %for.end57
  %.sink = phi i32 [ %j.0, %for.end57 ], [ %add62, %for.body65 ]
...
for.end73:                                        ; preds = %for.cond63
  call void @llvm.lifetime.end.p0(i64 20000, ptr %words) #7
...
for.inc25:                                        ; preds = %for.body19_to_outline
  br label %for.cond17.exitStub, !llvm.loop !13

for.cond17.exitStub:                              ; preds = %for.inc25
  ret void
}
