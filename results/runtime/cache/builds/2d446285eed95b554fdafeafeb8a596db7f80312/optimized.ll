...
for.inc.gvnsink.split:                            ; preds = %if.then8, %if.else
  %.sink15 = phi i32 [ 3, %if.else ], [ 2, %if.then8 ]
  %sub10 = sub nsw i32 %add, %.sink15
  br label %for.inc

for.inc:                                          ; preds = %if.end.thread, %for.inc.gvnsink.split, %if.end
  %x1.2 = phi i32 [ %sub10, %for.inc.gvnsink.split ], [ %add, %if.end ], [ %dec, %if.end.thread ]
  %inc = add nsw i32 %i.0, 1
...
for.inc37.gvnsink.split:                          ; preds = %if.then28, %if.else33
  %.sink14 = phi i32 [ 3, %if.else33 ], [ 2, %if.then28 ]
  %sub34 = sub nsw i32 %add16, %.sink14
  br label %for.inc37

for.inc37:                                        ; preds = %if.end26.thread, %for.inc37.gvnsink.split, %if.end26
  %x2.2 = phi i32 [ %sub34, %for.inc37.gvnsink.split ], [ %add16, %if.end26 ], [ %dec25, %if.end26.thread ]
