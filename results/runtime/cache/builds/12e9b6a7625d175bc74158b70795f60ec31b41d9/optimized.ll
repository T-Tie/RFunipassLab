...
if.else:                                          ; preds = %if.then
  ...
  %. = call i32 @llvm.smax.i32(i32 %0, i32 %2)
  %__c.__a = select i1 %cmp.i2, ptr %__c, ptr %__a
  br label %if.end16
...
if.else10:                                        ; preds = %if.else7
  ...
  %.27 = call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %__c.__b = select i1 %cmp.i4, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.else10, %if.else7, %if.else, %if.then
  %.sink = phi i32 [ %1, %if.then ], [ %., %if.else ], [ %0, %if.else7 ], [ %.27, %if.else10 ]
  %__a.sink = phi ptr [ %__b, %if.then ], [ %__c.__a, %if.else ], [ %__a, %if.else7 ], [ %__c.__b, %if.else10 ]
  store i32 %.sink, ptr %__result, align 4, !tbaa !5
  store i32 %3, ptr %__a.sink, align 4, !tbaa !5
  ret void
}
