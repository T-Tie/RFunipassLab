...
for.cond4:                                        ; preds = %for.end38, %for.end
  %i.1 = phi i32 [ 0, %for.end ], [ %add, %for.end38 ]
  %w.0 = phi i32 [ undef, %for.end ], [ %w.1, %for.end38 ]
...
for.cond7:                                        ; preds = %if.end35, %for.body6
  %j.0 = phi i32 [ %add, %for.body6 ], [ %inc37, %if.end35 ]
...
for.cond10:                                       ; preds = %if.else, %for.body9
  %k.0 = phi i32 [ %i.1, %for.body9 ], [ %inc24, %if.else ]
  %j.1 = phi i32 [ %j.0, %for.body9 ], [ %inc22, %if.else ]
  %flag.0 = phi i32 [ 0, %for.body9 ], [ %inc21, %if.else ]
...
for.body13:                                       ; preds = %for.cond10
  ...
  br i1 %cmp20, label %for.end25, label %if.else
...
for.cond42:                                       ; preds = %if.end51, %for.end41
  %i.2 = phi i32 [ 0, %for.end41 ], [ %inc53, %if.end51 ]
  %max.0 = phi i32 [ 0, %for.end41 ], [ %max.1, %if.end51 ]
...
for.cond60:                                       ; preds = %if.end79, %if.else58
  %i.3 = phi i32 [ 0, %if.else58 ], [ %inc81, %if.end79 ]
...
for.cond67:                                       ; preds = %for.body70, %if.then66
  %j.3 = phi i32 [ %i.3, %if.then66 ], [ %inc76, %for.body70 ]
