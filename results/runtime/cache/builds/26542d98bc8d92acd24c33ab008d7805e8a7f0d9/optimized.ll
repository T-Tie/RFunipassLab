...
for.inc74.gvnsink.split:                          ; preds = %if.else42, %if.then
  ...
  %idxprom45.sink = phi i64 [ %idxprom19, %if.then ], [ %idxprom77, %if.else42 ]
  %k.1.ph = phi i32 [ %inc18, %if.then ], [ %k.0, %if.else42 ]
  %call25 = call ptr @strcpy(ptr noundef nonnull dereferenceable(1) %arrayidx20.sink, ptr noundef nonnull dereferenceable(1) %arrayidx6) #13
...
for.end76:                                        ; preds = %for.cond2
  ...
  %4 = add i32 %smax17, 1
  %wide.trip.count18 = zext i32 %4 to i64
  br label %for.cond86
...
for.body88:                                       ; preds = %for.cond86
  ...
  %arrayidx103 = getelementptr inbounds nuw [1000 x [81 x i8]], ptr %newword, i64 0, i64 %indvars.iv13, i64 %5
  br label %for.cond94
