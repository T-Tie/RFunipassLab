...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 4016016, ptr %shuzu) #5
...
for.cond13:                                       ; preds = %for.inc53, %for.body12
  ...
  %y1.1 = phi i32 [ %y1.0, %for.body12 ], [ %y1.2, %for.inc53 ]
  %x1.1 = phi i32 [ %x1.0, %for.body12 ], [ %x1.2, %for.inc53 ]
...
if.end:                                           ; preds = %if.then, %land.lhs.true26, %land.lhs.true, %for.body15
  %y1.2 = phi i32 [ %j.1, %if.then ], [ %y1.1, %land.lhs.true26 ], [ %y1.1, %land.lhs.true ], [ %y1.1, %for.body15 ]
  %x1.2 = phi i32 [ %i.1, %if.then ], [ %x1.1, %land.lhs.true26 ], [ %x1.1, %land.lhs.true ], [ %x1.1, %for.body15 ]
  br i1 %cmp20, label %land.lhs.true38, label %for.inc53
...
for.inc53:                                        ; preds = %if.end, %land.lhs.true38, %land.lhs.true44, %if.then51
  %x2.2 = phi i32 [ %i.1, %if.then51 ], [ %x2.1, %land.lhs.true44 ], [ %x2.1, %land.lhs.true38 ], [ %x2.1, %if.end ]
  %y2.2 = phi i32 [ %j.1, %if.then51 ], [ %y2.1, %land.lhs.true44 ], [ %y2.1, %land.lhs.true38 ], [ %y2.1, %if.end ]
...
for.end58:                                        ; preds = %for.cond10
  ...
  %sub61 = add i32 %y1.0.neg, 1
  %add62 = add i32 %sub61, %y2.0
  %mul = mul nsw i32 %add62, %add60
  %add68 = add i32 %x2.0, 2
  %sub63 = add i32 %add68, %y1.0.neg
  %add64 = add i32 %sub63, %x1.0.neg
  %reass.add = add i32 %add64, %y2.0
  ...
  call void @llvm.lifetime.end.p0(i64 4016016, ptr %shuzu) #5
  ret i32 0
}
