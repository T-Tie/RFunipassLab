...
for.inc:                                          ; preds = %if.then18, %if.else28, %if.else39, %if.then37, %if.then26
  %total.1 = phi i32 [ %add, %if.then18 ], [ %add27, %if.then26 ], [ %add38, %if.then37 ], [ %add40, %if.else39 ], [ %total.0, %if.else28 ]
  %inc = add nsw i32 %4, 1
