...
for.body:                                         ; preds = %for.cond
  ...
  %call3 = call i64 @strlen(ptr noundef %word) #6
  %conv = trunc i64 %call3 to i32
  %cmp4 = icmp eq i32 %.sink, 1
  br i1 %cmp4, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  store i32 %conv, ptr %s, align 4, !tbaa !5
  %call6 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, ptr noundef %word) #7
  br label %for.inc
...
if.end:                                           ; preds = %for.body
  ...
  store i32 %add7, ptr %s, align 4, !tbaa !5
  %cmp8 = icmp sle i32 %add7, 80
  br i1 %cmp8, label %if.then9, label %if.else

if.then9:                                         ; preds = %if.end
  %call11 = call i32 (ptr, ...) @printf(ptr noundef @.str.2, ptr noundef %word) #7
  br label %for.inc

if.else:                                          ; preds = %if.end
  %call13 = call i32 (ptr, ...) @printf(ptr noundef @.str.3, ptr noundef %word) #7
  store i32 %conv, ptr %s, align 4, !tbaa !5
  br label %for.inc
...
for.end:                                          ; preds = %for.cond
  ...
  call void @llvm.lifetime.end.p0(i64 noundef 41, ptr noundef %word) #7
  ret i32 0
}
