...
LeafBlock3:                                       ; preds = %NodeBlock
  ...
  br i1 %SwitchLeaf4, label %if.else9, label %if.end17

LeafBlock:                                        ; preds = %NodeBlock
  %SwitchLeaf = icmp eq i32 %conv3, 10
  br i1 %SwitchLeaf, label %for.cond.preheader, label %if.end17

for.cond.preheader:                               ; preds = %LeafBlock
  br label %for.cond

if.else9:                                         ; preds = %LeafBlock3
...
if.end17:                                         ; preds = %LeafBlock3, %LeafBlock, %if.else9
  %inc15.sink = phi i32 [ %inc15, %if.else9 ], [ 1, %LeafBlock ], [ 1, %LeafBlock3 ]
