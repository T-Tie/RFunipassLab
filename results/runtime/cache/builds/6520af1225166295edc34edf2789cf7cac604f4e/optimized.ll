...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 20000, ptr nonnull %words) #6
  br label %for.cond
...
for.cond2:                                        ; preds = %for.cond, %for.inc55
  ...
  %indvars.iv8 = phi i64 [ %indvars.iv.next9, %for.inc55 ], [ 0, %for.cond ]
  %j.0 = phi i32 [ %j.2, %for.inc55 ], [ 0, %for.cond ]
  %csum.0 = phi i64 [ %csum.1, %for.inc55 ], [ 0, %for.cond ]
  %3 = sext i32 %2 to i64
  %cmp3 = icmp slt i64 %indvars.iv8, %3
  br i1 %cmp3, label %for.body4, label %for.end57

for.body4:                                        ; preds = %for.cond2
  %sext = shl i64 %csum.0, 32
  %conv = ashr exact i64 %sext, 32
  %arrayidx6 = getelementptr inbounds nuw [1000 x [20 x i8]], ptr %words, i64 0, i64 %indvars.iv8
  %call8 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) %arrayidx6) #7
...
for.cond17:                                       ; preds = %for.inc25, %if.then
  %indvars.iv.next6.sink = phi i64 [ %indvars.iv.next6, %for.inc25 ], [ %idxprom12, %if.then ]
  %call20.sink = phi ptr [ %call20, %for.inc25 ], [ @_ZSt4cout, %if.then ]
  %arrayidx22 = getelementptr inbounds [1000 x [20 x i8]], ptr %words, i64 0, i64 %indvars.iv.next6.sink
  ...
  %indvars.iv.next6 = add nsw i64 %indvars.iv.next6.sink, 1
  %cmp18.not.not = icmp slt i64 %indvars.iv.next6.sink, %indvars.iv8
...
for.cond36:                                       ; preds = %for.inc44, %if.then30
  %indvars.iv.next3.sink = phi i64 [ %indvars.iv.next3, %for.inc44 ], [ %idxprom31, %if.then30 ]
  %call39.sink = phi ptr [ %call39, %for.inc44 ], [ @_ZSt4cout, %if.then30 ]
  %arrayidx41 = getelementptr inbounds [1000 x [20 x i8]], ptr %words, i64 0, i64 %indvars.iv.next3.sink
  ...
  %indvars.iv.next3 = add nsw i64 %indvars.iv.next3.sink, 1
  %cmp37 = icmp slt i64 %indvars.iv.next3, %indvars.iv8
...
for.end46:                                        ; preds = %for.cond36
  %call50 = call i64 @strlen(ptr noundef nonnull dereferenceable(1) %arrayidx6) #7
  %add51 = add i64 %call50, 1
  br label %for.inc55.sink.split

for.inc55.sink.split:                             ; preds = %for.cond17, %for.end46
  %j.2.ph.in = phi i64 [ %indvars.iv.next3, %for.end46 ], [ %indvars.iv.next6, %for.cond17 ]
...
for.cond63:                                       ; preds = %for.inc71, %for.end57
  %indvars.iv.next13.sink = phi i64 [ %indvars.iv.next13, %for.inc71 ], [ %idxprom58, %for.end57 ]
  %call66.sink = phi ptr [ %call66, %for.inc71 ], [ @_ZSt4cout, %for.end57 ]
  %arrayidx68 = getelementptr inbounds [1000 x [20 x i8]], ptr %words, i64 0, i64 %indvars.iv.next13.sink
  ...
  %5 = sext i32 %4 to i64
  %cmp64 = icmp slt i64 %indvars.iv.next13, %5
...
for.end73:                                        ; preds = %for.cond63
  call void @llvm.lifetime.end.p0(i64 20000, ptr nonnull %words) #6
