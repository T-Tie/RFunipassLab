...
if.else32:                                        ; preds = %if.else
  %mul34 = fmul double %2, 2.000000e+00
...
for.inc:                                          ; preds = %if.else, %if.then, %if.else32
  %call42.sink = phi double [ %call42, %if.else32 ], [ %sub, %if.then ], [ %fneg26, %if.else ]
  %.sink = phi i64 [ 1, %if.else32 ], [ 1, %if.then ], [ 0, %if.else ]
  %mul43 = fmul double %2, 2.000000e+00
  %div44 = fdiv double %call42.sink, %mul43
  %arrayidx45 = getelementptr inbounds [2 x [99 x double]], ptr %m, i64 0, i64 %.sink
