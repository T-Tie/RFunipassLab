...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 100, ptr %h) #5
  call void @llvm.lifetime.start.p0(i64 100, ptr %len) #5
...
if.end:                                           ; preds = %if.then17, %if.then
  %temp.2 = phi i32 [ %3, %if.then17 ], [ %temp.0, %if.then ]
  %tIndex.2 = phi i32 [ %j.0, %if.then17 ], [ %tIndex.0, %if.then ]
  br label %if.end20

if.end20:                                         ; preds = %if.end, %for.body8
  %temp.1 = phi i32 [ %temp.2, %if.end ], [ %temp.0, %for.body8 ]
  %tIndex.1 = phi i32 [ %tIndex.2, %if.end ], [ %tIndex.0, %for.body8 ]
  br label %for.inc21
...
for.end42:                                        ; preds = %for.cond3
  ...
  call void @llvm.lifetime.end.p0(i64 100, ptr %len) #5
  call void @llvm.lifetime.end.p0(i64 100, ptr %h) #5
