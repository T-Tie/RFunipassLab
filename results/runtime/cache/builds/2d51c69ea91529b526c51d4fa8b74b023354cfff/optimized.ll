...
for.cond:                                         ; preds = %for.inc, %if.end
  %r.0 = phi i32 [ 1, %if.end ], [ %r.1, %for.inc ]
  %i.0 = phi i32 [ %min, %if.end ], [ %inc, %for.inc ]
  %div = sdiv i32 %x, 2
...
if.then3:                                         ; preds = %for.body
  %div4 = sdiv i32 %x, %i.0
  %call = call noundef i32 @_Z1fii(i32 noundef %div4, i32 noundef %i.0) #6
  %add = add nsw i32 %r.0, %call
  br label %for.inc
...
while.body:                                       ; preds = %while.cond
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef %x) #8
