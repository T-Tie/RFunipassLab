...
tailrecurse:                                      ; preds = %for.end41, %entry
  ...
  %sub = add nsw i32 %c2.tr, -1
  br i1 %cmp, label %for.cond, label %if.else

for.cond:                                         ; preds = %tailrecurse, %for.inc
  %i.0 = phi i32 [ %dec, %for.inc ], [ %sub, %tailrecurse ]
  %cmp1 = icmp sgt i32 %i.0, -1
  br i1 %cmp1, label %for.inc, label %for.end

for.inc:                                          ; preds = %for.cond
  %idxprom = zext nneg i32 %i.0 to i64
  %arrayidx = getelementptr inbounds nuw [40 x i32], ptr @a, i64 0, i64 %idxprom
  %1 = load i32, ptr %arrayidx, align 4, !tbaa !5, !invariant.load !9
  %call = tail call noundef nonnull align 8 dereferenceable(8) ptr @_ZNSolsEi(ptr noundef nonnull align 8 dereferenceable(272) @_ZSt4cout, i32 noundef %1)
