...
for.body4:                                        ; preds = %for.cond2
  %call5 = call noundef i32 @_Z1Fi(i32 noundef %i.1) #3
  %cmp6 = icmp slt i32 %z.0, %call5
  br i1 %cmp6, label %if.then, label %for.inc8

if.then:                                          ; preds = %for.body4
  %call7 = call noundef i32 @_Z1Fi(i32 noundef %i.1) #3
  br label %for.inc8
...
for.cond:                                         ; preds = %for.inc, %entry
  %x.sink = phi i32 [ %x, %entry ], [ %add, %for.inc ]
  %z.0 = phi i32 [ 1, %entry ], [ %z.1, %for.inc ]
  %add = add nsw i32 %x.sink, 1
  %0 = load i32, ptr @n, align 4, !tbaa !5
...
land.lhs.true:                                    ; preds = %for.body
  %call = call noundef i32 @_Z1Fi(i32 noundef %add) #4
  %add4 = add nsw i32 %z.0, 1
  %cmp5 = icmp sgt i32 %add4, %z.0
  br i1 %cmp5, label %if.then, label %for.inc

if.then:                                          ; preds = %land.lhs.true
  %call6 = call noundef i32 @_Z1Fi(i32 noundef %add) #4
...
attributes #2 = { mustprogress nofree nosync nounwind memory(read) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nounwind memory(read) }
attributes #4 = { nofree nosync nounwind memory(read) }
