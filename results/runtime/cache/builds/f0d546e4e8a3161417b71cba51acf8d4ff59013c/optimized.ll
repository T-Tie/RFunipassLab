...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 50, ptr noundef %dz) #6
  br label %for.cond

for.cond:                                         ; preds = %for.inc18, %entry
  %i.0 = phi i32 [ 0, %entry ], [ %inc19, %for.inc18 ]
  %end.0 = phi i32 [ 0, %entry ], [ %end.1.lcssa, %for.inc18 ]
...
for.cond1:                                        ; preds = %for.inc, %for.body
  %i.1 = phi i32 [ 0, %for.body ], [ %inc17, %for.inc ]
  %end.1 = phi i32 [ %end.0, %for.body ], [ %add16, %for.inc ]
...
for.body3:                                        ; preds = %for.cond1
  ...
  %call6 = call i64 @strlen(ptr noundef %dz) #7
...
if.then:                                          ; preds = %for.body3
  %call9 = call i32 (ptr, ...) @printf(ptr noundef @.str.2) #6
  br label %for.inc
...
if.then11:                                        ; preds = %if.else
  %call12 = call i32 (ptr, ...) @printf(ptr noundef @.str.3) #6
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.then11, %if.else
  %end.2 = phi i32 [ 0, %if.then ], [ %add, %if.then11 ], [ %end.1, %if.else ]
  %call15 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, ptr noundef %dz) #6
  %add16 = add nsw i32 %end.2, %conv
  %inc17 = add nsw i32 %i.1, 1
  br label %for.cond1, !llvm.loop !9

for.inc18:                                        ; preds = %for.cond1
  %i.1.lcssa = phi i32 [ %i.1, %for.cond1 ]
  %end.1.lcssa = phi i32 [ %end.1, %for.cond1 ]
  %inc19 = add nsw i32 %i.1.lcssa, 1
  br label %for.cond, !llvm.loop !12

for.end20:                                        ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 noundef 50, ptr noundef %dz) #6
