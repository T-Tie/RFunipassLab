...
for.cond.preheader:                               ; preds = %if.then, %entry
  %.ph = phi i32 [ %0, %entry ], [ %1, %if.then ]
  %.ph11 = phi i32 [ %1, %entry ], [ %0, %if.then ]
  br label %for.cond

for.cond:                                         ; preds = %for.cond.preheader, %if.end9
  %2 = phi i32 [ %7, %if.end9 ], [ %.ph, %for.cond.preheader ]
  %3 = phi i32 [ %8, %if.end9 ], [ %.ph11, %for.cond.preheader ]
  %i.0 = phi i32 [ %inc, %if.end9 ], [ 3, %for.cond.preheader ]
...
for.body:                                         ; preds = %for.cond
  ...
  br i1 %or.cond, label %if.end9.sink.split, label %if.else
...
if.end9.sink.split:                               ; preds = %for.body, %if.then7
  %.sink = phi i32 [ %6, %if.then7 ], [ %4, %for.body ]
  %.ph9 = phi i32 [ %4, %if.then7 ], [ %6, %for.body ]
  store i32 %.sink, ptr %b, align 4, !tbaa !5
  br label %if.end9

if.end9:                                          ; preds = %if.end9.sink.split, %if.else
  %7 = phi i32 [ %5, %if.else ], [ %.sink, %if.end9.sink.split ]
  %8 = phi i32 [ %6, %if.else ], [ %.ph9, %if.end9.sink.split ]
  call void @llvm.lifetime.end.p0(i64 4, ptr nonnull %c) #4
  %inc = add nuw nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !9

for.end:                                          ; preds = %for.cond
  %call10 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.2, i32 noundef %3, i32 noundef %2)
