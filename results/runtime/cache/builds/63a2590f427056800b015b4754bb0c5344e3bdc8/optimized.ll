...
for.end39:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef nonnull align 4 dereferenceable(4) %x1) #8
  store i32 0, ptr %x1, align 4, !tbaa !5
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef nonnull align 4 dereferenceable(4) %x2) #8
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 4, ptr noundef nonnull align 4 dereferenceable(4) %j) #8
  call fastcc void @outlined_ir_func_0(ptr noalias nofree noundef nonnull writeonly align 4 captures(none) dereferenceable(4) %i, i32 noundef %m1, ptr noalias nofree noundef nonnull align 4 captures(none) dereferenceable(4) %x1, i32 noundef %y) #10
  call fastcc void @outlined_ir_func_0(ptr noalias nofree noundef nonnull writeonly align 4 captures(none) dereferenceable(4) %j, i32 noundef %m2, ptr noalias nofree noundef nonnull align 4 captures(none) dereferenceable(4) %x2, i32 noundef %y) #10
  ...
  %puts = tail call i32 @puts(ptr noundef nonnull dereferenceable(1) %str.2.str) #9
...
for.cond:                                         ; preds = %for.inc, %entry_to_outline
  %4 = phi i32 [ %.promoted, %entry_to_outline ], [ %5, %for.inc ]
...
for.body:                                         ; preds = %for.cond
  ...
  switch i32 %storemerge, label %for.inc [
    ...
    i32 9, label %for.inc.sink.split
    i32 11, label %for.inc.sink.split
    i32 2, label %if.then8
  ]

if.then8:                                         ; preds = %for.body
  %call = tail call noundef i32 @_Z7runniani(i32 noundef %3) #11
  %tobool.not = icmp eq i32 %call, 0
  %. = select i1 %tobool.not, i32 28, i32 29
  br label %for.inc.sink.split

for.inc.sink.split:                               ; preds = %if.then8, %for.body, %for.body, %for.body, %for.body
  %.sink = phi i32 [ 30, %for.body ], [ 30, %for.body ], [ 30, %for.body ], [ 30, %for.body ], [ %., %if.then8 ]
  %dec = add nsw i32 %.sink, %4
  store i32 %dec, ptr %2, align 4, !tbaa !5
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %for.body
  %5 = phi i32 [ %add, %for.body ], [ %dec, %for.inc.sink.split ]
  %inc = add nuw nsw i32 %storemerge, 1
  br label %for.cond, !llvm.loop !12

for.end.exitStub:                                 ; preds = %for.cond
  %smax = call i32 @llvm.smax.i32(i32 noundef %1, i32 noundef 1) #12
...
attributes #10 = { nofree norecurse nosync nounwind willreturn }
attributes #11 = { nofree nosync nounwind willreturn memory(none) }
