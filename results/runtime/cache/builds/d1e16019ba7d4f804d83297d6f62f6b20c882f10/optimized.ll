...
for.inc:                                          ; preds = %if.end, %if.then9, %if.end.thread
  %num.17 = phi i32 [ 0, %if.end.thread ], [ %num.0, %if.then9 ], [ %num.0, %if.end ]
  %blank.16 = phi i64 [ 0, %if.end.thread ], [ 1, %if.then9 ], [ 0, %if.end ]
  ...
  %add17 = add i64 %call15, %blank.16
  %3 = trunc i64 %add17 to i32
  %conv20 = add i32 %num.17, %3
