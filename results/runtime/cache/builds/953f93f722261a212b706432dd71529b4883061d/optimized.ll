...
for.cond:                                         ; preds = %if.end9, %if.end
  %4 = phi i32 [ %2, %if.end ], [ %9, %if.end9 ]
  %5 = phi i32 [ %3, %if.end ], [ %10, %if.end9 ]
  %i.0 = phi i32 [ 3, %if.end ], [ %inc, %if.end9 ]
...
if.end9.sink.split:                               ; preds = %for.body, %if.then7
  ...
  %.ph9 = phi i32 [ %6, %if.then7 ], [ %8, %for.body ]
  store i32 %.sink, ptr %b, align 4, !tbaa !5
  br label %if.end9

if.end9:                                          ; preds = %if.end9.sink.split, %if.else
  %9 = phi i32 [ %7, %if.else ], [ %.sink, %if.end9.sink.split ]
  %10 = phi i32 [ %8, %if.else ], [ %.ph9, %if.end9.sink.split ]
  call void @llvm.lifetime.end.p0(i64 noundef 4, ptr noundef nonnull %c) #5
  %inc = add nuw nsw i32 %i.0, 1
  br label %for.cond, !llvm.loop !9

for.end:                                          ; preds = %for.cond
  %call10 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.2, i32 noundef %5, i32 noundef %4) #5
