...
if.end.thread:                                    ; preds = %while.body
  %cmp811 = icmp slt i32 %add, 80
  br i1 %cmp811, label %if.else14, label %if.then25
...
if.end19:                                         ; preds = %if.end, %if.else14
  ...
  %sum.112 = phi i32 [ %add7, %if.else14 ], [ %add, %if.end ]
...
if.end29:                                         ; preds = %if.end19, %if.then21, %if.then25
  ...
  %sum.3 = phi i32 [ %conv, %if.then25 ], [ %sum.112, %if.end19 ], [ 0, %if.then21 ]
...
while.end:                                        ; preds = %_ZStrsIcSt11char_traitsIcEERSt13basic_istreamIT_T0_ES6_PS3_.exit
  ...
  call void @llvm.lifetime.end.p0(i64 noundef 40, ptr noundef nonnull captures(none) %a) #9
  ret i32 0
}
