...
for.end33:                                        ; preds = %for.end23, %for.cond10.for.end33_crit_edge
  ...
  %x1.1 = phi i32 [ %x1.0, %for.cond10.for.end33_crit_edge ], [ %x1.2, %for.end23 ]
  %y1.1 = phi i32 [ %y1.0, %for.cond10.for.end33_crit_edge ], [ %y1.2, %for.end23 ]
  br label %for.cond34

for.cond34:                                       ; preds = %for.end50, %for.end33
  %indvars.iv43 = phi i64 [ %indvars.iv.next44, %for.end50 ], [ %.pre-phi, %for.end33 ]
  %x2.0 = phi i32 [ %x2.2, %for.end50 ], [ 0, %for.end33 ]
  %y2.0 = phi i32 [ %y2.2, %for.end50 ], [ 0, %for.end33 ]
...
for.body40:                                       ; preds = %for.cond38
  ...
  %arrayidx44 = getelementptr inbounds nuw [100 x [100 x i32]], ptr %a, i64 0, i64 %idxprom41, i64 %indvars.iv.next41
  %9 = load i32, ptr %arrayidx44, align 4, !tbaa !5
  %cmp45 = icmp eq i32 %9, 0
  br i1 %cmp45, label %for.end50.split.loop.exit, label %for.cond38, !llvm.loop !15

for.end50.split.loop.exit:                        ; preds = %for.body40
  %10 = trunc nsw i64 %indvars.iv.next41 to i32
  %11 = shl i64 %indvars.iv.next41, 32
  %12 = ashr exact i64 %11, 32
  br label %for.end50

for.end50:                                        ; preds = %for.cond38, %for.end50.split.loop.exit
  %j.2.lcssa = phi i64 [ %12, %for.end50.split.loop.exit ], [ -1, %for.cond38 ]
  %x2.2 = phi i32 [ %indvars, %for.end50.split.loop.exit ], [ %x2.0, %for.cond38 ]
  %y2.2 = phi i32 [ %10, %for.end50.split.loop.exit ], [ %y2.0, %for.cond38 ]
  %arrayidx54 = getelementptr inbounds [100 x [100 x i32]], ptr %a, i64 0, i64 %idxprom41, i64 %j.2.lcssa
...
for.end61:                                        ; preds = %for.end50, %for.cond34
  ...
  %14 = xor i32 %y1.1, -1
  %sub63 = add i32 %y2.1, %14
  %15 = xor i32 %x1.1, -1
  %sub66 = add i32 %x2.1, %15
