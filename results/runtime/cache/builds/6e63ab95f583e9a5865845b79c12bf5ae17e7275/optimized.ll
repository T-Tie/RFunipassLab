...
entry:
  %call = call i64 @strlen(ptr noundef readonly @s) #4
  %conv = trunc i64 %call to i32
  %sub = sub nsw i32 %conv, 1
  br label %for.cond

for.cond:                                         ; preds = %for.inc11, %entry
  %phiofops = phi i32 [ %conv, %entry ], [ %i.0, %for.inc11 ]
...
for.cond3:                                        ; preds = %for.inc, %if.then
  %j.0 = phi i32 [ %phiofops, %if.then ], [ %inc, %for.inc ]
...
attributes #4 = { nofree nosync nounwind willreturn memory(read) }
