...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 50, ptr noundef nonnull %dz) #6
...
for.cond1:                                        ; preds = %for.cond, %for.inc
  %1 = phi i32 [ %.pre2, %for.inc ], [ %0, %for.cond ]
  %i.1 = phi i32 [ %inc17, %for.inc ], [ 0, %for.cond ]
  %end.1 = phi i32 [ %add16, %for.inc ], [ %end.0, %for.cond ]
...
if.else:                                          ; preds = %for.body3
  ...
  br i1 %cmp10, label %for.inc.sink.split, label %for.inc

for.inc.sink.split:                               ; preds = %if.else, %for.body3
  %.sink = phi i32 [ 10, %for.body3 ], [ 32, %if.else ]
  %end.2.ph = phi i32 [ 0, %for.body3 ], [ %add, %if.else ]
  %putchar1 = call i32 @putchar(i32 noundef %.sink) #6
  br label %for.inc

for.inc:                                          ; preds = %for.inc.sink.split, %if.else
  %end.2 = phi i32 [ %end.1, %if.else ], [ %end.2.ph, %for.inc.sink.split ]
  %call15 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.1, ptr noundef nonnull %dz) #6
...
for.end20:                                        ; preds = %for.cond
  call void @llvm.lifetime.end.p0(i64 noundef 50, ptr noundef nonnull %dz) #6
