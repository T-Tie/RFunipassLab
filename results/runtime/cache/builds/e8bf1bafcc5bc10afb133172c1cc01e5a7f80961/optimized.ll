...
if.else10:                                        ; preds = %if.else7
  ...
  %.28 = call i32 @llvm.smax.i32(i32 %1, i32 %2)
  %__c.__b = select i1 %cmp.i4, ptr %__c, ptr %__b
  br label %if.end16

if.end16:                                         ; preds = %if.else10, %if.else, %if.then9, %if.then2
  %.sink27 = phi i32 [ %0, %if.then9 ], [ %1, %if.then2 ], [ %., %if.else ], [ %.28, %if.else10 ]
  %__b.sink = phi ptr [ %__a, %if.then9 ], [ %__b, %if.then2 ], [ %__c.__a, %if.else ], [ %__c.__b, %if.else10 ]
