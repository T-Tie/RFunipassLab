...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 4000, ptr nonnull %result) #5
...
for.body:                                         ; preds = %for.cond
  ...
  %smax = call i32 @llvm.smax.i32(i32 %index.0, i32 0)
...
for.end:                                          ; preds = %for.cond2, %for.end.split.loop.exit
  %j.0.lcssa = phi i32 [ %3, %for.end.split.loop.exit ], [ %smax, %for.cond2 ]
  %cmp6.not = icmp eq i32 %j.0.lcssa, %index.0
  br i1 %cmp6.not, label %if.then7, label %if.end11
...
if.end11:                                         ; preds = %if.then7, %for.end
  %index.1 = phi i32 [ %index.0, %for.end ], [ %inc10, %if.then7 ]
...
for.cond15:                                       ; preds = %for.cond15.preheader, %for.body17
  ...
  br i1 %exitcond19.not, label %for.end23, label %for.body17

for.body17:                                       ; preds = %for.cond15
  %arrayidx19 = getelementptr inbounds nuw [1000 x i32], ptr %result, i64 0, i64 %indvars.iv14
...
for.end23:                                        ; preds = %for.cond15
  ...
  call void @llvm.lifetime.end.p0(i64 4000, ptr nonnull %result) #5
  ret i32 0
}
