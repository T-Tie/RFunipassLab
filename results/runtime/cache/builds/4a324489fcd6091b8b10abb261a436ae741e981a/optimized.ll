...
for.inc74.gvnsink.split:                          ; preds = %if.then, %if.then44, %if.else58
  %idxprom67.pre-phi = phi i64 [ %idxprom19, %if.then ], [ %idxprom45, %if.then44 ], [ %idxprom45, %if.else58 ]
  %k.0.sink = phi i32 [ %inc18, %if.then ], [ %k.0, %if.then44 ], [ %k.0, %if.else58 ]
  %add66.sink1 = phi i32 [ %conv, %if.then ], [ %conv, %if.then44 ], [ %add, %if.else58 ]
