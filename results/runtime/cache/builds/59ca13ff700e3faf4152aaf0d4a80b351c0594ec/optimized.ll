...
entry:
  ...
  %call = call i32 (ptr, ...) @__isoc23_scanf(ptr noundef @.str, ptr noundef %h, ptr noundef %c, ptr noundef %d)
  br label %for.cond
...
if.end37.thread:                                  ; preds = %for.body30
  ...
  %arrayidx395 = getelementptr inbounds [10 x [10 x i32]], ptr %a, i64 0, i64 %idxprom384
  %idxprom406 = sext i32 %l1.1 to i64
  %arrayidx417 = getelementptr inbounds [10 x i32], ptr %arrayidx395, i64 0, i64 %idxprom406
  %6 = load i32, ptr %arrayidx417, align 4, !tbaa !5
  %cmp428 = icmp slt i32 %6, %5
  br i1 %cmp428, label %for.inc50, label %for.inc50

land.lhs.true:                                    ; preds = %if.end37
  %7 = phi i32 [ %4, %if.end37 ]
  %min.210 = phi i32 [ %min.1, %if.end37 ]
  %l2.29 = phi i32 [ %l2.1, %if.end37 ]
...
for.inc50:                                        ; preds = %if.end37.thread, %if.end37.thread, %if.end37, %land.lhs.true, %if.then44
  %l2.3 = phi i32 [ %k.0, %if.then44 ], [ %l2.29, %land.lhs.true ], [ %l2.1, %if.end37 ], [ %k.0, %if.end37.thread ], [ %k.0, %if.end37.thread ]
  %min.3 = phi i32 [ %7, %if.then44 ], [ %min.210, %land.lhs.true ], [ %min.1, %if.end37 ], [ %5, %if.end37.thread ], [ %5, %if.end37.thread ]
