...
if.then:                                          ; preds = %lor.lhs.false5, %lor.lhs.false3, %lor.lhs.false, %for.body
  %dec = add nsw i32 %add, -1
  br label %if.end

if.end:                                           ; preds = %if.then, %lor.lhs.false5
  %x1.1 = phi i32 [ %dec, %if.then ], [ %add, %lor.lhs.false5 ]
...
if.end11:                                         ; preds = %if.then8, %if.else
  ...
  %sub10 = sub nsw i32 %x1.1, %.sink2
  br label %for.inc

for.inc:                                          ; preds = %if.end, %if.end11
  %x1.2 = phi i32 [ %sub10, %if.end11 ], [ %x1.1, %if.end ]
  %inc = add nsw i32 %i.0, 1
...
if.then24:                                        ; preds = %lor.lhs.false22, %lor.lhs.false20, %lor.lhs.false18, %for.body15
  %dec25 = add nsw i32 %add16, -1
  br label %if.end26

if.end26:                                         ; preds = %if.then24, %lor.lhs.false22
  %x2.1 = phi i32 [ %dec25, %if.then24 ], [ %add16, %lor.lhs.false22 ]
...
if.end35:                                         ; preds = %if.then28, %if.else33
  %.sink1 = phi i32 [ 3, %if.else33 ], [ 2, %if.then28 ]
  %sub34 = sub nsw i32 %x2.1, %.sink1
  br label %for.inc37

for.inc37:                                        ; preds = %if.end26, %if.end35
  %x2.2 = phi i32 [ %sub34, %if.end35 ], [ %x2.1, %if.end26 ]
