...
if.end:                                           ; preds = %if.else58, %if.then44
  %add66.sink1 = phi i32 [ %add, %if.else58 ], [ %conv, %if.then44 ]
  %arrayidx68 = getelementptr inbounds [1000 x [81 x i8]], ptr %newword, i64 0, i64 %idxprom77
  br label %for.inc74.gvnsink.split

for.inc74.gvnsink.split:                          ; preds = %if.then, %if.end
  %add66.sink1.sink2 = phi i32 [ %add66.sink1, %if.end ], [ %conv, %if.then ]
  %arrayidx68.sink = phi ptr [ %arrayidx68, %if.end ], [ %arrayidx20, %if.then ]
  %k.1.ph = phi i32 [ %k.0, %if.end ], [ %inc18, %if.then ]
  %idxprom69 = sext i32 %add66.sink1.sink2 to i64
  %arrayidx70 = getelementptr inbounds [81 x i8], ptr %arrayidx68.sink, i64 0, i64 %idxprom69
  store i8 32, ptr %arrayidx70, align 1, !tbaa !12
  %inc71 = add nsw i32 %add66.sink1.sink2, 1
  br label %for.inc74
