...
if.end:                                           ; preds = %lor.lhs.false5
  %x1.1 = phi i32 [ %add, %lor.lhs.false5 ]
...
for.inc.gvnsink.split:                            ; preds = %if.then8, %if.else
  %.sink3 = phi i32 [ 3, %if.else ], [ 2, %if.then8 ]
  %sub10 = sub nsw i32 %x1.1, %.sink3
  br label %for.inc

for.inc:                                          ; preds = %if.end.thread, %for.inc.gvnsink.split, %if.end
  %x1.2 = phi i32 [ %sub10, %for.inc.gvnsink.split ], [ %x1.1, %if.end ], [ %dec, %if.end.thread ]
  %inc = add nsw i32 %i.0, 1
...
if.end26:                                         ; preds = %lor.lhs.false22
  %x2.1 = phi i32 [ %add16, %lor.lhs.false22 ]
...
for.inc37.gvnsink.split:                          ; preds = %if.then28, %if.else33
  %.sink2 = phi i32 [ 3, %if.else33 ], [ 2, %if.then28 ]
  %sub34 = sub nsw i32 %x2.1, %.sink2
  br label %for.inc37

for.inc37:                                        ; preds = %if.end26.thread, %for.inc37.gvnsink.split, %if.end26
  %x2.2 = phi i32 [ %sub34, %for.inc37.gvnsink.split ], [ %x2.1, %if.end26 ], [ %dec25, %if.end26.thread ]
