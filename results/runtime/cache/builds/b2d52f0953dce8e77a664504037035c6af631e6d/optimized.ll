...
for.cond10:                                       ; preds = %for.end71, %for.end9
  %min.0 = phi i32 [ 0, %for.end9 ], [ %min.1, %for.end71 ]
  %t.0 = phi i32 [ 0, %for.end9 ], [ %t.1, %for.end71 ]
  %r.0 = phi i32 [ undef, %for.end9 ], [ %r.1, %for.end71 ]
  %max.0 = phi i32 [ 0, %for.end9 ], [ %max.4, %for.end71 ]
  %i.1 = phi i32 [ 0, %for.end9 ], [ %inc73, %for.end71 ]
  %s.0 = phi i32 [ undef, %for.end9 ], [ %s.1, %for.end71 ]
...
for.cond13:                                       ; preds = %if.end30, %for.body12
  %r.1 = phi i32 [ %r.0, %for.body12 ], [ %r.2, %if.end30 ]
  %max.1 = phi i32 [ %max.0, %for.body12 ], [ %max.2, %if.end30 ]
  %j.1 = phi i32 [ 0, %for.body12 ], [ %inc32, %if.end30 ]
...
if.else:                                          ; preds = %for.body15
  %arrayidx21 = getelementptr inbounds [8 x [8 x i32]], ptr %a, i64 0, i64 %idxprom17
...
if.end30:                                         ; preds = %if.else, %if.then25, %if.then
...
for.cond34:                                       ; preds = %if.end68, %for.end33
  %min.1 = phi i32 [ %min.0, %for.end33 ], [ %min.5, %if.end68 ]
  %t.1 = phi i32 [ %t.0, %for.end33 ], [ %t.2, %if.end68 ]
  %max.4 = phi i32 [ %max.1, %for.end33 ], [ %max.5, %if.end68 ]
  %k.0 = phi i32 [ 0, %for.end33 ], [ %inc70, %if.end68 ]
  %s.1 = phi i32 [ %s.0, %for.end33 ], [ %s.2, %if.end68 ]
...
for.cond37:                                       ; preds = %if.end57, %for.body36
  %min.2 = phi i32 [ %min.1, %for.body36 ], [ %min.3, %if.end57 ]
  %l.0 = phi i32 [ 0, %for.body36 ], [ %inc59, %if.end57 ]
  %s.2 = phi i32 [ %s.1, %for.body36 ], [ %s.3, %if.end57 ]
...
if.else45:                                        ; preds = %for.body39
  ...
  %arrayidx49 = getelementptr inbounds [8 x i32], ptr %arrayidx47, i64 0, i64 %idxprom43
...
if.end57:                                         ; preds = %if.else45, %if.then51, %if.then41
...
if.then65:                                        ; preds = %land.lhs.true63
  %call66 = call i32 (ptr, ...) @printf(ptr noundef @.str.2, i32 noundef %i.1, i32 noundef %k.0)
  %inc67 = add nsw i32 %t.1, 1
  br label %if.end68

if.end68:                                         ; preds = %if.then65, %land.lhs.true63, %land.lhs.true, %for.end60
  %min.5 = phi i32 [ 100, %if.then65 ], [ %min.2, %land.lhs.true63 ], [ %min.2, %land.lhs.true ], [ %min.2, %for.end60 ]
