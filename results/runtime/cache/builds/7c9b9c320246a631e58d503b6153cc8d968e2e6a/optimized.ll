...
if.then8:                                         ; preds = %if.end
  %call = call noundef i32 @_Z7runniani(i32 noundef %y)
...
for.inc:                                          ; preds = %if.end, %if.end11, %if.end.thread
  %x1.2 = phi i32 [ %sub10, %if.end11 ], [ %add, %if.end ], [ %dec, %if.end.thread ]
  %inc = add nsw i32 %i.0, 1
...
for.cond13:                                       ; preds = %for.inc37, %for.end
  %x2.0 = phi i32 [ 0, %for.end ], [ %x2.2, %for.inc37 ]
  %j.0 = phi i32 [ 1, %for.end ], [ %inc38, %for.inc37 ]
  %cmp14 = icmp slt i32 %j.0, %m2
...
if.then28:                                        ; preds = %if.end26
  %call29 = call noundef i32 @_Z7runniani(i32 noundef %y)
...
for.inc37:                                        ; preds = %if.end26, %if.end35, %if.end26.thread
  %x2.2 = phi i32 [ %sub34, %if.end35 ], [ %add16, %if.end26 ], [ %dec25, %if.end26.thread ]
