...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 80000, ptr %B) #3
...
for.end15:                                        ; preds = %if.then, %for.cond5
  %phiofops = phi i1 [ true, %for.cond5 ], [ false, %if.then ]
  %flag.0 = phi i32 [ 1, %if.then ], [ 0, %for.cond5 ]
  br i1 %phiofops, label %if.then17, label %if.end23
...
if.end23:                                         ; preds = %if.then17, %for.end15
  %j.1 = phi i32 [ %add, %if.then17 ], [ %j.0, %for.end15 ]
  br label %for.inc24
...
for.end41:                                        ; preds = %for.cond27
  ...
  call void @llvm.lifetime.end.p0(i64 80000, ptr %B) #3
