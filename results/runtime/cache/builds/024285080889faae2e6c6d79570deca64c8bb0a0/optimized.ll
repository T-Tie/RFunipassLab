...
if.end:                                           ; preds = %entry
  ...
  call void @llvm.lifetime.start.p0(i64 4, ptr nonnull %lmax) #4
...
while.cond.preheader:                             ; preds = %if.then5, %if.end
  %.ph4 = phi i32 [ %3, %if.end ], [ %2, %if.then5 ]
  br label %while.cond

while.cond:                                       ; preds = %while.cond.preheader, %if.end14
  %4 = phi i32 [ %9, %if.end14 ], [ %.ph4, %while.cond.preheader ]
  %i.0 = phi i32 [ %inc, %if.end14 ], [ 3, %while.cond.preheader ]
...
if.then10:                                        ; preds = %while.body
  store i32 %7, ptr %lmax, align 4, !tbaa !5
  br label %if.end14.sink.split

if.else:                                          ; preds = %while.body
  %8 = load i32, ptr %lmax, align 4, !tbaa !5
  %cmp11 = icmp sgt i32 %6, %8
  br i1 %cmp11, label %if.end14.sink.split, label %if.end14

if.end14.sink.split:                              ; preds = %if.else, %if.then10
  %lmax.sink = phi ptr [ %max, %if.then10 ], [ %lmax, %if.else ]
  %.ph = phi i32 [ %6, %if.then10 ], [ %7, %if.else ]
  store i32 %6, ptr %lmax.sink, align 4, !tbaa !5
  br label %if.end14

if.end14:                                         ; preds = %if.end14.sink.split, %if.else
  %9 = phi i32 [ %7, %if.else ], [ %.ph, %if.end14.sink.split ]
  %inc = add nuw nsw i32 %i.0, 1
  br label %while.cond, !llvm.loop !9

while.end:                                        ; preds = %while.cond
  %call15 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.1, i32 noundef %4)
  %10 = load i32, ptr %lmax, align 4, !tbaa !5
  %call16 = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str, i32 noundef %10)
  call void @llvm.lifetime.end.p0(i64 4, ptr nonnull %lmax) #4
