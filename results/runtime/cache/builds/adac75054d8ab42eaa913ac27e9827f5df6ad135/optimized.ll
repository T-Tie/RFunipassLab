...
for.inc.gvnsink.split:                            ; preds = %land.lhs.true, %lor.lhs.false32, %lor.lhs.false22, %lor.lhs.false20, %lor.lhs.false18, %if.end, %if.else
  %.sink10 = phi i32 [ 28, %if.else ], [ 30, %if.end ], [ 30, %lor.lhs.false18 ], [ 30, %lor.lhs.false20 ], [ 30, %lor.lhs.false22 ], [ 29, %lor.lhs.false32 ], [ 29, %land.lhs.true ]
  %add37 = add nsw i32 %total1.1, %.sink10
  br label %for.inc

for.inc:                                          ; preds = %for.inc.gvnsink.split, %if.end26
  %total1.2 = phi i32 [ %add37, %for.inc.gvnsink.split ], [ %total1.1, %if.end26 ]
  %inc = add nsw i32 %1, 1
...
for.inc85.gvnsink.split:                          ; preds = %land.lhs.true73, %lor.lhs.false76, %lor.lhs.false64, %lor.lhs.false62, %lor.lhs.false60, %if.end58, %if.else81
  %.sink9 = phi i32 [ 28, %if.else81 ], [ 30, %if.end58 ], [ 30, %lor.lhs.false60 ], [ 30, %lor.lhs.false62 ], [ 30, %lor.lhs.false64 ], [ 29, %lor.lhs.false76 ], [ 29, %land.lhs.true73 ]
  %add82 = add nsw i32 %total2.1, %.sink9
  br label %for.inc85

for.inc85:                                        ; preds = %for.inc85.gvnsink.split, %if.end68
  %total2.2 = phi i32 [ %add82, %for.inc85.gvnsink.split ], [ %total2.1, %if.end68 ]
