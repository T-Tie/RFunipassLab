...
for.cond13.preheader:                             ; preds = %for.cond
  %smax6 = call i32 @llvm.smax.i32(i32 noundef %m2, i32 noundef 1) #9
  br label %for.cond13
...
for.inc:                                          ; preds = %for.body, %if.end.thread, %if.else, %if.then9
  %x1.1 = phi i32 [ %dec, %if.end.thread ], [ %sub, %if.then9 ], [ %sub10, %if.else ], [ %add, %for.body ]
  %inc = add nuw i32 %0, 1
...
for.cond13:                                       ; preds = %for.cond13.preheader, %for.inc37
  ...
  %exitcond7.not = icmp eq i32 %1, %smax6
...
for.inc37:                                        ; preds = %for.body15, %if.end26.thread, %if.else33, %if.then31
  %x2.1 = phi i32 [ %dec25, %if.end26.thread ], [ %sub32, %if.then31 ], [ %sub34, %if.else33 ], [ %add16, %for.body15 ]
