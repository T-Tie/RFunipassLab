...
if.then.i:                                        ; preds = %entry
  call fastcc void @outlined_ir_func_0() #15
...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 8, ptr noundef nonnull align 8 dereferenceable(8) %float_n) #18
  %call = tail call nonnull ptr @polybench_alloc_data(i64 noundef 1680000, i32 noundef 8) #15
  %call1 = tail call ptr @polybench_alloc_data(i64 noundef 1440000, i32 noundef 8) #15
  %call2 = tail call nonnull ptr @polybench_alloc_data(i64 noundef 1200, i32 noundef 8) #15
  call fastcc void @init_array(ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) %float_n, ptr nofree noundef nonnull writeonly captures(none) %call) #19
  tail call void @polybench_timer_start() #15
  tail call fastcc void @kernel_covariance(ptr nofree noundef nonnull captures(none) %call, ptr nofree noundef nonnull captures(none) %call2) #14
  unreachable
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) initializes((0, 8)) %float_n, ptr nofree noundef nonnull writeonly captures(none) %data) unnamed_addr #8 {
entry:
...
for.cond:                                         ; preds = %for.end, %entry
  ...
  %exitcond11.not = icmp eq i64 %indvars.iv8, 1400
  br i1 %exitcond11.not, label %for.end12, label %for.cond2
...
for.body5:                                        ; preds = %for.cond2
  ...
  %arrayidx9 = getelementptr inbounds nuw [1200 x double], ptr %data, i64 %indvars.iv8, i64 %indvars.iv
...
define internal fastcc void @kernel_covariance(ptr nofree noundef nonnull captures(none) %data, ptr nofree noundef nonnull captures(none) %mean) unnamed_addr #9 {
entry:
  br label %for.cond

for.cond:                                         ; preds = %for.inc12, %entry
  %indvars.iv6 = phi i64 [ %indvars.iv.next7, %for.inc12 ], [ 0, %entry ]
  %exitcond9.not = icmp eq i64 %indvars.iv6, 1200
  br i1 %exitcond9.not, label %for.body20.preheader, label %for.body

for.body20.preheader:                             ; preds = %for.cond
  %0 = load double, ptr %mean, align 8, !tbaa !5, !invariant.load !9
  %1 = load double, ptr %data, align 8, !tbaa !5, !invariant.load !9
  %sub = fsub double %1, %0
  store double %sub, ptr %data, align 8, !tbaa !5
  br label %for.body20

for.body:                                         ; preds = %for.cond
  %arrayidx = getelementptr inbounds nuw double, ptr %mean, i64 %indvars.iv6
  br label %for.cond1
...
for.inc:                                          ; preds = %for.cond1
  %arrayidx7 = getelementptr inbounds nuw [1200 x double], ptr %data, i64 %indvars.iv, i64 %indvars.iv6
  %2 = load double, ptr %arrayidx7, align 8, !tbaa !5, !invariant.load !9
...
define internal fastcc void @outlined_ir_func_0() unnamed_addr #10 {
...
attributes #9 = { nofree norecurse noreturn nosync nounwind memory(argmem: readwrite) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
...
attributes #19 = { nounwind memory(write) }
