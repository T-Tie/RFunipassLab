...
entry:
  ...
  %call.i = call i32 @gettimeofday(ptr noundef nonnull align 8 captures(none) %Tp.i, ptr noundef null) #14
  %cmp.not.i = icmp eq i32 %call.i, 0
  br i1 %cmp.not.i, label %rtclock.exit, label %if.then.i

if.then.i:                                        ; preds = %entry
  %call1.i = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.3, i32 noundef %call.i) #14
...
entry:
  %call = tail call nonnull ptr @polybench_alloc_data(i64 noundef 1690000, i32 noundef 8) #14
  %call1 = tail call nonnull ptr @polybench_alloc_data(i64 noundef 1690000, i32 noundef 8) #14
  tail call fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %call, ptr nofree noundef nonnull writeonly captures(none) %call1) #19
  unreachable
}

; Function Attrs: nofree norecurse noreturn nosync nounwind memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %A, ptr nofree noundef nonnull writeonly captures(none) %B) unnamed_addr #11 {
entry:
  br label %for.body

for.body:                                         ; preds = %entry, %for.end
  %indvars.iv5 = phi i64 [ 0, %entry ], [ %indvars.iv.next6, %for.end ]
  %0 = trunc nuw nsw i64 %indvars.iv5 to i32
...
for.inc:                                          ; preds = %for.cond1
  ...
  %3 = tail call double @llvm.fmuladd.f64(double %conv, double %conv4, double noundef 2.000000e+00) #20
  %div = fdiv double %3, 1.300000e+03
  %arrayidx7 = getelementptr inbounds nuw [1300 x double], ptr %A, i64 %indvars.iv5, i64 %indvars.iv
  store double %div, ptr %arrayidx7, align 8, !tbaa !11
  %4 = trunc i64 %indvars.iv to i32
  %5 = add i32 %4, 3
  %conv10 = uitofp nneg i32 %5 to double
  %6 = tail call double @llvm.fmuladd.f64(double %conv, double %conv10, double noundef 3.000000e+00) #20
  %div12 = fdiv double %6, 1.300000e+03
  %arrayidx16 = getelementptr inbounds nuw [1300 x double], ptr %B, i64 %indvars.iv5, i64 %indvars.iv
...
define internal fastcc void @outlined_ir_func_1() unnamed_addr #12 {
...
attributes #12 = { cold minsize nofree nounwind optsize memory(readwrite, argmem: none) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
