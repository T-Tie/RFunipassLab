...
entry:
  ...
  call void @llvm.lifetime.start.p0(i64 noundef 8, ptr noundef nonnull align 8 captures(none) dereferenceable(8) %float_n) #17
  %call = tail call ptr @polybench_alloc_data(i64 noundef 1680000, i32 noundef 8) #14
  %call1 = tail call ptr @polybench_alloc_data(i64 noundef 1440000, i32 noundef 8) #14
  %call2 = tail call ptr @polybench_alloc_data(i64 noundef 1200, i32 noundef 8) #14
  call fastcc void @init_array(ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) %float_n, ptr nofree noundef writeonly captures(none) %call) #18
  tail call void @polybench_timer_start() #14
  tail call fastcc void @kernel_covariance(ptr nofree noundef captures(none) %call, ptr nofree noundef nonnull captures(none) %call2) #13
  unreachable
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr noalias nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) initializes((0, 8)) %float_n, ptr nofree noundef writeonly captures(none) %data) unnamed_addr #8 {
entry:
...
for.cond:                                         ; preds = %for.end, %entry
  ...
  %exitcond14.not = icmp eq i64 %indvars.iv11, 1400
  br i1 %exitcond14.not, label %for.end12, label %for.cond2
...
for.body5:                                        ; preds = %for.cond2
  %0 = mul nuw nsw i64 %indvars.iv, %indvars.iv11
  %1 = trunc nuw nsw i64 %0 to i32
  %mul = uitofp nneg i32 %1 to double
  %div = fdiv double %mul, 1.200000e+03
  %arrayidx9 = getelementptr inbounds nuw [1200 x double], ptr %data, i64 %indvars.iv11, i64 %indvars.iv
...
define internal fastcc void @kernel_covariance(ptr nofree noundef captures(none) %data, ptr nofree noundef nonnull captures(none) %mean) unnamed_addr #9 {
entry:
  br label %for.cond

for.cond:                                         ; preds = %for.inc12, %entry
  %indvars.iv16 = phi i64 [ %indvars.iv.next17, %for.inc12 ], [ 0, %entry ]
  %exitcond19.not = icmp eq i64 %indvars.iv16, 1200
  br i1 %exitcond19.not, label %for.body20.preheader, label %for.body
...
for.body:                                         ; preds = %for.cond
  %arrayidx = getelementptr inbounds nuw double, ptr %mean, i64 %indvars.iv16
  br label %for.cond1
...
for.inc:                                          ; preds = %for.cond1
  %arrayidx7 = getelementptr inbounds nuw [1200 x double], ptr %data, i64 %indvars.iv, i64 %indvars.iv16
...
if.then_after_outline.exitStub:
  ...
  %1 = tail call i64 @fwrite(ptr noundef nonnull @.str.4, i64 noundef 50, i64 noundef 1, ptr noundef %0) #19
  ret void
}
...
attributes #17 = { nofree nounwind willreturn }
attributes #18 = { nounwind memory(write) }
