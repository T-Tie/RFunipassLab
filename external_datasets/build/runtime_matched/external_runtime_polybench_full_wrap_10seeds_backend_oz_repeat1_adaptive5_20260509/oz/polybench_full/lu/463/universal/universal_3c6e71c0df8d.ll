...
entry:
  ...
  tail call fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %call) #16
  unreachable
}

; Function Attrs: nofree norecurse noreturn nosync nounwind memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %A) unnamed_addr #8 {
entry:
  br label %for.body
...
for.cond1:                                        ; preds = %for.body3, %for.body
  ...
  %j.0 = phi i64 [ 0, %for.body ], [ 1, %for.body3 ]
  br i1 %cmp2, label %for.body3, label %for.cond8

for.body3:                                        ; preds = %for.cond1
  %arrayidx6 = getelementptr inbounds nuw [2000 x double], ptr %A, i64 0, i64 %j.0
...
for.cond8:                                        ; preds = %for.cond1, %for.body11
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body11 ], [ 0, %for.cond1 ]
  %exitcond.not = icmp eq i64 %indvars.iv, 1999
  br i1 %exitcond.not, label %for.end18, label %for.body11

for.body11:                                       ; preds = %for.cond8
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1
  %arrayidx15 = getelementptr inbounds nuw [2000 x double], ptr %A, i64 0, i64 %indvars.iv.next
