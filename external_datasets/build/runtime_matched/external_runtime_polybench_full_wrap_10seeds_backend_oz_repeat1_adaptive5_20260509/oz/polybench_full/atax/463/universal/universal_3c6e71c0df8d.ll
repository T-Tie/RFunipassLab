...
entry:
  ...
  tail call fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %call, ptr nofree noundef nonnull writeonly captures(none) %call1) #16
  unreachable
}

; Function Attrs: nofree norecurse noreturn nosync nounwind memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %A, ptr nofree noundef nonnull writeonly captures(none) %x) unnamed_addr #8 {
entry:
  br label %for.cond
...
for.cond:                                         ; preds = %for.body, %entry
  ...
  br i1 %exitcond.not, label %for.body6, label %for.body
...
for.body6:                                        ; preds = %for.cond7, %for.cond
  br label %for.cond7

for.cond7:                                        ; preds = %for.body10, %for.body6
  %indvars.iv7 = phi i64 [ %indvars.iv.next8, %for.body10 ], [ 0, %for.body6 ]
  %exitcond10.not = icmp eq i64 %indvars.iv7, 2100
  br i1 %exitcond10.not, label %for.body6, label %for.body10

for.body10:                                       ; preds = %for.cond7
  %1 = trunc nuw nsw i64 %indvars.iv7 to i32
  %conv12 = uitofp nneg i32 %1 to double
  %div14 = fdiv double %conv12, 9.500000e+03
  %arrayidx18 = getelementptr inbounds nuw [2100 x double], ptr %A, i64 0, i64 %indvars.iv7
