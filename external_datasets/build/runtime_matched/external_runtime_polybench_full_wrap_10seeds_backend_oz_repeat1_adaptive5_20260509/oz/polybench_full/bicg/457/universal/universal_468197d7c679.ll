...
entry:
  ...
  %call.i = call i32 @gettimeofday(ptr noundef nonnull align 8 captures(none) %Tp.i, ptr noundef null) #13
  %cmp.not.i = icmp eq i32 %call.i, 0
  br i1 %cmp.not.i, label %rtclock.exit, label %if.then.i

if.then.i:                                        ; preds = %entry
  %call1.i = tail call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(1) @.str.3, i32 noundef %call.i) #13
...
rtclock.exit:                                     ; preds = %entry, %if.then.i
  ...
  %2 = tail call double @llvm.fmuladd.f64(double %conv2.i, double noundef 0x3EB0C6F7A0B5ED8D, double %conv.i) #15
...
entry:
  tail call void @free(ptr noundef captures(none) %ptr) #16
  ret void
}
...
if.then.i:                                        ; preds = %entry
  ...
  %2 = call i64 @fwrite(ptr noundef nonnull @.str.4, i64 noundef 50, i64 noundef 1, ptr noundef %1) #17
...
entry:
  %call = tail call nonnull ptr @polybench_alloc_data(i64 noundef 3990000, i32 noundef 8) #13
  %call1 = tail call ptr @polybench_alloc_data(i64 noundef 1900, i32 noundef 8) #13
  %call2 = tail call ptr @polybench_alloc_data(i64 noundef 2100, i32 noundef 8) #13
  %call3 = tail call nonnull ptr @polybench_alloc_data(i64 noundef 1900, i32 noundef 8) #13
  %call4 = tail call nonnull ptr @polybench_alloc_data(i64 noundef 2100, i32 noundef 8) #13
  tail call fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %call, ptr nofree noundef nonnull writeonly align 8 captures(none) %call4, ptr nofree noundef nonnull writeonly align 8 captures(none) %call3) #19
  unreachable
}

; Function Attrs: nofree norecurse noreturn nosync nounwind memory(argmem: write) uwtable
define internal fastcc void @init_array(ptr nofree noundef nonnull writeonly captures(none) %A, ptr nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) initializes((0, 8)) %r, ptr nofree noundef nonnull writeonly align 8 captures(none) dereferenceable(8) initializes((0, 8)) %p) unnamed_addr #11 {
entry:
  store double 0.000000e+00, ptr %p, align 8, !tbaa !11
  br label %for.body5

for.body5:                                        ; preds = %entry, %for.end26
  %indvars.iv10 = phi i64 [ 0, %entry ], [ %indvars.iv.next11, %for.end26 ]
  store double 0.000000e+00, ptr %r, align 8, !tbaa !11
  br label %for.cond12

for.cond12:                                       ; preds = %for.body15, %for.body5
  %indvars.iv = phi i64 [ %indvars.iv.next, %for.body15 ], [ 0, %for.body5 ]
  %exitcond.not = icmp eq i64 %indvars.iv, 1900
  br i1 %exitcond.not, label %for.end26, label %for.body15

for.body15:                                       ; preds = %for.cond12
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1
  %0 = mul nuw nsw i64 %indvars.iv.next, %indvars.iv10
  %1 = trunc nuw i64 %0 to i32
  %rem16 = urem i32 %1, 2100
  %conv17 = uitofp nneg i32 %rem16 to double
  %div19 = fdiv double %conv17, 2.100000e+03
  %arrayidx23 = getelementptr inbounds nuw [1900 x double], ptr %A, i64 %indvars.iv10, i64 %indvars.iv
...
attributes #19 = { nofree noreturn nounwind memory(write) }
